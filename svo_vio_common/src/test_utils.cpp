...
std::tuple<Keypoints, Bearings, Positions> generateRandomVisible3dPoints(
    ...
    for (uint32_t i = 0u; i < num_points; ++i) {
        pos.col(i) *= sampleUniformRealDistribution<FloatType>(false, min_depth,
                                                               max_depth);
    }
...
FloatType overlappingFieldOfView(const CameraBundle& rig,
    ...
    Keypoints px_A =
        generateUniformKeypoints(rig.getCamera(cam_A).imageWidth(),
                                 rig.getCamera(cam_A).imageHeight(), 0u, 20u);
    Bearings f_A;
    std::vector<bool> success(px_A.cols(), false);
    rig.getCamera(cam_A).backProject3(Eigen::Ref<Keypoints>(px_A), &f_A,
                                      &success);
    Transformation T_B_A =
        rig.get_T_C_B(cam_B) * rig.get_T_C_B(cam_A).inverse();
    ...
    for (int i = 0; i < px_B.cols(); ++i) {
        ...
        if (p_B.col(i)(2) > 0 &&
            isVisible(rig.getCamera(cam_B).imageWidth(),
                      rig.getCamera(cam_B).imageHeight(), px_B.col(i))) {
            ++num_visible;
        }
    }
