...
FloatType overlappingFieldOfView(const CameraBundle& rig,
...
bool isVisible(const typename DerivedKeyPoint::Scalar image_width,
               const typename DerivedKeyPoint::Scalar image_height,
               const Eigen::MatrixBase<DerivedKeyPoint>& px) {
    return px[0] >= 0 && px[1] >= 0 && px[0] < image_width &&
           px[1] < image_height;
}

//! Return if pixel px is within image boundaries with margin.
template <typename DerivedKeyPoint>
bool isVisibleWithMargin(const typename DerivedKeyPoint::Scalar image_width,
                         const typename DerivedKeyPoint::Scalar image_height,
                         const Eigen::MatrixBase<DerivedKeyPoint>& px,
                         const typename DerivedKeyPoint::Scalar margin) {
    return px[0] >= margin && px[1] >= margin &&
           px[0] < (image_width - margin) && px[1] < (image_height - margin);
}

//! Return if pixel px is within image boundaries with margin.
inline bool isVisibleWithMargin(const int image_width,
                                const int image_height,
                                const int x,
                                const int y,
                                const int margin) {
    return x >= margin && y >= margin && x < (image_width - margin) &&
...
randomMatrixUniformDistributed(int rows,
    ...
    for (int x = 0; x < cols; ++x) {
        for (int y = 0; y < rows; ++y) {
            m(y, x) = sampleUniformRealDistribution<FloatType>(deterministic,
                                                               from, to);
        }
    }
    return m;
...
Eigen::Matrix<FloatType, rows, cols> randomMatrixUniformDistributed(
    bool deterministic = false, FloatType from = 0.0, FloatType to = 1.0) {
...
randomMatrixNormalDistributed(int rows,
    ...
    for (int x = 0; x < cols; ++x) {
        for (int y = 0; y < rows; ++y) {
            m(y, x) =
                sampleNormalDistribution<FloatType>(deterministic, mean, sigma);
        }
    }
    return m;
}
...
template <int rows, int cols>
Eigen::Matrix<FloatType, rows, cols> randomMatrixNormalDistributed(
    bool deterministic = false, FloatType mean = 0.0, FloatType sigma = 1.0) {
    return randomMatrixNormalDistributed(rows, cols, deterministic, mean,
                                         sigma);
}

template <int size>
Eigen::Matrix<FloatType, size, 1> randomVectorNormalDistributed(
    bool deterministic = false, FloatType mean = 0.0, FloatType sigma = 1.0) {
