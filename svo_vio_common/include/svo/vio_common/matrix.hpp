...
inline Eigen::Matrix<FloatType, 3, 3> skewSymmetric(const FloatType w1,
                                                    const FloatType w2,
                                                    const FloatType w3) {
    return (Eigen::Matrix<FloatType, 3, 3>() << 0.0f, -w3, w2, w3, 0.0f, -w1,
            -w2, w1, 0.0f)
        .finished();
}
...
std::tuple<int, FloatType, Eigen::Matrix<FloatType, Eigen::Dynamic, 1> >
directLinearTransform(
    const Eigen::Matrix<FloatType, Eigen::Dynamic, Eigen::Dynamic>& A,
    FloatType rank_tol = 1e-9);
...
getMatrixCols(Eigen::MatrixBase<Derived>& X,
