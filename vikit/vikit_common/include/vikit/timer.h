...
class Timer {
 public:
    ...
    inline void resume() { start_time_ = Clock::now(); }

    /// Returns duration in seconds
    inline double stop() {
        const TimePoint end_time(Clock::now());
        duration_ =
            std::chrono::duration_cast<Nanoseconds>(end_time - start_time_);
    ...
    static double getCurrentTime() {
        return static_cast<double>(std::chrono::duration_cast<Nanoseconds>(
                                       Clock::now() - TimePoint())
                                       .count()) *
               1e-9;
    }
    ...
    static std::string getCurrentTimeStr() {
        ...
        if (t == NULL) return std::string("ERROR");
        std::ostringstream ss;
        ss << t->tm_year - 100 << "-" << std::setw(2) << std::setfill('0')
           << t->tm_mon + 1 << "-" << std::setw(2) << std::setfill('0')
           << t->tm_mday << "_" << std::setw(2) << std::setfill('0')
           << t->tm_hour << "-" << std::setw(2) << std::setfill('0')
           << t->tm_min << "-" << std::setw(2) << std::setfill('0')
           << t->tm_sec;
        return ss.str();
    }
