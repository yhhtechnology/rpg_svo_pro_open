...
inline bool readNextLine(const size_t num_fields_per_line,
    ...
    for (size_t idx = 0u; idx < num_fields_per_line; ++idx) {
        ...
        CHECK(!(*fields)[idx].empty())
            << "Too few fields per line."
            << " Found " << idx + 1 << " fields but expected "
            << num_fields_per_line << ".";
    }
    return true;
}
...
inline Eigen::Vector3d convertStringsToEigenVector3d(
    const std::string& string_x,
...
inline Quaternion convertStringsToQuaternion(const std::string& qw,
                                             const std::string& qx,
                                             const std::string& qy,
                                             const std::string& qz) {
    CHECK(!qw.empty());
    ...
    try {
        ...
        Eigen::Vector4d quat_coeffs(std::stod(qw), std::stod(qx), std::stod(qy),
                                    std::stod(qz));
        ...
        quaternion = Quaternion(quat_coeffs(0), quat_coeffs(1), quat_coeffs(2),
                                quat_coeffs(3));
    } catch (const std::invalid_argument& invalid_argument_exception) {
        ...
        LOG(FATAL) << "Out of range exception. The given tuple '(" << qw << ", "
                   << qx << ", " << qy << ", " << qz
                   << ")' does not seem to fit the definition "
                   << "of an aslam::Quaternion!"
                   << out_of_range_exception.what();
    } catch (const std::exception& exception) {
        LOG(FATAL) << "Conversion of the given tuple '(" << qw << ", " << qx
                   << ", " << qy << ", " << qz << ")' to aslam::Quaternion "
