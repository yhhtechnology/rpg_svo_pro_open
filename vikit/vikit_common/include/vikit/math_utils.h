...
void computeInliersOneView(
    const std::vector<Eigen::Vector3d,
        ...
        feature_sphere_vec,
    const std::vector<Eigen::Vector3d,
                      Eigen::aligned_allocator<Eigen::Vector3d> >& xyz_vec,
...
/// Sampsonus score
double sampsonDistance(const Eigen::Vector3d& f_cur,
...
inline Eigen::Matrix3d rightJacobianExpMapSO3(const Eigen::Vector3d& x) {
    // x is the axis-angle representation (exponential coordinates) for a
    // rotation
    ...
    if (normx < 10e-8) {
        ...
        const Eigen::Matrix3d X =
            vk::skew(x);  // element of Lie algebra so(3): X = x^
        Jr = Eigen::Matrix3d::Identity() -
             ((1 - cos(normx)) / (normx * normx)) * X +
             ((normx - sin(normx)) / (normx * normx * normx)) * X *
                 X;  // right Jacobian
    }
    return Jr;
}
...
inline Eigen::Matrix3d rightJacobianExpMapSO3inverse(const Eigen::Vector3d& x) {
    ...
    if (normx < 10e-8) {
        ...
        const Eigen::Matrix3d X =
            vk::skew(x);  // element of Lie algebra so(3): X = x^
        Jrinv = Eigen::Matrix3d::Identity() + 0.5 * X +
                (1 / (normx * normx) -
                 (1 + cos(normx)) / (2 * normx * sin(normx))) *
                    X * X;
    }
    return Jrinv;
}
...
inline void jacobianFrame_xyz2uv(const Eigen::Vector3d& xyz_in_f,
...
inline void jacobianPoint_xyz2uv(const Eigen::Vector3d& p_in_f,
