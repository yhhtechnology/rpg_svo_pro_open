...
#define TEST_JACOBIAN_FINITE_DIFFERENCE(FUNCTOR_TYPE, X, STEP, TOLERANCE, ...) \
    do {                                                                       \
        ...
        typename aslam::common::NumericalDiffTraits<FUNCTOR_TYPE>::type        \
            numDiff(functor, STEP);                                            \
...
class ZeroNumericalDiff {
 public:
    ...
    bool getJacobianNumerical(const InputType& /* _x */,
 ...
 public:
    ...
    bool getJacobianNumerical(const InputType& _x, JacobianType& jac) const {
        static_assert(InputsAtCompileTime > 0,
                      "Numerical Differentiation does not work for zero-sized "
                      "Jacobians.");
        using std::sqrt;
        using std::abs;
        /* Local variables */
        ...
        for (int j = 0; j < n; ++j) {
            ...
            switch (mode) {
                ...
                case CentralSecond:
                    ...
                    x[j] = _x[j];
                    jac.col(j) = ((8.0 * val2) + val4 - val1 - (8.0 * val3)) /
                                 (h * 12.0);
                    break;
                default:
                    eigen_assert(false);
...
struct NumericalDiffTraits {
    typedef typename NumericalDiffTraitsSelector<
        _Functor,
        mode,
        _Functor::InputsAtCompileTime>::type type;
};
