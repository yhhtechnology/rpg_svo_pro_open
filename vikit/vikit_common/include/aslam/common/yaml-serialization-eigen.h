...
struct convert<Eigen::Matrix<Scalar_, A_, B_, C_, D_, E_> > {
    ...
    static bool decode(const Node& node,
        ...
        if (!node.IsMap()) {
            LOG(ERROR) << "Unable to get parse the matrix because the node is "
                          "not a map.";
            return false;
        }
    ...
    static bool decode(const Node& node,
        ...
        typedef
            typename Eigen::Matrix<Scalar, Eigen::Dynamic, B, C, D, E>::Index
                IndexType;
        IndexType rows = node["rows"].as<IndexType>();
        IndexType cols = node["cols"].as<IndexType>();
    ...
    static bool decode(const Node& node,
        ...
        typedef
            typename Eigen::Matrix<Scalar, A, Eigen::Dynamic, C, D, E>::Index
                IndexType;
        IndexType rows = node["rows"].as<IndexType>();
        IndexType cols = node["cols"].as<IndexType>();
