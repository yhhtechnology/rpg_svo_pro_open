...
Homography estimateHomography(const Bearings& f_cur,
    ...
    if (decomp[1].score / decomp[0].score < 0.9) {
        ...
        // Sampson error can be roughly thought as the squared distance between
        // a
        ...
        for (Homography& D : decomp) {
            ...
            const Matrix3d E_cur_ref =
                D.R_cur_ref * vk::skew(D.t_cur_ref);  // Essential Matrix
            for (size_t i = 0; i < N; ++i) {
                const double d =
                    vk::sampsonDistance(f_cur.col(i), E_cur_ref, f_ref.col(i));
