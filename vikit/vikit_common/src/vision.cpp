...
void halfSample(const cv::Mat& in, cv::Mat& out) {
    ...
    if (is_aligned16(in.data) && is_aligned16(out.data) &&
        ((in.cols % 16) == 0) && static_cast<size_t>(in.cols) == in.step) {
    ...
    for (int y = 0; y < out.rows && bottom < end;
         y++, top += in_stride * 2, bottom += in_stride * 2, p += out_stride) {
        for (int x = 0; x < out.cols; x++) {
            p[x] = static_cast<uint8_t>((uint16_t(top[x * 2]) + top[x * 2 + 1] +
                                         bottom[x * 2] + bottom[x * 2 + 1]) /
                                        4);
        }
    }
}
...
void calcSharrDeriv(const cv::Mat& src, cv::Mat& dst) {
    ...
    for (y = 0; y < rows; y++) {
...
#ifdef __SSE2__
        for (; x <= colsn - 8; x += 8) {
            __m128i s0 = _mm_unpacklo_epi8(
                _mm_loadl_epi64((const __m128i*)(srow0 + x)), z);
            __m128i s1 = _mm_unpacklo_epi8(
                _mm_loadl_epi64((const __m128i*)(srow1 + x)), z);
            __m128i s2 = _mm_unpacklo_epi8(
                _mm_loadl_epi64((const __m128i*)(srow2 + x)), z);
            __m128i t0 =
                _mm_add_epi16(_mm_mullo_epi16(_mm_add_epi16(s0, s2), c3),
        ...
        // do horizontal convolution, interleave the results and store them to
        // dst
        x = 0;
#ifdef __SSE2__
        for (; x <= colsn - 8; x += 8) {
            ...
            __m128i t1 =
                _mm_add_epi16(_mm_mullo_epi16(_mm_add_epi16(s2, s4), c3),
                              _mm_mullo_epi16(s3, c10));
            __m128i t2 = _mm_unpacklo_epi16(t0, t1);
            t0 = _mm_unpackhi_epi16(t0, t1);
            // this can probably be replaced with aligned stores if we aligned
            // dst properly.
...
#ifdef __SSE2__
void convertRawDepthImageSse_16u_to_32f(cv::Mat& depth_16u,
                                        cv::Mat& depth_32f,
                                        float scale) {
    ...
    for (int idx = 0; idx < depth_16u.size().area();
        ...
        _input =
            _mm_cvtepi32_ps(_mm_unpacklo_epi16(_inputi, _mm_setzero_si128()));
        ...
        _input =
            _mm_cvtepi32_ps(_mm_unpackhi_epi16(_inputi, _mm_setzero_si128()));
