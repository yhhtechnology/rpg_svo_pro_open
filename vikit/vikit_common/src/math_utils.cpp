...
bool depthFromTriangulationExact(const Matrix3d& R_r_c,
    ...
    if (std::fabs(denom) < 0.000001) return false;
...
double computeInliers(const Bearings& features1,  ///< c1
    ...
    inliers.clear();
    inliers.reserve(features1.cols());
    outliers.clear();
    outliers.reserve(features1.cols());
    ...
    for (int j = 0; j < features1.cols(); ++j) {
        xyz_vec_in_1.col(j) = triangulateFeatureNonLin(
            R_12, t, features1.col(j), features2.col(j));
        double e1 = reprojError(features1.col(j), xyz_vec_in_1.col(j),
                                error_multiplier2);
        double e2 = reprojError(features2.col(j),
                                R_12.transpose() * (xyz_vec_in_1.col(j) - t),
                                error_multiplier2);
...
void computeInliersOneView(
    const std::vector<Vector3d, aligned_allocator<Vector3d> >&
        feature_sphere_vec,
    ...
    for (size_t j = 0; j < xyz_vec.size(); j++) {
        double e =
            reprojError(feature_sphere_vec[j], R.transpose() * (xyz_vec[j] - t),
                        error_multiplier2);
        if (e < reproj_thresh)
            inliers.push_back(j);
        else
...
double sampsonDistance(const Vector3d& f_cur,
