...
void loadData(vk::Bearings& f_ref, vk::Bearings& f_cur, double& focal_length) {
...
TEST(Homography, testHomography) {
    ...
    vk::Homography H = vk::estimateHomography(f_cur, f_ref, focal_length,
                                              reproj_error_thresh, 0.0);

    // compute inliers
    std::vector<int> inliers, outliers;
    vk::Bearings xyz_in_cur;
    vk::computeInliers(f_cur, f_ref, H.R_cur_ref, H.t_cur_ref,
                       reproj_error_thresh, focal_length, xyz_in_cur, inliers,
                       outliers);
