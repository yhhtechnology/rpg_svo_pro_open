...
class CamerasTest : public ::testing::Test {
 protected:
    virtual void SetUp() {
        ...
        omni_cam_ = vk::cameras::CameraGeometryBase::loadFromYaml(
            data_dir_ + "/calib_omni.yaml");
...
TEST_F(CamerasTest, EquidistantDistortion) {
    double x = 0.5, y = 0.8;
    vk::cameras::EquidistantDistortion distortion_model(-0.0027, 0.0241,
                                                        -0.0430, 0.0311);
    distortion_model.distort(x, y);
    ...
    TEST_JACOBIAN_FINITE_DIFFERENCE(EqJacobianFunctor, Eigen::Vector2d(x, y),
...
TEST_F(CamerasTest, RadialTangentialDistortion) {
    ...
    distortion_model.distort(x, y);
    distortion_model.undistort(x, y);
    // NOTE: with radtan we don't achieve same precision with only five
    // iterations
    // at this pose (0.5, 0.8) far from image center
    EXPECT_NEAR(x, 0.5, 1e-2);
    EXPECT_NEAR(y, 0.8, 1e-2);

    struct RadialTangentialJacobianFunctor
        : public aslam::common::NumDiffFunctor<2, 2> {
        RadialTangentialJacobianFunctor(
            const vk::cameras::RadialTangentialDistortion& dist)
            : dist_(dist){};
    ...
    constexpr double kStepSize = 1e-4;
    constexpr double kEpsJacobian = 1e-4;
    TEST_JACOBIAN_FINITE_DIFFERENCE(RadialTangentialJacobianFunctor,
                                    Eigen::Vector2d(x, y), kStepSize,
                                    kEpsJacobian, distortion_model);
}
...
TEST_F(CamerasTest, CameraYaml) {
    vk::cameras::CameraGeometryBase::Ptr cam =
        vk::cameras::CameraGeometryBase::loadFromYaml(data_dir_ +
                                                      "/calib_cam.yaml");
...
TEST(CameraTest, EquidistantProjection) {
    ...
    vk::cameras::Camera::Ptr cam =
        std::make_shared<vk::cameras::EquidistantFisheyeGeometry>(
            640, 480, f, Eigen::Vector2d(320, 240), 320);
    // Forward
...
TEST_F(CamerasTest, OmniCameraBackprojection) {
    ...
    const Eigen::Vector3d expected_bearing(0.733011271294813, 0.549758453471110,
                                           0.400574735838165);

    Eigen::Vector3d bearing;
    omni_cam_->backProject3(keypoint, &bearing);
    EXPECT_TRUE(
        EIGEN_MATRIX_NEAR(expected_bearing, bearing, kEpsBackprojection));
}

TEST_F(CamerasTest, OmniCameraProjection) {
    constexpr double kEpsProjection = 1e-4;
    const Eigen::Vector3d landmark(1.0, 1.0, -1.0);
    const Eigen::Vector2d expected_keypoint(4.729118411664447e+02,
                                            3.929118411664447e+02);
...
TEST_F(CamerasTest, OmniCameraJacobian) {
    struct OmniJacobianFunctor : public aslam::common::NumDiffFunctor<2, 3> {
        OmniJacobianFunctor(vk::cameras::OmniGeometry::Ptr cam) : cam_(cam){};
    ...
    TEST_JACOBIAN_FINITE_DIFFERENCE(OmniJacobianFunctor, landmark, kStepSize,
                                    kEpsJacobian, omni_cam_);
}
