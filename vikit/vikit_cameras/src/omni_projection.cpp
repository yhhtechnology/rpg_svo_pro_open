...
Eigen::Matrix2d distortionToAffineCorrection(
...
OmniProjection::OmniProjection(
    ...
    : polynomial_(polynomial),
      ...
      affine_correction_(
          omni_projection::distortionToAffineCorrection(distortion)),
...
void OmniProjection::project3(
    ...
    if (out_jacobian_point) {
        ...
        double duraw_dx =
            (xy_norm - x * x / xy_norm) / xy_sqr * rho + drho_dx * x / xy_norm;
        double duraw_dy =
            (-1.0 * x * y / xy_norm) / xy_sqr * rho + drho_dy * x / xy_norm;
        double duraw_dz = drho_dz * x / xy_norm;
        double dvraw_dx =
            (-1.0 * x * y / xy_norm) / xy_sqr * rho + drho_dx * y / xy_norm;
        double dvraw_dy =
            (xy_norm - y * y / xy_norm) / xy_sqr * rho + drho_dy * y / xy_norm;
        double dvraw_dz = drho_dz * y / xy_norm;
        (*out_jacobian_point) << duraw_dx, duraw_dy, -duraw_dz, dvraw_dx,
            dvraw_dy, -dvraw_dz;
...
double OmniProjection::errorMultiplier() const { return 1.0; }

double OmniProjection::getAngleError(double img_err) const {
    Eigen::Vector3d center_f;
    backProject3(principal_point_, &center_f);
    const Eigen::Vector2d offset_x(principal_point_(0) + img_err,
                                   principal_point_(1));
    Eigen::Vector3d offset_x_f;
    backProject3(offset_x, &offset_x_f);
    const Eigen::Vector2d offset_y(principal_point_(0),
                                   principal_point_(1) + img_err);
...
void OmniProjection::print(std::ostream& out) const {
    ...
    out << "  Inverse polynomial = " << inverse_polynomial_.transpose()
        << std::endl;
    out << "  Affine correction = " << std::endl
        << affine_correction_ << std::endl;
