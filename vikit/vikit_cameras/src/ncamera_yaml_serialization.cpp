...
bool convert<std::shared_ptr<vk::cameras::NCamera> >::decode(
    const Node& node, vk::cameras::NCamera::Ptr& ncamera) {
    ncamera.reset();
    try {
        if (!node.IsMap()) {
            LOG(ERROR)
                << "Unable to parse the ncamera because the node is not a map.";
            return true;
        }
        ...
        {
          LOG(WARNING) << "Unable to get the id for the ncamera. Generating new
        random id.";
          ncam_id.randomize();
        } else {
        ...
        */
        ...
        if (!cameras_node.IsSequence()) {
            LOG(ERROR) << "Unable to parse the cameras because the camera node "
                          "is not a sequence.";
            return true;
        }
        ...
        for (size_t camera_index = 0; camera_index < num_cameras;
             ++camera_index) {
            // Decode the camera
            const Node& camera_node = cameras_node[camera_index];
            if (!camera_node) {
                LOG(ERROR) << "Unable to get camera node for camera "
                           << camera_index;
                return true;
            }
            ...
            // Get the transformation matrix T_B_C (takes points from the frame
            // C to frame B).
            Eigen::Matrix4d T_B_C_raw;
            if (!YAML::safeGet(camera_node, "T_B_C", &T_B_C_raw)) {
                LOG(ERROR) << "Unable to get extrinsic transformation T_B_C "
                              "for camera "
                           << camera_index;
                return true;
            }
            ...
            vk::cameras::Transformation T_B_C(q_B_C,
                                              T_B_C_raw.block<3, 1>(0, 3));
...
bool convert<vk::cameras::NCamera>::decode(const Node& /*node*/,
                                           vk::cameras::NCamera& /*ncamera*/) {
    LOG(FATAL) << "Not implemented!";
    return false;
}

Node convert<vk::cameras::NCamera>::encode(
    const vk::cameras::NCamera& ncamera) {
    Node ncamera_node;
    ...
    */
    ...
    for (size_t camera_index = 0; camera_index < num_cameras; ++camera_index) {
        ...
        camera_node["T_B_C"] =
            ncamera.get_T_C_B(camera_index).inverse().getTransformationMatrix();
