...
CameraGeometryBase::Ptr makePinholeCamera(const Eigen::VectorXd& intrinsics,
    ...
    CameraGeometryBase::Ptr cam = std::make_shared<PinholeGeometry>(
        width, height,
        PinholeProjection<NoDistortion>(intrinsics, NoDistortion()));
    return cam;
}
...
CameraGeometryBase::Ptr loadFromYAML(const std::string& filename,
    ...
    if (!data) {
        std::cerr << "Could not load camera from file: " << filename
                  << std::endl;
        return nullptr;
    }
    ...
    if (cam_model == "PinholeRadialTangential") {
        double d0 =
            data["cam_d0"].IsDefined() ? data["cam_d0"].as<double>() : 0.0;
        double d1 =
            data["cam_d1"].IsDefined() ? data["cam_d1"].as<double>() : 0.0;
        double d2 =
            data["cam_d2"].IsDefined() ? data["cam_d2"].as<double>() : 0.0;
        double d3 =
            data["cam_d3"].IsDefined() ? data["cam_d3"].as<double>() : 0.0;
        if (d0 == 0 && d1 == 0 && d2 == 0 && d3 == 0) {
            cam_model = "PinholeNoDistortion";
        } else {
            cam.reset(new PinholeRadTanGeometry(
                data["cam_width"].as<int>(), data["cam_height"].as<int>(),
                PinholeProjection<RadialTangentialDistortion>(
                    data["cam_fx"].as<double>(), data["cam_fy"].as<double>(),
                    data["cam_cx"].as<double>(), data["cam_cy"].as<double>(),
    ...
    if (cam_model == "PinholeNoDistortion") {
        ...
        cam.reset(new PinholeEquidistantGeometry(
            ...
            PinholeProjection<EquidistantDistortion>(
                data["cam_fx"].as<double>(), data["cam_fy"].as<double>(),
                data["cam_cx"].as<double>(), data["cam_cy"].as<double>(),
                EquidistantDistortion(data["cam_d0"].as<double>(),
        ...
        cam.reset(new PinholeAtanGeometry(
            ...
            PinholeProjection<AtanDistortion>(
                data["cam_fx"].as<double>(), data["cam_fy"].as<double>(),
                data["cam_cx"].as<double>(), data["cam_cy"].as<double>(),
                AtanDistortion(data["cam_d0"].as<double>()))));
    } else if (cam_model == "Pinhole") {
        std::cerr
            << "ERROR: Camera Factory: Select between 'PinholeNoDistortion' "
