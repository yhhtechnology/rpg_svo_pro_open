...
CameraGeometryBase::Ptr CameraGeometryBase::loadFromYaml(
    ...
    try {
        ...
        LOG(ERROR) << "Failed to load Camera from file " << yaml_file
                   << " with the error: \n"
                   << ex.what();
    }
...
bool CameraGeometryBase::isMasked(
    ...
    return keypoint[0] < 0.0 || keypoint[0] >= static_cast<double>(width_) ||
           keypoint[1] < 0.0 || keypoint[1] >= static_cast<double>(height_) ||
           (!mask_.empty() &&
