...
bool convert<std::shared_ptr<vk::cameras::CameraGeometryBase> >::decode(
    const Node& node, vk::cameras::CameraGeometryBase::Ptr& camera) {
    camera.reset();
    try {
        if (!node.IsMap()) {
            LOG(ERROR) << "Unable to get parse the camera because the node is "
                          "not a map.";
            return true;
        }
        ...
        if (YAML::safeGet(distortion_config, "type", &distortion_type) &&
            YAML::safeGet(distortion_config, "parameters",
            ...
            if (camera_type == "pinhole" && distortion_type == "none") {
                ...
                camera.reset(new vk::cameras::PinholeEquidistantGeometry(
                    image_width, image_height,
                    vk::cameras::PinholeProjection<
                        vk::cameras::EquidistantDistortion>(
                        intrinsics, vk::cameras::EquidistantDistortion(
                                        distortion_parameters))));
            } else if (camera_type == "pinhole" &&
...
Node convert<vk::cameras::CameraGeometryBase::Ptr>::encode(
    ...
    return convert<vk::cameras::CameraGeometryBase>::encode(
...
template <>
void encodeDistortion(const vk::cameras::NoDistortion& distortion,
...
template <>
void encodeDistortion(const vk::cameras::RadialTangentialDistortion& distortion,
                      Node* distortion_node) {
    CHECK_NOTNULL(distortion_node);
    (*distortion_node)["type"] = "radial-tangential";
    Eigen::VectorXd parameters(4, 1);
    parameters << distortion.k1_, distortion.k2_, distortion.p1_,
        distortion.p2_;
    (*distortion_node)["parameters"] = parameters;
}

template <>
void encodeDistortion(const vk::cameras::EquidistantDistortion& distortion,
                      Node* distortion_node) {
    CHECK_NOTNULL(distortion_node);
    (*distortion_node)["type"] = "equidistant";
    Eigen::VectorXd parameters(4, 1);
    parameters << distortion.k1_, distortion.k2_, distortion.k3_,
        distortion.k4_;
    (*distortion_node)["parameters"] = parameters;
}

template <>
void encodeDistortion(const vk::cameras::AtanDistortion& distortion,
                      Node* distortion_node) {
    CHECK_NOTNULL(distortion_node);
...
bool encodePinhole(const vk::cameras::CameraGeometryBase& camera,
    ...
    if (ConstGeometryPtr pinhole_camera =
            dynamic_cast<ConstGeometryPtr>(&camera)) {
