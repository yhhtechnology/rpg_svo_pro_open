...
bool convert<std::shared_ptr<vk::cameras::CameraGeometryBase> >::decode(
  ...
  camera.reset();
  try {
    printf("decode shared_ptr in camera_yaml_serialization.cpp !!!!!!!!!  \n");
    if(!node.IsMap()) {
       ...
       YAML::safeGet(node, "intrinsics", &intrinsics))
    {
      ...
      else if(camera_type == "unified" && distortion_type == "radial-tangential")
      {
        // MEI model
        double xi;
        YAML::safeGet(node, "xi", &xi);
        std::cout<<"xi = "<<xi<<"!!!!!!!!!!!!!!!!! \n";
        camera.reset(new vk::cameras::UnifiedRadTanGeometry(
                    image_width, image_height,
                    vk::cameras::UnifiedProjection<vk::cameras::RadialTangentialDistortion>(
                      xi,
                      intrinsics,
                      vk::cameras::RadialTangentialDistortion(distortion_parameters))));
      }
      ...
      {
        // 这里的 omni 好像是和VINS的 OCAMCamera 一样
