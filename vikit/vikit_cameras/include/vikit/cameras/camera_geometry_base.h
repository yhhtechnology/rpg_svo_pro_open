...
struct ProjectionResult {
    ...
    ///          camera_->project3(Eigen::Vector3d(0, 0, -10), &keypoint);
...
class CameraGeometryBase {
 public:
    ...
    /// Load a camera rig form a yaml file. Returns a nullptr if the loading
    /// fails.
    static Ptr loadFromYaml(const std::string& yaml_file);

    /// Computes bearing vector from pixel coordinates. Z-component of the
    /// returned
    ...
    /// Computes pixel coordinates from bearing vector with Jacobian w.r.t.
    /// point.
    ...
    virtual void printParameters(std::ostream& out,
                                 const std::string& s = "Camera: ") const = 0;

    /// Equivalent to focal length for projective cameras and factor for
    /// omni-directional cameras that transforms small angular error to
    /// pixel-error.
    ...
    /// CameraType value representing the camera model used by the derived
    /// class.
    ...
    /// Return if a given keypoint is within the specified margin to the
    /// boundary
    ...
    /// Set the mask. Masks must be the same size as the image and they follow
    /// the same
