...
class AtanDistortion {
 public:
    ...
    AtanDistortion(const double& s) { initializeParameters(s); }

    AtanDistortion(const Eigen::VectorXd& parameters) {
    ...
    inline void distort(double& x, double& y) const {
        ...
        const double factor =
            (r < 0.001) ? 1.0 : s_inv_ * std::atan(r * tans_) / r;
        x *= factor;
        y *= factor;
    }
    ...
    enum IntrinsicParameters { kRadialDistortionFactor };
