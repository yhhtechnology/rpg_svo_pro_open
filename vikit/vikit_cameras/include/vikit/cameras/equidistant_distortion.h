...
class EquidistantDistortion {
 public:
    EquidistantDistortion(const double k1,
                          const double k2,
                          const double k3,
                          const double k4)
    ...
    inline Eigen::Matrix2d jacobian(const Eigen::Vector2d& uv) const {
        ...
        const double scaling = thetad / r;
        const double dscaling_du =
            (dthetad_dr * dr_du * r - dr_du * thetad) / r2;
        const double dscaling_dv =
            (dthetad_dr * dr_dv * r - dr_dv * thetad) / r2;
    ...
    inline void undistort(double& x, double& y) const {
        ...
        for (int i = 0; i < 5; ++i) {
            ...
            theta = thetad / (1.0 + k1_ * theta2 + k2_ * theta4 + k3_ * theta6 +
                              k4_ * theta8);
        }
