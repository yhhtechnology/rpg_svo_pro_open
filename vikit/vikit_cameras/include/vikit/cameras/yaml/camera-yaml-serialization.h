...
struct convert<std::shared_ptr<vk::cameras::CameraGeometryBase>> {
    ...
    /// This function was written to *not* throw exceptions. Hence, decode
    /// always
    /// returns true, but when it fails, the shared pointer will be null.
    static bool decode(
        const Node& node,
        std::shared_ptr<vk::cameras::CameraGeometryBase>& camera);
    static Node encode(
        const std::shared_ptr<vk::cameras::CameraGeometryBase>& camera);
};

template <>
struct convert<vk::cameras::CameraGeometryBase> {
    static bool decode(const Node& node,
                       vk::cameras::CameraGeometryBase& camera);
