...
float NormalDistributionScaleEstimator::compute(
    ...
    float var = 0.0;
    for (const float d : errors) var += (d - mean) * (d - mean);
...
// Weight Functions
/* ************************************************************************* */
float UnitWeightFunction::weight(const float& error) const { return 1.0f; }

TukeyWeightFunction::TukeyWeightFunction(const float b) : b_square_(b * b) {}
...
HuberWeightFunction::HuberWeightFunction(const float k) : k_(k) {}
