...
template <int D, typename T, typename Implementation>
void MiniLeastSquaresSolver<D, T, Implementation>::optimizeGaussNewton(
    State& state) {
    ...
    for (iter_ = 0; iter_ < solver_options_.max_iter; ++iter_) {
        ...
        if ((iter_ > 0 && new_chi2 > chi2_ &&
             solver_options_.stop_when_error_increases) ||
            stop_) {
        ...
        if (x_norm < solver_options_.eps) {
            VLOG(400) << "Converged, x_norm " << x_norm << " < "
                      << solver_options_.eps;
            break;
        }
    }
...
template <int D, typename T, typename Implementation>
void MiniLeastSquaresSolver<D, T, Implementation>::optimizeLevenbergMarquardt(
    State& state) {
    // init parameters
    ...
    for (iter_ = 0; iter_ < solver_options_.max_iter; ++iter_) {
        ...
        do {
            ...
            // solve the linear system to obtain small perturbation in direction
            // of gradient
            if (solve(H_, g_, dx_)) {
            ...
            if (rho_ > 0) {
                ...
                mu_ *= std::max(
                    1. / 3., std::min(1. - std::pow(2 * rho_ - 1, 3), 2. / 3.));
                nu_ = 2.;
                VLOG(400) << "It. " << iter_ << "\t Trial " << trials_
                          << "\t Success"
                          << "\t n_meas = " << n_meas_
                          << "\t new_chi2 = " << new_chi2 << "\t mu = " << mu_
                          << "\t nu = " << nu_;
            } else {
...
bool MiniLeastSquaresSolver<D, T, Implementation>::solveDefaultImpl(
    const HessianMatrix& H, const GradientVector& g, UpdateVector& dx) {
    dx = H.ldlt().solve(g);
