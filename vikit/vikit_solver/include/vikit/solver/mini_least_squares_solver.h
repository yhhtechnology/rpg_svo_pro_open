...
enum class Strategy { GaussNewton, LevenbergMarquardt };
...
struct MiniLeastSquaresSolverOptions {
    ...
    /// this
    /// ensures that x is a descent direction. If mu is large, x is a short step
    /// in
    /// the steepest direction. This is good if the current iterate is far from
    /// the
...
class MiniLeastSquaresSolver {
 ...
 public:
    ...
    void setPrior(const State& prior, const Matrix<double, D, D>& Information);

    /// Reset all parameters to restart the optimization
    void reset();

    /// Get the squared error
    inline double getError() const { return chi2_; }

    /// The the Hessian matrix (Information Matrix).
    inline const Matrix<double, D, D>& getHessian() const { return H_; }
 ...
 protected:
    ...
    /// If these parameters are requested, the system is linearized at the
    /// current
    /// state.
    double evaluateError(const State& state,
                         HessianMatrix* H,
                         GradientVector* g) {
    ...
    bool solve(const HessianMatrix& H,
    ...
    void startIteration() {
        if (&MiniLeastSquaresSolver::startIteration !=
            &Implementation::startIteration)
            impl().startIteration();
    }

    void finishIteration() {
        if (&MiniLeastSquaresSolver::finishIteration !=
            &Implementation::finishIteration)
            impl().finishIteration();
    }

    void finishTrial() {
        if (&MiniLeastSquaresSolver::finishTrial !=
            &Implementation::finishTrial)
            impl().finishTrial();
    }

 private:
    /// Default implementation to solve the linear system H*dx = g to obtain
    /// optimal perturbation dx.
    bool solveDefaultImpl(const HessianMatrix& H,
                          const GradientVector& g,
                          UpdateVector& dx);
 ...
 protected:
    ...
    double chi2_ =
        0.0;  ///< Whitened error / log-likelihood: 1/(2*sigma^2)*(z-h(x))^2.
    double rho_ = 0.0;  ///< Error reduction: chi2-new_chi2.
    double mu_ = 0.01;  ///< Damping parameter.
    double nu_ =
        2.0;  ///< Factor that specifies how much we increase mu at every trial.
