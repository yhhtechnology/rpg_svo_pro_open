...
struct BearingVectorFunctor : public aslam::common::NumDiffFunctor<3, 6> {
    BearingVectorFunctor(const Transformation& T_cam_imu,
                         const Eigen::Vector3d& p_in_imu)
        : T_cam_imu_(T_cam_imu), p_in_imu_(p_in_imu) {}
    ...
    virtual bool functional(const typename NumDiffFunctor::InputType& x,
        ...
        if (Jout) Frame::jacobian_xyz2f_imu(T_cam_imu_, p_in_imu, *Jout);
...
struct UnitPlaneFunctor : public aslam::common::NumDiffFunctor<2, 6> {
    UnitPlaneFunctor(const Transformation& T_cam_imu,
                     const Eigen::Vector3d& p_in_imu,
                     const Eigen::Vector3d& f_measured)
        : T_cam_imu_(T_cam_imu), p_in_imu_(p_in_imu), f_measured_(f_measured) {}
    ...
    virtual bool functional(const typename NumDiffFunctor::InputType& x,
        ...
        const Eigen::Vector3d p_in_imu = T_update * p_in_imu_;
        const Eigen::Vector3d p_in_cam = T_cam_imu_ * p_in_imu;
        if (Jout) Frame::jacobian_xyz2uv_imu(T_cam_imu_, p_in_imu, *Jout);
...
struct ImagePlaneFunctor : public aslam::common::NumDiffFunctor<2, 6> {
    ImagePlaneFunctor(std::shared_ptr<const svo::Camera> cam,
                      const Transformation& T_cam_imu,
                      const Eigen::Vector3d& p_in_imu)
        : cam_(cam), T_cam_imu_(T_cam_imu), p_in_imu_(p_in_imu) {}
...
class SVOFrameTest : public ::testing::Test {
 protected:
    SVOFrameTest() { std::srand(std::time(NULL)); }
};
...
TEST_F(SVOFrameTest, testBearingVectorJacobian) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        TEST_JACOBIAN_FINITE_DIFFERENCE(BearingVectorFunctor, input, 1e-6, 1e-5,
                                        T_cam_imu, p_in_imu);
    }
}
...
TEST_F(SVOFrameTest, testUnitPlaneJacobian) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        while (true) {
            ...
            if (p_in_cam(2) > 1e-3 && f_in_cam(2) > 1e-3) break;
        }
        f_in_imu.normalize();
        TEST_JACOBIAN_FINITE_DIFFERENCE(UnitPlaneFunctor, input, 1e-7, 1e-3,
...
TEST_F(SVOFrameTest, testOmniImageJacobian) {
    ...
    svo::CameraPtr omni_cam = svo::CameraPtr(new vk::cameras::OmniGeometry(
    ...
    for (int i = 0; i < 10; i++) {
        ...
        while (true) {
            p_in_imu = test_utils::generateRandomPoint(1, 10);
            Eigen::Vector2d kp;
            if (omni_cam->project3(T_cam_imu * p_in_imu, &kp)
                    .isKeypointVisible())
                break;
        }
        // on image plane, should be 2-order higher than unit plane error
        TEST_JACOBIAN_FINITE_DIFFERENCE(ImagePlaneFunctor, input, 1e-4,
                                        5 * 1e-2, omni_cam, T_cam_imu,
                                        p_in_imu);
    }
}
...
TEST_F(SVOFrameTest, testPinholeImageJacobian) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        while (true) {
            p_in_imu = test_utils::generateRandomPoint(1, 10);
            Eigen::Vector2d kp;
            if (pinhole_cam->project3(T_cam_imu * p_in_imu, &kp)
                    .isKeypointVisible())
                break;
        }
        // on image plane, should be 2-order higher than unit plane error
        TEST_JACOBIAN_FINITE_DIFFERENCE(ImagePlaneFunctor, input, 1e-4,
                                        5 * 1e-2, pinhole_cam, T_cam_imu,
                                        p_in_imu);
    }
    ...
    timer.stop();
    std::cout << "Analytical inline function(ms):" << timer.getMilliseconds()
              << std::endl;
    timer.start();
    for (int i = 0; i < 100; i++)
        Frame::jacobian_xyz2image_imu(*pinhole_cam, T_cam_imu, p_in_imu,
                                      frame_jac);
    timer.stop();
    std::cout << "Using vikit library(ms):" << timer.getMilliseconds()
              << std::endl;
}
