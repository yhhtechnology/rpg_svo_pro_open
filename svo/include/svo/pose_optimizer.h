...
class PoseOptimizer : public vk::solver::MiniLeastSquaresSolver<6,
                                                                Transformation,
                                                                PoseOptimizer> {
 public:
    ...
    double focal_length_ =
        1.0;  /// focal length TODO: should be different for every camera
    std::ofstream ofs_reproj_errors_;

    double evaluateError(const Transformation& T_imu_world,
                         HessianMatrix* H,
                         GradientVector* g);

    double evaluateErrorImpl(const Transformation& T_imu_world,
                             HessianMatrix* H,
                             GradientVector* g,
                             std::vector<float>* unwhitened_errors);

    void removeOutliers(const double reproj_err_threshold,
                        Frame* frame,
                        std::vector<double>* reproj_errors,
                        size_t* n_deleted_edges,
                        size_t* n_deleted_corners);

    void update(const State& T_frameold_from_world,
