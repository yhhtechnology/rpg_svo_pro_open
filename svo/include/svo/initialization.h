...
struct InitializationOptions {
    ...
    /// it is much better to have more disparity to ensure the initial
    /// pointcloud
    /// is good.
    double init_min_disparity = 50.0;

    /// When checking whether the disparity of the tracked features are large
    /// enough,
    /// we check that a certain percentage of tracked features have disparities
    /// large
    /// than init_min_disparity. The default percentage is 0.5, which means the
    /// median
    /// is checked. For example, if this parameter is set to 0.25, it means we
    /// go for
    /// realtive pose estimation only when at least 25% of the tracked features
    /// have
    ...
    /// first frame, the first frame is not accepted and we check the next
    /// frame.
    ...
    /// and check the quality of the triangulation by evaluating the
    /// reprojection
    /// errors. All points that have more reprojection error than
    /// reproj_error_thresh
    /// are considered outliers. Only return SUCCESS if we have more inliers
    /// than  init_min_inliers.
    size_t init_min_inliers = 40;

    /// Reprojection threshold in pixels. The same as we also use in pose
    /// optimizer.
...
class AbstractInitialization {
 public:
    ...
    double depth_at_current_frame_ =
        1.0;  //!< Depth prior of 3D points in current frame
...
/// Tracks features using KLT and then estimate the relative pose using 5pt
/// RANSAC
...
class StereoInit : public AbstractInitialization {
 public:
    StereoInit(const InitializationOptions& init_options,
               const FeatureTrackerOptions& tracker_options,
               const DetectorOptions& detector_options,
               const CameraBundlePtr& cams);
