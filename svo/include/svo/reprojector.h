...
class Reprojector {
 public:
    ...
    Reprojector(const ReprojectorOptions& options, size_t camera_index);
    ...
    /// A candidate is a point that projects into the image plane and for which
    /// we
    ...
    struct Candidate {
        ...
        Candidate(const FramePtr _ref_frame,
                  const size_t _ref_index,
                  const Keypoint& _cur_px,
                  const size_t _n_reproj,
                  const Score _score,
                  const FeatureType& _type,
                  const size_t _n_obs)
    ...
    size_t camera_index_;  // When using multiple cameras, each camera has a
                           // reprojector.

    /// Project points seed in close_kfs into frame.
    void reprojectFrames(const FramePtr& frame,
 ...
 private:
    inline bool doesFrameHaveEnoughFeatures(const FramePtr& frame) {
        return options_.max_n_features_per_frame > 0 &&
               frame->numTrackedFeatures() >= options_.max_n_features_per_frame;
    }
};
