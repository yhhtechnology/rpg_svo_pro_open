...
enum class KeyframeCriterion { DOWNLOOKING, FORWARD };
...
struct BaseOptions {
    ...
    /// value. This is because from monocular views, we can't estimate the
    /// scale.
    ...
    /// to the last frame by minimizing the photometric error between the
    /// frames.
    ...
    /// similarly to lambda_rot, this parameter trades-off the visual
    /// measurements
    ...
    /// this parameter to randomly sample N features that are used for
    /// alignment.
    ...
    /// the frame. With a good camera and image resolution of 640x480 a
    /// threshold
...
enum class TrackingQuality { kInsufficient, kBad, kGood };
extern const std::
    unordered_map<svo::TrackingQuality, std::string, EnumClassHash>
        kTrackingQualityName;

enum class UpdateResult { kDefault, kKeyframe, kFailure };
...
class FrameHandlerBase
    : public rpg_common::CallbackHost<const FrameBundlePtr&> {
 public:
    ...
    /// Default constructor
    FrameHandlerBase(const BaseOptions& base_options,
    ...
    /// @name Main Interface
    ///
    /// These are the main functions to be used in this class when interfacing
    /// with
    ...
    inline void setInitialCamPose(const Transformation& T_world_cam,
                                  size_t cam_index = 0) {
    ...
    /// These parameters should be private but are currently not for easier
    /// debugging.
 ...
 protected:
    ...
    bool is_sparse_aligment_good_ = true;
    Transformation T_b0b1_;
    ...
    void optimizeStructure(const FrameBundlePtr& frames,
                           int max_n_pts,
                           int max_iter);
    ...
    /// Set all cells in detector to occupied in which a point reprojected
    /// during
    /// the reprojection stage.
    virtual void setDetectorOccupiedCells(const size_t reprojector_grid_idx,
