...
PreintegratedImuMeasurement::PreintegratedImuMeasurement(
    const Eigen::Vector3d& omega_bias, const Eigen::Vector3d& acc_bias)
...
void PreintegratedImuMeasurement::addMeasurement(const ImuMeasurement& m) {
    if (last_imu_measurement_set_) {
        ...
        const Eigen::Vector3d a =
            last_imu_measurement.linear_acceleration_ - acc_bias_;
        const Eigen::Vector3d w =
            last_imu_measurement.angular_velocity_ - omega_bias_;
        const Quaternion R_incr = Quaternion::exp(w * dt);

        // second order integration:
        delta_t_ij_ +=
            delta_v_ij_ * dt + (delta_R_ij_.rotate(a)) * dt * dt * 0.5;
...
bool ImuHandler::getMeasurementsContainingEdges(
    ...
    extracted_measurements.insert(extracted_measurements.begin(), it,
                                  measurements_.end());
    ...
    if (remove_measurements) {
        // delete measurements that will not be used anymore (such that we keep
        // it+1,
        // the first frame with smaller timestamp (earlier) than
        // frame_timestamp,
...
bool ImuHandler::getMeasurements(const double old_cam_timestamp,
...
bool ImuHandler::getClosestMeasurement(const double timestamp,
    ...
    if (dt_best > imu_calib_.max_imu_delta_t) {
        LOG(WARNING)
            << "ImuHandler: getClosestMeasurement: no measurement found!"
               " closest measurement: "
            << dt_best * 1000.0 << "ms.";
        return false;
    }
...
bool ImuHandler::getRelativeRotationPrior(const double old_cam_timestamp,
                                          const double new_cam_timestamp,
    ...
    for (; it != measurements.rend(); ++it, ++it_plus) {
        ...
        const Eigen::Vector3d omega_corrected =
            it->angular_velocity_ - omega_bias_;
        R_oldimu_newimu =
            R_oldimu_newimu * Quaternion::exp(omega_corrected * dt);
    }
    return true;
}
...
bool ImuHandler::addImuMeasurement(const ImuMeasurement& m) {
    ulock_t lock(measurements_mut_);
    measurements_.push_front(
        m);  // new measurement is at the front of the list!
...
bool ImuHandler::loadImuMeasurementsFromCsvFile(const std::string& filename) {
    ...
    while (vk::readNextLine(7, csv_file_stream, &fields)) {
        ...
        Eigen::Vector3d omega =
            vk::convertStringsToEigenVector3d(fields[1], fields[2], fields[3]);
        Eigen::Vector3d linacc =
            vk::convertStringsToEigenVector3d(fields[4], fields[5], fields[6]);
        const ImuMeasurement m(
            static_cast<double>(stamp_ns) / 1e9, omega,
            linacc);  // TODO(cfo): save uint64_t timestamps!!!
        measurements_.push_front(m);
        if (options_.temporal_stationary_check) {
            temporal_imu_window_.push_front(m);
        }
...
ImuCalibration ImuHandler::loadCalibrationFromFile(
    const std::string& filename) {
    YAML::Node data = YAML::LoadFile(filename);
    ImuCalibration calib;
    if (data["imu_params"].IsDefined()) {
        calib.delay_imu_cam = data["imu_params"]["delay_imu_cam"].as<double>();
        calib.max_imu_delta_t =
            data["imu_params"]["max_imu_delta_t"].as<double>();
        calib.saturation_accel_max = data["imu_params"]["acc_max"].as<double>();
        calib.saturation_omega_max =
            data["imu_params"]["omega_max"].as<double>();
        calib.gyro_noise_density =
            data["imu_params"]["sigma_omega_c"].as<double>();
        calib.acc_noise_density =
            data["imu_params"]["sigma_acc_c"].as<double>();
        calib.gyro_bias_random_walk_sigma =
            data["imu_params"]["sigma_omega_bias_c"].as<double>();
        calib.acc_bias_random_walk_sigma =
            data["imu_params"]["sigma_acc_bias_c"].as<double>();
...
ImuInitialization ImuHandler::loadInitializationFromFile(
    const std::string& filename) {
    YAML::Node data = YAML::LoadFile(filename);
    ...
    if (data["imu_initialization"].IsDefined()) {
        ...
        init.velocity_sigma =
            data["imu_initialization"]["velocity_sigma"].as<double>();
        init.omega_bias_sigma =
            data["imu_initialization"]["omega_bias_sigma"].as<double>();
        init.acc_bias_sigma =
            data["imu_initialization"]["acc_bias_sigma"].as<double>();
    } else {
...
bool ImuHandler::getAngularVelocity(double timestamp,
    ...
    for (ImuMeasurements::const_iterator it = measurements_.begin();
         it != measurements_.end(); ++it) {
    ...
    if (min_delta < imu_calib_.max_imu_delta_t) return true;
    return false;
}

bool ImuHandler::getInitialAttitude(double timestamp,
                                    Quaternion& R_imu_world) const {
    ImuMeasurement m;
    if (!getClosestMeasurement(timestamp, m)) {
        LOG(WARNING)
            << "ImuHandler: Could not get initial attitude. No measurements!";
        return false;
    }
...
IMUTemporalStatus ImuHandler::checkTemporalStatus(const double time_sec) {
    ...
    return stationary ? IMUTemporalStatus::kStationary
                      : IMUTemporalStatus::kMoving;
}
