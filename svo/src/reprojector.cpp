...
Reprojector::Reprojector(const ReprojectorOptions& options, size_t camera_index)
    : options_(options), camera_index_(camera_index) {}

void Reprojector::reprojectFrames(const FramePtr& cur_frame,
    ...
    // Initialize grid
    if (!grid_) {
        grid_.reset(new OccupandyGrid2D(
            options_.cell_size,
    ...
    if (options_.use_kfs_from_global_map && options_.max_n_global_kfs > 0) {
        if (!fixed_landmark_grid_) {
            fixed_landmark_grid_.reset(new OccupandyGrid2D(
        ...
        for (const FramePtr& ref_frame : visible_kfs) {
            for (size_t i = 0; i < ref_frame->num_features_; ++i) {
                ...
                if (point->last_projected_kf_id_.at(camera_index_) ==
                    cur_frame->id_) {
                    continue;
                }
                ...
                Candidate candidate;
                if (reprojector_utils::getCandidate(cur_frame, ref_frame, i,
                                                    candidate)) {
        ...
        for (const auto& c : candidates_) {
            ...
            old_proj_info.emplace_back(std::make_pair(
                pt, SucFailN{pt->n_succeeded_reproj_, pt->n_failed_reproj_}));
        }
        reprojector_utils::sortCandidatesByNumObs(candidates_);
        reprojector_utils::matchCandidates(
            cur_frame, options_.max_fixed_landmarks, options_.affine_est_offset,
            options_.affine_est_gain, candidates_, *fixed_landmark_grid_,
            fixed_lm_stats_, options_.seed_sigma2_thresh);
        VLOG(10) << "Reproject landmarks from global map in cam-"
                 << camera_index_ << ": trials = " << fixed_lm_stats_.n_trials
    ...
    for (const FramePtr& ref_frame : visible_kfs) {
        ...
        for (size_t i = 0; i < ref_frame->num_features_; ++i) {
            ...
            if (ref_frame->landmark_vec_[i] == nullptr ||
                ...
                isMapPoint(ref_frame->type_vec_[i]) || isFixedLandmark(type))
                continue;
            ...
            // ignore for the first 5 frames because in the benchmark node we
            // may have
    ...
    if (doesFrameHaveEnoughFeatures(cur_frame)) {
        // In this case we already have enough features but we just want to set
        // the
        // occupancy such that the depth filter doesn't extract too many new
        // features.
        // TODO(cfo): Actually, this should only be done if a new keyframe is
        // selected.
        reprojector_utils::setGridCellsOccupied(candidates_, *grid_);
        candidates_.clear();
        return;
    ...
    VLOG(10) << "After projection:"
             << " remaining converged seed candidates num "
             << candidates_.size() << std::endl;
    ...
    if (options_.max_unconverged_seeds_ratio > 0) {
        const double min_lm_seeds_ratio =
            1 - options_.max_unconverged_seeds_ratio;
        size_t max_allowed_alternative = static_cast<size_t>(
    ...
    reprojector_utils::sortCandidatesByReprojStats(candidates_);
    reprojector_utils::matchCandidates(
        cur_frame, max_allowed_total, options_.affine_est_offset,
...
void sortCandidatesByReprojStats(Reprojector::Candidates& candidates) {
    std::sort(candidates.begin(), candidates.end(),
              [](const Reprojector::Candidate& lhs,
                 const Reprojector::Candidate& rhs) {
                  if (lhs.type > rhs.type ||
                      (lhs.type == rhs.type && lhs.n_reproj > rhs.n_reproj) ||
                      (lhs.type == rhs.type && lhs.n_reproj == rhs.n_reproj &&
                       lhs.score > rhs.score))  // TODO(check)
                      return true;
                  return false;
              });
}

void sortCandidatesByNumObs(Reprojector::Candidates& candidates) {
    std::sort(candidates.begin(), candidates.end(),
              [](const Reprojector::Candidate& lhs,
                 const Reprojector::Candidate& rhs) {
...
bool matchCandidate(const FramePtr& frame,
    ...
    if (c.ref_frame->landmark_vec_.at(c.ref_index) == nullptr) {
        ...
        if (!point->getCloseViewObs(frame->pos(), ref_frame,
                                    ref_feature_index)) {
            return false;
        }
        FeatureWrapper ref_ftr =
            ref_frame->getFeatureWrapper(ref_feature_index);
        CHECK_NOTNULL(ref_ftr.landmark.get());  // debug
        const FloatType ref_depth =
            (ref_frame->pos() - ref_ftr.landmark->pos()).norm();
        Matcher::MatchResult res = matcher.findMatchDirect(
            *ref_frame, *frame, ref_ftr, ref_depth, c.cur_px);
        if (res != Matcher::MatchResult::kSuccess) {
            point->n_failed_reproj_++;
            return false;  // TODO(cfo): We should return match result and write
                           // in statistics.
        }
    ...
    if (isEdgelet(c.type)) {
        GradientVector g_predicted =
            (matcher.A_cur_ref_ * grad_ref).normalized();
    ...
    // Here we add a reference in the feature and frame to the 3D point, the
    // other way
...
bool getCandidate(const FramePtr& cur_frame,
    ...
    if (ref_frame->landmark_vec_[ref_index] != nullptr) {
        ...
        xyz_world = point->pos();
        n_reproj = point->n_succeeded_reproj_ - point->n_failed_reproj_;
    } else {
        xyz_world =
            ref_frame->T_world_cam() * ref_frame->getSeedPosInFrame(ref_index);
    }
    ...
    candidate = Reprojector::Candidate(
        ...
        ref_frame->landmark_vec_[ref_index]
            ? ref_frame->landmark_vec_[ref_index]->obs_.size()
            : 0u);

    return true;
}

bool projectPointAndCheckVisibility(const FramePtr& frame,
...
void setGridCellsOccupied(const Reprojector::Candidates& candidates,
                          OccupandyGrid2D& grid) {
...
void reprojectMapPoints(const FramePtr& frame,
    ...
    for (const FramePtr& ref_frame : overlap_kfs) {
        // Try to reproject each map point that the other KF observes
        for (size_t i = 0; i < ref_frame->num_features_; ++i) {
            if (ref_frame->type_vec_[i] ==
                FeatureType::kMapPointSeedConverged) {
            ...
            if (ref_frame->type_vec_[i] == FeatureType::kMapPoint ||
                ...
                Reprojector::Candidate candidate;
                if (reprojector_utils::getCandidate(frame, ref_frame, i,
                                                    candidate)) {
    ...
    VLOG(5) << "Map points count: " << std::endl
            ...
            << " - converged map point seed: " << n_converge_map_point_seed
            << std::endl
    ...
    reprojector_utils::matchCandidates(
        ...
        options.affine_est_offset, options.affine_est_gain, candidates, *grid,
        stats);
