...
bool AbstractInitialization::trackFeaturesAndCheckDisparity(
    ...
    const double avg_disparity =
        std::accumulate(disparity.begin(), disparity.end(), 0.0) /
        disparity.size();
    VLOG(3) << "Init: Tracked " << num_tracked_tot
    ...
    if (avg_disparity < options_.init_min_disparity) return false;
    return true;
}

InitResult HomographyInit::addFrameBundle(const FrameBundlePtr& frames_cur) {
    ...
    feature_tracking_utils::getFeatureMatches(frame_cur, frame_ref,
                                              &matches_cur_ref);
    ...
    if (H_cur_ref.score < options_.init_min_inliers) {
        SVO_WARN_STREAM("Init Homography: Have "
                        << H_cur_ref.score << "inliers. "
                        << options_.init_min_inliers
    ...
    if (initialization_utils::triangulateAndInitializePoints(
            frames_cur->at(0), frames_ref_->at(0), T_cur_from_ref_,
            options_.reproj_error_thresh, depth_at_current_frame_,
...
class TranslationSacProblemWithTriangulation
    : public opengv::sac_problems::relative_pose::TranslationOnlySacProblem {
 public:
    typedef opengv::sac_problems::relative_pose::TranslationOnlySacProblem Base;
    TranslationSacProblemWithTriangulation(adapter_t& adapter)
        : Base(adapter) {}
...
InitResult TwoPointInit::addFrameBundle(const FrameBundlePtr& frames_cur) {
#ifdef SVO_USE_OPENGV
    ...
    initialization_utils::copyBearingVectors(*frames_cur->at(0),
                                             *frames_ref->at(0),
                                             matches_cur_ref, &f_cur, &f_ref);

    // Compute model
    static double inlier_threshold =
        1.0 - std::cos(frames_cur->at(0)->getAngleError(
                  options_.reproj_error_thresh));
    const Eigen::Matrix3d R_cur_ref =
        (R_cur_world_ * R_ref_world_.inverse()).getRotationMatrix();
    ...
    T_cur_from_ref_ =
        Transformation(Quaternion(R), ransac.model_coefficients_.rightCols(1));
    ...
    VLOG(5) << "2Pt RANSAC:" << std::endl
            ...
            << "H.rotation_matrix() = " << T_cur_from_ref_.getRotationMatrix()
            << std::endl
            << "H.translation() = " << T_cur_from_ref_.getPosition();

    // Triangulate
    if (initialization_utils::triangulateAndInitializePoints(
            frames_cur->at(0), frames_ref->at(0), T_cur_from_ref_,
            options_.reproj_error_thresh, depth_at_current_frame_,
            options_.init_min_inliers, matches_cur_ref)) {
        return InitResult::kSuccess;
    }
...
#else
    SVO_ERROR_STREAM(
        "You need to compile SVO with OpenGV to use TwoPointInit!");
    return InitResult::kFailure;
#endif
}

InitResult FivePointInit::addFrameBundle(const FrameBundlePtr& frames_cur) {
#ifdef SVO_USE_OPENGV
    // Track and detect features.
    if (!trackFeaturesAndCheckDisparity(frames_cur))
        return InitResult::kTracking;
    ...
    // enough inliers?
    if (ransac.inliers_.size() < options_.init_min_inliers) {
        VLOG(3) << "5Pt RANSAC has only " << ransac.inliers_.size()
                << " inliers. " << options_.init_min_inliers << " required.";
        return InitResult::kNoKeyframe;
    }
    ...
    VLOG(5) << "5Pt RANSAC:" << std::endl
            ...
            << "# Inliers = " << ransac.inliers_.size() << std::endl
            << "Model = " << ransac.model_coefficients_ << std::endl
            << "T.rotation_matrix() = " << T_cur_from_ref_.getRotationMatrix()
            << std::endl
...
InitResult OneShotInit::addFrameBundle(const FrameBundlePtr& frames_cur) {
    CHECK(frames_cur->size() == 1)
        << "OneShot Initialization doesn't work with Camera Array";
    ...
    for (const std::pair<size_t, size_t> it : matches_cur_ref) {
        ...
        const Vector3d xyz_in_cam =
            (f_ref / f_ref.z()) * depth_at_current_frame_;
    ...
    PoseOptimizer::SolverOptions options =
    ...
    for (size_t i = 0; i < frame_cur->numFeatures(); ++i) {
        if (!frame_cur->landmark_vec_.at(i)) continue;
    ...
    if (frames_cur->numLandmarks() < options_.init_min_features) {
        std::cout << "tracking " << frames_ref->numLandmarks() << " features"
                  << std::endl;
        return InitResult::kFailure;
    }
    return InitResult::kSuccess;
...
StereoInit::StereoInit(const InitializationOptions& init_options,
    ...
    : AbstractInitialization(
          init_options, tracker_options, detector_options, cams) {
    StereoTriangulationOptions stereo_options;
    detector_ = feature_detection_utils::makeDetector(detector_options,
                                                      cams->getCameraShared(0));
...
InitResult ArrayInitGeometric::addFrameBundle(
    ...
    // Create Problem
    auto algorithm =
    opengv::sac_problems::relative_pose::NoncentralRelativePoseSacProblem::SIXPT;
    boost::shared_ptr<Problem> problem_ptr(new Problem(adapter, algorithm));

    // Run RANSAC:
    static double inlier_threshold = 1.0 -
    std::cos(frames_cur->at(0)->getAngleError(options_.reproj_error_thresh));
    ransac.sac_model_ = problem_ptr;
    ransac.threshold_ = inlier_threshold;
    ...
    for(size_t cam_id=0; cam_id<frames_cur->size(); ++cam_id)
    {
      ...
      //      frame_cur, frame_ref, T_cur_ref,  options_.reproj_error_thresh,
    points_in_cur);
    ...
    if(options_.verbose)
      std::cout << "Total triangulated points =  " << frames_cur->nFts() <<
    std::endl;

    if(frames_cur->nFts() < options_.init_min_inliers)
    {
      SVO_WARN_STREAM("Init WARNING: "<<options_.init_min_inliers<<" inliers
    minimum required.");
      return InitResult::kFailure;
    }
    ...
    {
      initialization_utils::displayFeatureTracks(frames_cur->at(i),
    frames_ref_->at(i));
...
InitResult ArrayInitOptimization::addFrameBundle(
    ...
    InitResult res = trackBundleFeatures(frames_cur);
    if (res != InitResult::kSuccess) return res;
    ...
    gtsam::noiseModel::Isotropic::shared_ptr pixel_noise_base =
        gtsam::noiseModel::Isotropic::Sigma(
            2, 1.0 / frames_cur->at(0)->getErrorMultiplier());

    gtsam::SharedNoiseModel pixel_noise = gtsam::noiseModel::Robust::Create(
        // gtsam::noiseModel::mEstimator::Tukey::Create(4.685),
        // pixel_noise_base);
        // gtsam::noiseModel::mEstimator::Fair::Create(1.3998),
        // pixel_noise_base);
        // gtsam::noiseModel::mEstimator::Cauchy::Create(0.1),
        // pixel_noise_base);
        // gtsam::noiseModel::mEstimator::Welsh::Create(2.9846),
        // pixel_noise_base);
    ...
    gtsam::Pose3 pose_ref(gtsam::Rot3(T_world_refimu.getRotationMatrix()),
                          gtsam::Point3(T_world_refimu.getPosition()));
    graph.push_back(gtsam::PriorFactor<gtsam::Pose3>(
        gtsam::Symbol('x', 0), pose_ref,
        pose_prior_noise));  // add directly to graph
    ...
    gtsam::noiseModel::Gaussian::shared_ptr T_prior_noise =
        gtsam::noiseModel::Diagonal::Information(
            relpose_prior_info);  // translation

    // Add prior on relative pose between first two frames
    graph.push_back(gtsam::BetweenFactor<gtsam::Pose3>(
        gtsam::Symbol('x', 0), gtsam::Symbol('x', 1), T_prior, T_prior_noise));
    ...
    for(const FramePtr& frame : frames_ref_->frames_)
    {
      for(const FeaturePtr& ftr : frame->fts_)
      {
        ...
        initial_estimate.insert(gtsam::Symbol('l', point_id++),
    gtsam::Point3(xyz));
      }
    }
    ...
    gtsam::Cal3_S2> ProjectionFactor;
    for(const FramePtr& frame : frames_ref_->frames_)
    {
      ...
      for(const FeaturePtr& ftr : frame->fts_)
      {
        graph.push_back(
              ProjectionFactor(
                gtsam::Point2(vk::project2(ftr->f)), pixel_noise,
    gtsam::Symbol('x', 0),
    ...
    SVO_ASSERT(point_id_cur == point_id_ref,
               "Initialization: Cur and Ref Bundles don't have same amount of
    features.");
    SVO_ASSERT(point_id_cur == point_id,
               "Initialization: Cur Bundle and Values don't have same amount of
    features.");

    // solve
    gtsam::LevenbergMarquardtParams params;
    params.verbosity = gtsam::LevenbergMarquardtParams::ERROR;
    gtsam::Values result = gtsam::LevenbergMarquardtOptimizer(graph,
    initial_estimate, params).optimize();
    ...
    T_world_refimu = Transformation(T_ref.rotation().toQuaternion(),
    T_ref.translation().vector());
    Transformation T_world_curimu(T_cur.rotation().toQuaternion(),
    T_cur.translation().vector());
...
#else
    SVO_ERROR_STREAM(
        "You need to compile SVO with GTSAM to use ArrayInitOptimization!");
    return InitResult::kFailure;
#endif
}
...
bool triangulateAndInitializePoints(
    ...
    initialization_utils::triangulatePoints(*frame_cur, *frame_ref, T_cur_ref,
                                            reprojection_threshold,
                                            matches_cur_ref, points_in_cur);

    if (matches_cur_ref.size() < min_inliers_threshold) {
        LOG(WARNING) << "Init WARNING: " << min_inliers_threshold
                     << " inliers minimum required. "
    ...
    initialization_utils::rescaleAndInitializePoints(
        frame_cur, frame_ref, matches_cur_ref, points_in_cur, T_cur_ref,
        depth_at_current_frame);

    return true;
}

void triangulatePoints(const Frame& frame_cur,
    ...
    for (size_t i = 0; i < matches_cur_ref.size(); ++i) {
        ...
        const Position xyz_in_cur =
            vk::triangulateFeatureNonLin(T_cur_ref.getRotationMatrix(),
                                         T_cur_ref.getPosition(), f_cur, f_ref);
...
void rescaleAndInitializePoints(
    ...
    frame_cur->T_f_w_.getPosition() = -frame_cur->T_f_w_.getRotation().rotate(
    ...
    for (size_t i = 0; i < matches_cur_ref.size(); ++i) {
        const Vector3d xyz_in_world =
            T_world_cur * (points_in_cur.col(i) * scale);
        const int point_id_cur =
            frame_cur->track_id_vec_(matches_cur_ref[i].first);
        const int point_id_ref =
            frame_ref->track_id_vec_(matches_cur_ref[i].second);
    ...
    SVO_INFO_STREAM("Init: Triangulated " << matches_cur_ref.size()
                                          << " points");
}

void displayFeatureTracks(const FramePtr& frame_cur,
                          const FramePtr& frame_ref) {
...
AbstractInitialization::UniquePtr makeInitializer(
    ...
    switch (init_options.init_type) {
        case InitializerType::kHomography:
            initializer.reset(new HomographyInit(init_options, tracker_options,
                                                 detector_options, cams));
            break;
        case InitializerType::kTwoPoint:
            initializer.reset(new TwoPointInit(init_options, tracker_options,
                                               detector_options, cams));
            break;
        case InitializerType::kFivePoint:
            initializer.reset(new FivePointInit(init_options, tracker_options,
                                                detector_options, cams));
            break;
        case InitializerType::kOneShot:
            initializer.reset(new OneShotInit(init_options, tracker_options,
                                              detector_options, cams));
            break;
        case InitializerType::kStereo:
            initializer.reset(new StereoInit(init_options, tracker_options,
                                             detector_options, cams));
            break;
        case InitializerType::kArrayGeometric:
            initializer.reset(new ArrayInitGeometric(
                init_options, tracker_options, detector_options, cams));
            break;
        case InitializerType::kArrayOptimization:
            initializer.reset(new ArrayInitOptimization(
                init_options, tracker_options, detector_options, cams));
            break;
        default:
