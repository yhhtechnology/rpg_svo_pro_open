...
void FrameHandlerMono::addImage(const cv::Mat& img, const uint64_t timestamp) {
...
UpdateResult FrameHandlerMono::processFirstFrame() {
    ...
    if (res == InitResult::kTracking) return UpdateResult::kDefault;
    ...
    if (bundle_adjustment_type_ == BundleAdjustmentType::kCeres) {
        map_->addKeyframe(
            initializer_->frames_ref_->at(0),
            bundle_adjustment_type_ == BundleAdjustmentType::kCeres);
    } else if (bundle_adjustment_type_ == BundleAdjustmentType::kGtsam) {
        CHECK(bundle_adjustment_) << "bundle_adjustment_type_ is kGtsam but "
                                     "bundle_adjustment_ is NULL";
    ...
    frame_utils::getSceneDepth(newFrame(), depth_median_, depth_min_,
                               depth_max_);
    VLOG(40) << "Current Frame Depth: "
             << "min: " << depth_min_ << ", max: " << depth_max_
             << ", median: " << depth_median_;
    depth_filter_->addKeyframe(newFrame(), depth_median_, 0.5 * depth_min_,
                               depth_median_ * 1.5);
...
UpdateResult FrameHandlerMono::processFrame() {
    ...
    if (depth_filter_->options_.extra_map_points) {
        ...
        OccupandyGrid2D map_point_grid(
            depth_filter_->sec_feature_detector_->grid_);
        reprojector_utils::reprojectMapPoints(newFrame(), overlap_kfs_.at(0),
                                              reprojectors_.at(0)->options_,
                                              &map_point_grid);
    ...
    if (options_.update_seeds_with_old_keyframes) {
        ...
        depth_filter_->updateSeeds({newFrame()}, lastFrameUnsafe());
        VLOG(40)
            << "Updating seeds in current frame using overlapping keyframes...";
        ...
        while (map_->size() > options_.max_n_kfs) {
            if (bundle_adjustment_type_ == BundleAdjustmentType::kCeres) {
                ...
                FramePtr furthest_frame =
                    map_->getFurthestKeyframe(newFrame()->pos());
...
UpdateResult FrameHandlerMono::relocalizeFrame(
    const Transformation& /*T_cur_ref*/, const FramePtr& ref_keyframe) {
    ++relocalization_n_trials_;
    if (ref_keyframe == nullptr) return UpdateResult::kFailure;

    VLOG_EVERY_N(1, 20) << "Relocalizing frame";
    FrameBundle::Ptr ref_frame(
        new FrameBundle({ref_keyframe}, ref_keyframe->bundleId()));
