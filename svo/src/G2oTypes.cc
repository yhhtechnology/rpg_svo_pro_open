...
ImuCamPose::ImuCamPose(KeyFrame* pKF) : its(0) {
    ...
    Rcb[0] =
        Converter::toMatrix3d(pKF->mImuCalib.Tcb.rowRange(0, 3).colRange(0,
        3));
...
ImuCamPose::ImuCamPose(Frame* pF) : its(0) {
    ...
    Rcb[0] =
        Converter::toMatrix3d(pF->mImuCalib.Tcb.rowRange(0, 3).colRange(0,
        3));
    Rbc[0] = Rcb[0].transpose();
...
ImuCamPose::ImuCamPose(Eigen::Matrix3d& _Rwc,
                       Eigen::Vector3d& _twc,
                       KeyFrame* pKF)
    : its(0) {
...
bool ImuCamPose::isDepthPositive(const Eigen::Vector3d& Xw, int cam_idx)
const {
...
InvDepthPoint::InvDepthPoint(double _rho,
                             double _u,
                             double _v,
                             KeyFrame* pHostKF)
    : u(_u),
...
void EdgeMono::linearizeOplus() {
    ...
    _jacobianOplusXj = proj_jac * Rcb * SE3deriv;  // TODO optimize this
    product
}
...
void EdgeMonoOnlyPose::linearizeOplus() {
    ...
    SE3deriv << 0.0, z, -y, 1.0, 0.0, 0.0, -z, 0.0, x, 0.0, 1.0, 0.0, y, -x,
        0.0, 0.0, 0.0, 1.0;
    _jacobianOplusXi =
        proj_jac * Rcb * SE3deriv;  // symbol different becasue of update
        mode
}
