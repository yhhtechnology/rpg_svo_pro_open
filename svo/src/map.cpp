...
void Map::safeDeletePoint(const PointPtr& pt) {
    ...
    for (const KeypointIdentifier& obs : pt->obs_) {
        if (const FramePtr& frame = obs.frame.lock()) {
            ...
            SVO_ERROR_STREAM(
                "could not lock weak_ptr<Frame> in Map::safeDeletePoint");
    }
    pt->obs_.clear();
}
...
void Map::emptyPointsTrash() {
    std::lock_guard<std::mutex> lock(points_to_delete_mutex_);
    SVO_DEBUG_STREAM("Deleting " << points_to_delete_.size()
                                 << " point from trash");
...
void Map::getOverlapKeyframes(
    ...
    for (const auto& kf : keyframes_) {
        // check first if Point is visible in the Keyframe, use therefore
        // KeyPoints
            ...
            if (frame->isVisible(keypoint.second)) {
                close_kfs->push_back(
                    std::make_pair(kf.second,
                                   (frame->T_f_w_.getPosition() -
                                    kf.second->T_f_w_.getPosition())
                                       .norm()));
                break;  // this keyframe has an overlapping field of view -> add
                        // to close_kfs
            }
        }
    }
...
void Map::getClosestNKeyframesWithOverlap(
    ...
    std::nth_element(overlap_kfs.begin(), overlap_kfs.begin() + N,
                     overlap_kfs.end(),
    ...
    std::transform(
        overlap_kfs.begin(), overlap_kfs.end(), std::back_inserter(*close_kfs),
...
void Map::transform(const Matrix3d& R, const Vector3d& t, const double& s) {
    for (const auto& kf : keyframes_) {
        ...
        Matrix3d rot =
            R * kf.second->T_f_w_.getRotation().inverse().getRotationMatrix();
...
void Map::checkDataConsistency() const {
    for (const auto& kf : keyframes_) {
        ...
        CHECK_EQ(static_cast<size_t>(frame->px_vec_.cols()),
                 frame->type_vec_.size());
        CHECK_EQ(static_cast<size_t>(frame->px_vec_.cols()),
                 frame->landmark_vec_.size());
        CHECK_EQ(static_cast<size_t>(frame->px_vec_.cols()),
        ...
        for (size_t i = 0; i < frame->num_features_; ++i) {
            if (frame->landmark_vec_[i]) {
                ...
                size_t ref_count = 0;
                for (const KeypointIdentifier& o :
            ...
            CHECK((isSeed(frame->type_vec_[i]) &&
                   frame->landmark_vec_[i] == nullptr) ||
