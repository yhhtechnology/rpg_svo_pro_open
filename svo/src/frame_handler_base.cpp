...
inline double distanceFirstTwoKeyframes(svo::Map& map) {
    ...
    double dist = (first_kf->T_world_imu().getPosition() -
...
const std::unordered_map<svo::Stage, std::string, EnumClassHash> kStageName{
    {Stage::kPaused, "Paused"},
    {Stage::kInitializing, "Initializing"},
    {Stage::kTracking, "Tracking"},
    {Stage::kRelocalization, "Reloc"}};
...
const std::unordered_map<svo::UpdateResult, std::string, EnumClassHash>
    kUpdateResultName{{UpdateResult::kDefault, "Default"},
                      {UpdateResult::kKeyframe, "KF"},
...
FrameHandlerBase::FrameHandlerBase(
    ...
    need_new_kf_ =
        std::bind(&FrameHandlerBase::needNewKf, this, std::placeholders::_1);
    ...
    img_align_options.estimate_illumination_gain =
        options_.img_align_est_illumination_gain;
    img_align_options.estimate_illumination_offset =
        options_.img_align_est_illumination_offset;

    sparse_img_align_.reset(new SparseImgAlign(
        SparseImgAlign::getDefaultSolverOptions(), img_align_options));
    pose_optimizer_.reset(
        new PoseOptimizer(PoseOptimizer::getDefaultSolverOptions()));
    if (options_.poseoptim_using_unit_sphere)
        pose_optimizer_->setErrorType(
            PoseOptimizer::ErrorType::kBearingVectorDiff);
...
bool FrameHandlerBase::addImageBundle(const std::vector<cv::Mat>& imgs,
    ...
    if (last_frames_) {
        ...
        if (last_frames_->getMinTimestampNanoseconds() >=
...
bool FrameHandlerBase::addFrameBundle(const FrameBundlePtr& frame_bundle) {
    ...
    // computed a new map. in this case, we replace our map with the latest
    // estimate
    ...
    if (have_motion_prior_) {
        ...
        if (!backend_scale_initialized_) {
            if (backend_scale_stable) {
                ...
                // the points in the last frame were updated, hence we also
                // update the
                // seeds with the new depth values
                frame_utils::getSceneDepth(last_frames_->frames_[0],
                                           depth_median_, depth_min_,
                                           depth_max_);
        ...
        if (last_frames_) {
            ...
            for (size_t i = 0; i < new_frames_->size(); ++i) {
                new_frames_->at(i)->T_f_w_ = new_frames_->at(i)->T_cam_imu() *
                                             T_newimu_lastimu_prior_ *
    ...
    if (bundle_adjustment_) {
#ifdef SVO_LOOP_CLOSING
        // if we have loop closing module, see whether there is any correction
        // we can do
        if (lc_) {
    ...
    if (update_res_ == UpdateResult::kKeyframe) {
        ...
        if (lc_ && last_rm_kf) {
            ...
            // update the keyframe information and trigger pose graph
    ...
    num_obs_last_ =
        new_frames_->numTrackedFeatures() + new_frames_->numFixedLandmarks();
    if (stage_ == Stage::kTracking) {
        if (isInRecovery()) {
            CHECK_GT(new_frames_->getMinTimestampSeconds(),
                     last_good_tracking_time_sec_);
            if ((new_frames_->getMinTimestampSeconds() -
                 last_good_tracking_time_sec_) <
                options_.global_map_lc_timeout_sec_) {
                setRecovery(false);
                last_good_tracking_time_sec_ =
                    new_frames_->getMinTimestampSeconds();
            }
        } else {
            last_good_tracking_time_sec_ =
                new_frames_->getMinTimestampSeconds();
...
void FrameHandlerBase::setRotationIncrementPrior(
    const Quaternion& R_lastimu_newimu) {
...
//------------------------------------------------------------------------------
void FrameHandlerBase::setInitialPose(
    const FrameBundlePtr& frame_bundle) const {
    if (have_rotation_prior_) {
        ...
        for (size_t i = 0; i < frame_bundle->size(); ++i) {
            frame_bundle->at(i)->T_f_w_ =
                cams_->get_T_C_B(i) * T_world_imuinit.inverse();
        }
    }
}
...
size_t FrameHandlerBase::sparseImageAlignment() {
    // optimize the pose of the new frame such that it matches the pose of the
    // previous frame best
    // this will improve the relative transformation between the previous and
    // the new frame
    ...
    VLOG(40) << "Sparse image alignment tracked " << img_align_n_tracked
             << " features.";
...
size_t FrameHandlerBase::projectMapInFrame() {
    ...
    for (size_t camera_idx = 0; camera_idx < cams_->numCameras();
         ++camera_idx) {
        ...
        map_->getClosestNKeyframesWithOverlap(
            new_frames_->at(camera_idx), cur_reprojector->options_.max_n_kfs,
    ...
    if (options_.use_async_reprojectors && cams_->numCameras() > 1) {
        ...
        for (size_t camera_idx = 0; camera_idx < cams_->numCameras();
            ...
            auto func = std::bind(
                ...
                reprojectors_.at(camera_idx).get(), new_frames_->at(camera_idx),
                overlap_kfs_.at(camera_idx), trash_points.at(camera_idx));
        ...
        for (size_t camera_idx = 0; camera_idx < cams_->numCameras();
             ++camera_idx) {
            reprojectors_.at(camera_idx)
                ->reprojectFrames(new_frames_->at(camera_idx),
                                  overlap_kfs_.at(camera_idx),
    ...
    for (const ReprojectorPtr& reprojector : reprojectors_) {
        ...
        cumul_stats_global_map.n_matches +=
            reprojector->fixed_lm_stats_.n_matches;
        cumul_stats_global_map.n_trials +=
            reprojector->fixed_lm_stats_.n_trials;
    }
    ...
    size_t n_total_ftrs =
        cumul_stats_.n_matches + (cumul_stats_global_map.n_matches <= 10
                                      ? 0
                                      : cumul_stats_global_map.n_matches);
    ...
    if (n_total_ftrs < options_.quality_min_fts) {
        SVO_WARN_STREAM_THROTTLE(
            1.0,
            "Not enough matched features: " + std::to_string(n_total_ftrs));
    }
...
size_t FrameHandlerBase::optimizePose() {
    // pose optimization
    // optimize the pose of the frame in such a way, that the projection of all
    // feature world coordinates
    // is not far off the position of the feature points within the frame. The
    // optimization is done for all points
    ...
    size_t sfba_n_edges_final =
        pose_optimizer_->run(new_frames_, options_.poseoptim_thresh);
...
//------------------------------------------------------------------------------
void FrameHandlerBase::optimizeStructure(const FrameBundle::Ptr& frames,
                                         int max_n_pts,
                                         int max_iter) {
    VLOG(40) << "Optimize structure.";
    // some feature points will be optimized w.r.t keyframes they were observed
    // in the way that their projection error into all other keyframes is
    // minimzed

    if (max_n_pts == 0)
        return;  // don't return if max_n_pts == -1, this means we optimize ALL
                 // points
        ...
        if (max_n_pts > 0) {
            ...
            std::nth_element(pts.begin(), pts.begin() + max_n_pts, pts.end(),
                             [](const PointPtr& lhs, const PointPtr& rhs) {
                                 // we favour points that have not been
                                 // optimized in a while
                                 return (lhs->last_structure_optim_ <
                                         rhs->last_structure_optim_);
                             });
    ...
    for (size_t i = 0; i < frame->num_features_; ++i) {
        if (frame->landmark_vec_[i]) {
            ...
            if (isCorner(ref.keyframe->type_vec_[ref.seed_id])) {
                ...
                double angle =
                    feature_detection_utils::getAngleAtPixelUsingHistogram(
                        frame->img_pyr_[frame->level_vec_[i]],
                        (frame->px_vec_.col(i) / (1 << frame->level_vec_[i]))
                            .cast<int>(),
                        4u);
    ...
    if (options_.kfselect_criterion == KeyframeCriterion::DOWNLOOKING) {
        for (const auto& frame : visible_kfs) {
            ...
            if (fabs(relpos.x()) / depth_median_ < options_.kfselect_min_dist &&
                fabs(relpos.y()) / depth_median_ <
                    options_.kfselect_min_dist * 0.8 &&
                fabs(relpos.z()) / depth_median_ <
                    options_.kfselect_min_dist * 1.3)
                return false;
        }
    ...
    if (options_.kfselect_min_disparity > 0) {
        ...
        for (size_t i = 0; i < frame->num_features_; ++i) {
            if (frame->landmark_vec_[i]) {
                ...
                for (auto it = observations.rbegin(); it != observations.rend();
                     ++it) {
                    if (it->frame_id == kf_id) {
                        if (FramePtr kf = it->frame.lock()) {
                            disparities.push_back(
                                (frame->px_vec_.col(i) -
                                 kf->px_vec_.col(it->keypoint_index_))
                                    .norm());
                        }
                        break;
                    }
        ...
        if (!disparities.empty()) {
            ...
            if (disparity < options_.kfselect_min_disparity) {
                VLOG(40)
                    << "KF Select: NO NEW KEYFRAME disparity not large enough";
                return false;
            }
        }
    }
...
void FrameHandlerBase::getMotionPrior(const bool /*use_velocity_in_frame*/) {
    if (have_rotation_prior_) {
        ...
        T_newimu_lastimu_prior_ =
            Transformation(R_imulast_world_ * R_imu_world_.inverse(),
                           t_lastimu_newimu_)
                .inverse();
        have_motion_prior_ = true;
...
void FrameHandlerBase::setFirstFrames(
    const std::vector<FramePtr>& first_frames) {
    resetAll();
    last_frames_.reset(new FrameBundle(first_frames));
    for (auto f : last_frames_->frames_) {
        f->setKeyframe();
        map_->addKeyframe(
            f, bundle_adjustment_type_ == BundleAdjustmentType::kCeres);
    }
    stage_ = Stage::kTracking;
}

void FrameHandlerBase::setBundleAdjuster(
    const AbstractBundleAdjustmentPtr& ba) {
