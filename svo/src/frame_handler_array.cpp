...
UpdateResult FrameHandlerArray::processSecondFrame() {
    ...
    for (const FramePtr& frame : initializer_->frames_ref_->frames_) {
        frame->setKeyframe();
        map_->addKeyframe(
            frame, bundle_adjustment_type_ == BundleAdjustmentType::kCeres);
    }

    // make new frame keyframe
    for (const FramePtr& frame : new_frames_->frames_) {
        frame->setKeyframe();
        frame_utils::getSceneDepth(frame, depth_median_, depth_min_,
                                   depth_max_);
...
UpdateResult FrameHandlerArray::processFrame() {
    ...
    for (size_t i = 0; i < new_frames_->size(); ++i) makeKeyframe(i);
...
UpdateResult FrameHandlerArray::makeKeyframe(const size_t camera_id) {
    ...
    if (!frame_utils::getSceneDepth(frame, depth_median, depth_min,
                                    depth_max)) {
        depth_min = 0.2;
        depth_median = 3.0;
        depth_max = 100;
