...
FrameHandlerStereo::FrameHandlerStereo(
    ...
    : FrameHandlerBase(base_options,
                       ...
                       stereo_camera) {
    // init initializer
    stereo_triangulation_.reset(new StereoTriangulation(
        stereo_options,
        feature_detection_utils::makeDetector(feature_detector_options,
                                              cams_->getCameraShared(0))));
}
...
void FrameHandlerStereo::addImages(const cv::Mat& img_left,
...
UpdateResult FrameHandlerStereo::processFirstFrame() {
    if (initializer_->addFrameBundle(new_frames_) == InitResult::kFailure) {
        SVO_ERROR_STREAM(
            "Initialization failed. Not enough triangulated points.");
    ...
    frame_utils::getSceneDepth(new_frames_->at(0), depth_median_, depth_min_,
                               depth_max_);
    depth_filter_->addKeyframe(new_frames_->at(0), depth_median_,
