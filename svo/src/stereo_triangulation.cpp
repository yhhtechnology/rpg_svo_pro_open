...
void StereoTriangulation::compute(const FramePtr& frame0,
    ...
    if (frame0->numLandmarks() >= options_.triangulate_n_features) {
        VLOG(5)
            << "Calling stereo triangulation with sufficient number of features"
            << " has no effect.";
        return;
    }
    ...
    feature_detector_->detect(frame0->img_pyr_, frame0->getMask(),
    ...
    Bearings new_f;
    frame_utils::computeNormalizedBearingVectors(new_px, *frame0->cam(),
                                                 &new_f);

    // Add features to first frame.
    const long n_old = static_cast<long>(frame0->numFeatures());
    const long n_new = new_px.cols();
    frame0->resizeFeatureStorage(frame0->num_features_ +
    ...
    frame0->type_vec_.insert(frame0->type_vec_.begin() + n_old,
                             new_types.cbegin(), new_types.cend());
    ...
    for (const size_t& i_ref : indices) {
        matcher.options_.align_1d =
            isEdgelet(frame0->type_vec_[i_ref]);  // TODO(cfo): check effect
        FloatType depth = 0.0;
