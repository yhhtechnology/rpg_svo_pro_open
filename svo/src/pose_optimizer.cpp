...
PoseOptimizer::PoseOptimizer(SolverOptions solver_options)
    : vk::solver::MiniLeastSquaresSolver<6, Transformation, PoseOptimizer>(
          solver_options) {}
...
void PoseOptimizer::setRotationPrior(const Quaternion& R_frame_world,
                                     double lambda) {
...
size_t PoseOptimizer::run(const FrameBundle::Ptr& frame_bundle,
    ...
    CHECK_GT(frame_bundle->numFeatures(), 0u)
        << "PoseOptimizer: No features in frames";
    ...
    for (const FramePtr& f : frame_bundle->frames_) {
        removeOutliers(reproj_thresh_px, f.get(), &final_errors,
                       &n_deleted_edges, &n_deleted_corners);
    }

    VLOG(5) << "PoseOptimzer: drop " << n_deleted_corners
            << " corner outliers and " << n_deleted_edges
            << " edgelet outliers out of " << n_meas_ << " measurements.";

    // TODO(zzc): for bearing vector difference, can we also save errors in
    // pixels?
    // save statistics
    double error_scale =
        (err_type_ == ErrorType::kUnitPlane) ? focal_length_ : 1.0;
    stats_.reproj_error_before =
        vk::getMedian<float>(start_errors) * error_scale;
    stats_.reproj_error_after =
        vk::getMedian<double>(final_errors) * error_scale;
...
double PoseOptimizer::evaluateError(const Transformation& T_imu_world,
                                    HessianMatrix* H,
                                    GradientVector* g) {
    return evaluateErrorImpl(T_imu_world, H, g, nullptr);
}

double PoseOptimizer::evaluateErrorImpl(const Transformation& T_imu_world,
                                        HessianMatrix* H,
                                        GradientVector* g,
            ...
            if (isEdgelet(frame->type_vec_[i])) {
                ...
                if (err_type_ == ErrorType::kUnitPlane)
                    pose_optimizer_utils::calculateEdgeletResidualUnitPlane(
                        frame->f_vec_.col(i), xyz_world,
                        frame->grad_vec_.col(i), T_imu_world, T_cam_imu,
                        measurement_sigma, robust_weight_, &unwhitened_error,
                        &chi2_error, H, g);
                else if (err_type_ == ErrorType::kImagePlane)
                    pose_optimizer_utils::calculateEdgeletResidualImagePlane(
                        frame->px_vec_.col(i), xyz_world,
                        frame->grad_vec_.col(i), T_imu_world, T_cam_imu,
                        *frame->cam(), measurement_sigma, robust_weight_,
                        &unwhitened_error, &chi2_error, H, g);
                else if (err_type_ == ErrorType::kBearingVectorDiff)
                    pose_optimizer_utils::
                        calculateEdgeletResidualBearingVectorDiff(
                            frame->px_vec_.col(i), frame->f_vec_.col(i),
                            xyz_world, frame->grad_vec_.col(i), T_imu_world,
                            T_cam_imu, *frame->cam(), measurement_sigma,
                            robust_weight_, &unwhitened_error, &chi2_error, H,
                            g);
            } else {
                if (err_type_ == ErrorType::kUnitPlane)
                    ...
                    pose_optimizer_utils::calculateFeatureResidualImagePlane(
                        frame->px_vec_.col(i), xyz_world, T_imu_world,
                        T_cam_imu, *frame->cam(), measurement_sigma,
                        robust_weight_, &unwhitened_error, &chi2_error, H, g);
                } else if (err_type_ == ErrorType::kBearingVectorDiff) {
                    pose_optimizer_utils::
                        calculateFeatureResidualBearingVectorDiff(
                            frame->f_vec_.col(i), xyz_world, T_imu_world,
                            T_cam_imu, measurement_sigma, robust_weight_,
                            &unwhitened_error, &chi2_error, H, g);
                }
            }
            if (unwhitened_errors) {
...
void PoseOptimizer::removeOutliers(const double reproj_err_threshold,
                                   Frame* frame,
    ...
    static double threshold_bearing_diff = std::fabs(
        2 * std::sin(0.5 * frame->getAngleError(reproj_err_threshold)));
        ...
        if (isEdgelet(frame->type_vec_[i])) {
            if (err_type_ == ErrorType::kUnitPlane) {
                pose_optimizer_utils::calculateEdgeletResidualUnitPlane(
                    ...
                    T_imu_world, T_cam_imu, 0.0, robust_weight_,
                    &unwhitened_error, &chi2_error, nullptr, nullptr);
            } else if (err_type_ == ErrorType::kBearingVectorDiff) {
                pose_optimizer_utils::calculateEdgeletResidualBearingVectorDiff(
                    frame->px_vec_.col(i), frame->f_vec_.col(i), xyz_world,
                    frame->grad_vec_.col(i), T_imu_world, T_cam_imu,
                    *frame->cam(), 0.0, robust_weight_, &unwhitened_error,
                    &chi2_error, nullptr, nullptr);
            } else if (err_type_ == ErrorType::kImagePlane) {
                pose_optimizer_utils::calculateEdgeletResidualImagePlane(
                    frame->px_vec_.col(i), xyz_world, frame->grad_vec_.col(i),
                    T_imu_world, T_cam_imu, *frame->cam(), 0.0, robust_weight_,
                    &unwhitened_error, &chi2_error, nullptr, nullptr);
            }
        } else {
            if (err_type_ == ErrorType::kUnitPlane) {
                pose_optimizer_utils::calculateFeatureResidualUnitPlane(
                    frame->f_vec_.col(i), xyz_world, T_imu_world, T_cam_imu,
                    0.0, robust_weight_, &unwhitened_error, &chi2_error,
                    nullptr, nullptr);
            } else if (err_type_ == ErrorType::kBearingVectorDiff) {
...
void PoseOptimizer::update(const State& T_imuold_world,
...
void PoseOptimizer::applyPrior(const State& T_cur_from_world) {
    if (iter_ == 0) {
        ...
        // std::cout << "prior_lambda (pose_opt): " << prior_lambda_ <<
        // std::endl;
        // std::cout << H_max_diag << std::endl;
        if (solver_options_.verbose) {
            std::cout << "applying rotation prior, I = "
                      << H_max_diag * prior_lambda_ << std::endl;
        }
    }

    H_.noalias() += I_prior_;
    // Jres_.noalias() +=
    // I_prior_*Transformation::log(prior_*T_cur_from_world.inverse());
    g_.noalias() -=
        I_prior_ * Transformation::log(T_cur_from_world * prior_.inverse());
...
void calculateFeatureResidualUnitPlane(
    ...
    if (unwhitened_error) *unwhitened_error = e.norm();

    // Whiten error: R*e, where R is the square root of information matrix
    // (1/sigma).
...
void calculateEdgeletResidualUnitPlane(
    ...
    if (unwhitened_error) *unwhitened_error = std::abs(e);
...
void calculateEdgeletResidualBearingVectorDiff(
    ...
    *chi2_error = 0.5 * e * e * weight;
    ...
    if (H && g) {
        ...
        PoseOptimizer::Vector6d J_ftf =
            2 * f_diff.transpose() * (-1.0) * J_bearing;
        PoseOptimizer::Vector6d J_ptp =
            2 * px_diff.transpose() * (-1.0) * J_proj;
