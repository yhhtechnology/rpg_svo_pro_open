...
bool saveMap(const MapPtr& map,
    ...
    for (const auto& keyval : map->keyframes_) {
        ...
        const Eigen::Quaterniond& q =
            frame->T_world_cam().getRotation().toImplementation();
        out << YAML::BeginMap << YAML::Key << "frame_id" << YAML::Value
            << frame->id() << YAML::Key << "cam_name" << YAML::Value
            << frame->cam()->getLabel() << YAML::Key << "timestamp"
            << YAML::Value << frame->getTimestampNSec() << YAML::Key
            << "T_world_cam" << YAML::Value << YAML::Flow << YAML::BeginSeq
            << t.x() << t.y() << t.z() << q.x() << q.y() << q.z() << q.w()
            << YAML::EndSeq
            //<< YAML::Comment("tx ty tz qx qy qz qw")
            << YAML::EndMap;
    }
    out << YAML::EndSeq << YAML::Key << "features" << YAML::Value
        << YAML::BeginSeq;
    ...
    */
    ...
    out << YAML::EndSeq << YAML::Key << "points" << YAML::Value
        << YAML::BeginSeq;

    // safe points
    for (const PointPtr point : points) {
        out << YAML::BeginMap << YAML::Key << "point_id" << YAML::Value
            << point->id() << YAML::Key << "pos" << YAML::Value << YAML::Flow
            << YAML::BeginSeq << point->pos_[0] << point->pos_[1]
...
bool loadMap(const std::string& load_dir, MapPtr& map) {
    ...
    */
    ...
    // load frames
    for (YAML::const_iterator it = data["frames"].begin();
         it != data["frames"].end(); ++it) {
        YAML::Node frame = *it;
        ...
        FramePtr f = std::make_shared<Frame>(frame["frame_id"].as<int>(),
                                             frame["timestamp"].as<double>(),
                                             cam, Transformation(q_wc, t_wc));
    ...
    for (YAML::const_iterator it = data["points"].begin();
         it != data["points"].end(); ++it) {
        YAML::Node point = *it;
        PointPtr p = std::make_shared<Point>(
            point["point_id"].as<int>(),
            Vector3d(point["pos"][0].as<double>(), point["pos"][1].as<double>(),
    ...
    // load features
    size_t n = 0;
    for (YAML::const_iterator it = data["features"].begin();
        ...
        Eigen::Vector2d px(ftr["px"][0].as<double>(),
                           ftr["px"][1].as<double>());
