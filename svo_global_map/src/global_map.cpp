...
void CachedPointObservations::addObservation(const FramePtr& f,
    ...
    if (!is_parallax_good_ && obs_unit_bearing_from_pt_.size() >= 2) {
        for (size_t i = 0; i < obs_unit_bearing_from_pt_.size() - 1; i++) {
            const double parallax = std::acos(obs_unit_bearing_from_pt_[i].dot(
                obs_unit_bearing_from_pt_.back()));
...
GlobalMap::GlobalMap(const GlobalMapOptions& options,
    ...
    if (options_.use_imu && options_.use_relative_pose_fixation) {
        std::cout
            << "Use both IMU and relative constraints."
    ...
    CachedPointObservations::min_parallax_rad =
        options_.min_parallax_thresh_rad;
    ...
    CachedPointObservations::fim_det_cbr_minimum =
        1.0 / options_.pt_sigma_thresh;
...
void GlobalMap::addKeyframe(const FramePtr& new_frame) {
    ...
    if (options_.ignore_redundant_landmarks && !reprojected_landmark_grid_) {
        ...
        reprojected_landmark_grid_.reset(new OccupandyGrid2D(
            cell_size, OccupandyGrid2D::getNCell(new_frame->cam()->imageWidth(),
                                                 cell_size),
            OccupandyGrid2D::getNCell(new_frame->cam()->imageHeight(),
                                      cell_size)));
    }
    ...
    // add to the id - keyframe map
    {
        ...
        if (options_.use_imu) {
            if (from_id_to_imu_meas_map_.find(new_frame->id()) ==
                ...
                out_ << "\n>>> Cannot find IMU measurements slot from frame id "
                     << new_frame->id() << ", will not add to the graph."
                     << std::endl;
                return;
            }
        }
        ...
        frame_id_to_vis_pt_ids_in_graph_map_[new_frame->id()] =
            UniquePointIds{};
    ...
    // update the graph
    {
        ...
        if (!same_bundle_already_added_) {
            graph_->augmentStateWithPose(cur_bundle_id,
                                         new_frame->T_world_imu());
            out_ << "\n>>> Added frame id " << cur_bundle_id
                 << " with time (sec) "
            ...
            if (options_.refine_extrinsics) {
                const size_t cam_id =
                    static_cast<size_t>(new_frame->nframe_index_);
            ...
            if (options_.use_imu) {
                ...
                if (prev_added_frame_id_ != -1) {
                    ...
                    getIMUMeasurementsBetween(prev_added_frame_id_,
                                              newly_added_frame_id_, true,
                                              &imu_meas_from_prev);

                    CHECK(imu_constraints_frame_id_pairs_.find(
                              prev_added_frame_id_) ==
        ...
        if (options_.verbose) {
            ...
            for (const auto& v :
                 frame_id_to_vis_pt_ids_in_graph_map_[cur_frame_id]) {
                out_ << v << ", ";
            }
            out_ << std::endl;
        }
    ...
    if (initial_ba_fixed_) {
        out_ << ">>> === GLOBAL MAP INFO ===" << std::endl
             ...
             << "; "
             << "- " << frame_id_with_inertial_states_.size()
             << " inertial states"
             << "; "
             ...
             << " frames (excluding the initial one) and "
             << fixed_pts_all_.size() << " points with priors; "
             ...
             << match_points_info_to_add_.size() << " frame matches."
             << std::endl
             << "- " << imu_constraints_frame_id_pairs_.size()
             << " IMU constraints"
             << "; "
...
void GlobalMap::updateStates() {
    if (optimizer_->latest_estimate_state_index_ > last_updated_bundle_id_) {
        ...
        updateStatesWithEstimate(
            optimizer_->estimate_,
            optimizer_->latest_estimate_state_index_.load());
        last_updated_bundle_id_ =
            optimizer_->latest_estimate_state_index_.load();
    }
}

void GlobalMap::quitOptimizerThread() { optimizer_->quitThread(); }

void GlobalMap::getIMUMeasurementsBetween(
    const int start_f_id,
    const int end_f_id,
    const bool remove,
...
bool GlobalMap::checkAndFixInitialBA() {
    ...
    for (auto it = frame_id_to_kf_in_graph_map_.cbegin();
        ...
        out_ << "- Bundle id " << f->bundleId() << " (frame " << f->id()
             << ") has " << vis_pts_in_ba.size() << " visible landmarks."
             << std::endl;
    ...
    // pose and/or points fixation
    {
        if (options_.use_imu) {
            const Frame& init_f =
                *(frame_id_to_kf_in_graph_map_.begin()->second);
...
void GlobalMap::updatePriors() {
    if (options_.debug_fix_all_poses) {
        const FramePtr& new_f =
            frame_id_to_kf_in_graph_map_[newly_added_frame_id_];
    ...
    if (!options_.use_imu) {
        CHECK_GT(fixed_pts_all_.size(), 0u)
            << "We should have initial fixation "
    ...
    if (options_.use_relative_pose_fixation) {
        if (frame_id_to_kf_in_graph_map_.size() > 2) {
            checkAndFixRelativePose(prev_added_frame_id_,
                                    newly_added_frame_id_);
        }
    }
}
...
bool GlobalMap::checkAndFixRelativePose(const int prev_id, const int cur_id) {
    ...
    out_ << "There are " << n_common_lm << " common landmarks between "
         << prev_id << " and " << cur_id << std::endl;
...
void GlobalMap::addRangePriorUnsafe(const Point& pt,
                                    const Frame& f,
...
void GlobalMap::fixFramePoseUnsafe(const FramePtr& f,
...
void GlobalMap::fixRelativePoseUnsafe(const FramePtr& f1,
                                      const FramePtr& f2,
                                      const gtsam::SharedNoiseModel& noise) {
    CHECK_NOTNULL(noise.get());

    Transformation T_1_2 =
        frame_id_to_unoptimized_pose_map_[f1->id()].inverse() *
...
void GlobalMap::checkAndRemoveRelativePosePriors() {
    for (auto it = fixed_relative_frame_id_pairs_.begin();
        ...
        if (n_common_lm > options_.relative_release_min_common) {
            ...
            out_ << "Released the fixation for frame pair: " << id1 << " and "
                 << id2 << std::endl;
        } else {
            it++;
        }
...
void GlobalMap::addVisualMeasurementsToGraph(const FramePtr& new_frame) {
    ...
    for (size_t ftr_idx = 0; ftr_idx < new_frame->numFeatures(); ftr_idx++) {
        ...
        if (it_in_ba !=
            pt_id_to_lm_in_graph_map_
                .end()) {  // if it is already added to the graph
            CHECK(it_cached == pt_id_to_cached_pt_obs_map_.end());
            { this->addObservationToLandmark(new_frame, ftr_idx); }
        } else if (it_cached ==
                   pt_id_to_cached_pt_obs_map_.end()) {  // if it is not in the
                                                         // graph and not cahced
                                                         // yet
    ...
    UniquePointIds good_lm_ids;
    {
        for (auto it = pt_id_to_cached_pt_obs_map_.begin();
            ...
            if (it->second.isPointGood()) {
                ...
                if (options_.add_range_prior_to_good_pt &&
                    !options_.use_smart_factor) {
...
void GlobalMap::addReobservedLandmarkToGraph(const FramePtr& new_frame) {
    ...
    for (size_t ftr_idx = 0; ftr_idx < new_frame->numFeatures(); ftr_idx++) {
        ...
        ftr_info[kNumPrevObs] = std::count_if(
            pt->obs_.begin(), pt->obs_.end(), [&](const KeypointIdentifier& v) {
            ...
            [&](const std::pair<size_t, FtrInfo>& lhs,
                ...
                return (lhs.second[kOldest] < rhs.second[kOldest]) ||
                       (lhs.second[kOldest] == rhs.second[kOldest] &&
                        lhs.second[kNumPrevObs] > rhs.second[kNumPrevObs]);
            });
    }
...
void GlobalMap::updateFixedLandmarks() {
    ...
    for (auto it = lm_id_to_num_reobs_map_.begin();
        ...
        if (it->second >=
                static_cast<size_t>(options_.fix_lm_reobs_threshold) &&
    ...
    for (auto match_it = match_points_info_to_add_.begin();
        ...
        for (auto pit = cur_match_info.pt_id_matches_.begin();
            ...
            if (first_id == second_id) {
                ...
                if (all_in_graph) {
                    n_reobserved_landmarks_in_graph_++;
                    for (const int f_id : {cur_match_info.lc_kf_id_,
                                           cur_match_info.cur_kf_id_}) {
...
bool GlobalMap::isFeatureGoodToAdd(const Frame& f, const size_t ftr_idx) {
    ...
    if (ft == FeatureType::kOutlier ||
        (!options_.use_edgelet && isEdgelet(ft))) {
        return false;
    }
    // point
...
void GlobalMap::addObservationToLandmark(const FramePtr& f,
    ...
    if (graph_->isPointIDSmartLandmark(f->landmark_vec_[ftr_idx]->id())) {
        ...
        graph_->addObservationToLandmark(*f, ftr_idx,
                                         isTbcAdded(f->nframe_index_));
    }
        ...
        for (const auto pt_id : it_vis->second) {
            ...
            if (getPointFromGraphByIdUnsafe(pt_id, estimate, &new_pos)) {
                if (options_.point_stable_thresh_meter > 0 &&
                    (new_pos - pt->pos_).norm() >
                        options_.point_stable_thresh_meter) {
                    f->type_vec_[ftr_idx] = FeatureType::kOutlier;
                } else {
...
void GlobalMap::getOverlapKeyframesMaxN(const Frame& query_f,
                                        const int max,
...
void GlobalMap::getQueryKFPositions(
    rpg::PositionVec* query_kf_pos,
...
void GlobalMap::getVizLinks(VisibilityInfoVec* vis_info,
                            const int last_n) const {
...
void GlobalMap::getKeypointsVisLinks(VisibilityInfoVec* vis_info) const {
    ...
    for (const auto& e : frame_id_to_kf_in_graph_map_) {
        ...
        const Eigen::Vector3f f_pos =
            f->T_world_imu().getPosition().cast<float>();
