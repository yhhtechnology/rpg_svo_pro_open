...
GTSAMOptimizer::~GTSAMOptimizer() { quitThread(); }
...
bool GTSAMOptimizer::optimizeImpl() {
    ...
    BundleId last_added_state_id_before_optimization = -1;
    {
        ...
        last_added_state_id_before_optimization =
            graph_->last_added_state_index_;
    ...
    if (new_smart_factor_point_ids.size() > 0) {
        ...
        for (size_t i = 0; i < new_smart_factor_point_ids.size(); i++) {
            ...
            if (!s_ptr->point().is_initialized() || !s_ptr->isValid()) {
                if (s_ptr->triangulateSafe(s_ptr->cameras(all_values))
                        .valid()) {
                    n_triang++;
                }
            }
        }
        std::cout << "Triangulated " << n_triang
                  << " points before optimization." << std::endl;
    }
