...
struct GlobalMapOptions {
    ...
    // Initialization: we wait until there are 'init_min_frame_before_opt'
    // frames
...
struct CachedPointObservations {
    ...
    CachedPointObservations() { fim_.setZero(); }
...
class GlobalMap {
 public:
    ...
    // Get keyframe positions: all the keyframes and the keyframes used for
    // query
    ...
    // The visiblity lines for keypoints (used for determining overlapping
    // views)
    ...
    inline void getReobservedPointsVisLinksFrontend(
        VisibilityInfoVec* vis_info) const {
        this->framePointsToVisLink(frame_id_to_reobs_pts_ids_frontend_,
                                   vis_info);
    }
    ...
    inline void accumulateIMUMeasurements(const ImuMeasurements& imu_meas) {
        ...
        if (imu_meas_since_cur_start_kf_.front().timestamp_ >
            imu_meas.back().timestamp_) {
            return;
        }
        // CHECK_LT(imu_meas_since_cur_start_kf_.front().timestamp_,
        // imu_meas.back().timestamp_);
        imu_meas_since_cur_start_kf_.insert(
    ...
    inline void addMatchingPointInfo(const MatchedPointsInfo& match_info) {
        ...
        if (match_points_info_to_add_.size() > kMaxN) {
            match_points_info_to_add_.erase(
                match_points_info_to_add_.begin() + kMaxN,
    ...
    inline bool isAllOptimizationDone() const {
        ...
        if (it == frame_id_to_kf_in_graph_map_.end()) {
            ...
            return last_updated_bundle_id_ ==
                   frame_id_to_kf_in_graph_map_.at(newly_added_frame_id_)
                       ->bundleId();
        }
    }

    inline bool hasInitialBA() const { return initial_ba_fixed_; }

    // Get the bundle id -> pose map
    void getBundleIDIMUPosesMap(BundleIdToIMUPose* pose_map) const;

    // Get maximum N overlapped keyframes
    void getOverlapKeyframesMaxN(const Frame& query_f,
                                 const int max,
 ...
 private:
    ...
    void getIMUMeasurementsBetween(const int start_f_id,
                                   const int end_f_id,
                                   const bool remove,
    ...
    bool addObservationToLandmarkByPointId(const FramePtr& f,
                                           const int point_id);
    ...
    // - Range priors
    void addRangePriorUnsafe(const Point& pt,
                             const Frame& f,
                             const double ratio_dist);

    // Bookkeeping functions
    // - visible points to frame
    inline void registerPointVisibleToFrame(const int frame_id,
                                            const int pt_id,
        ...
        auto f_to_ftr_idx_it =
            frame_id_to_vis_ftr_idx_in_graph_map_.find(frame_id);
    ...
    inline void registerFrameRelatedPoints(const int frame_id,
                                           const int pt_id,
    ...
    inline void getKfToPointLink(const FramePtr& f,
                                 const int pt_id,
    ...
    inline bool pointVisibleInFrameInGraph(const int pt_id,
                                           const int frame_id) {
    ...
    inline bool hasFutureObservations(const Frame& f,
                                      const size_t ftr_idx) const {
