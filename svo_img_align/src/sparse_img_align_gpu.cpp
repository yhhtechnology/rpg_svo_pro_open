...
SparseImgAlignGpu::SparseImgAlignGpu(SolverOptions optimization_options,
                                     SparseImgAlignOptions options)
    : SparseImgAlignBase(optimization_options, options) {
    setPatchSize<SparseImgAlignGpu>(4);
}

size_t SparseImgAlignGpu::run(const FrameBundle::Ptr& ref_frames,
    ...
    // Transform to GPU datatypes. TODO: This should be done only ones for
    // transformation/camera data
    ...
    if (dynamic_cast<FrameGpu*>(ref_frames->at(0).get()) == nullptr ||
        !dynamic_cast<FrameGpu*>(ref_frames->at(0).get())->cu_camera_) {
        SVO_WARN_STREAM(
            "Sparse image align: Input frame is not a GPU. Creating GPU data "
            "structures now.");
        // Input is not a GPU frame bundle. GPU datastructures need to be
        // instantiated.
        for (int i = 0; i < nbr_cameras; ++i) {
            ...
            for (int j = 0;
                 j < static_cast<int>(cu_ref_imgs_pyramid_copy_.at(i).size());
                 ++j) {
                cu_ref_imgs_pyramid_copy_.at(i).at(j) =
                    std::make_shared<imp::cu::ImageGpu8uC1>(
                        imp::cu::ImageGpu8uC1(imp::ImageCv8uC1(
                            ref_frames_->frames_.at(i)->img_pyr_.at(j))));
                cu_cur_imgs_pyramid_copy_.at(i).at(j) =
                    std::make_shared<imp::cu::ImageGpu8uC1>(
                        imp::cu::ImageGpu8uC1(imp::ImageCv8uC1(
                            cur_frames_->frames_.at(i)->img_pyr_.at(j))));
            }

            cu_T_imu_cam_bundle_.at(i) =
                std::make_shared<imp::cu::Matrix<FloatTypeGpu, 3, 4> >(
                    imp::cu::Matrix<FloatTypeGpu, 3, 4>(
                        ref_frames->frames_.at(i)
                            ->T_imu_cam()
                            .getTransformationMatrix()
                            .block<3, 4>(0, 0)
                            .cast<FloatTypeGpu>()));
            cu_T_cam_imu_bundle_.at(i) =
                std::make_shared<imp::cu::Matrix<FloatTypeGpu, 3, 4> >(
                    imp::cu::Matrix<FloatTypeGpu, 3, 4>(
                        ref_frames->frames_.at(i)
                            ->T_cam_imu()
            ...
            aslam::PinholeCamera* pinhole = dynamic_cast<aslam::PinholeCamera*>(
                ref_frames->frames_.at(i)->cam().get());

            if ((pinhole != nullptr) &&
                (pinhole->getDistortion().getType() ==
                 aslam::Distortion::Type::kNoDistortion)) {
                cu_camera_bundle_.at(i) =
                    std::make_shared<imp::cu::PinholeCamera>(
                        imp::cu::PinholeCamera(pinhole->fu(), pinhole->fv(),
                                               pinhole->cu(), pinhole->cv()));
            } else {
                SVO_ERROR_STREAM(
                    "Camera model not supported yet for GPU version");
                return 0;
            }
        }

    } else {
        // Input is a GPU frame bundle. Simply copy the pointers to the GPU
        // data.
        for (int i = 0; i < nbr_cameras; ++i) {
            cu_ref_imgs_pyramid_copy_.at(i).resize(options_.max_level + 1);
            cu_cur_imgs_pyramid_copy_.at(i).resize(options_.max_level + 1);
            ...
            for (int j = 0;
                 ...
                 ++j) {
                cu_ref_imgs_pyramid_copy_.at(i).at(j) =
                    std::static_pointer_cast<FrameGpu>(ref_frames_->at(i))
                        ->cu_img_pyramid_copy_.at(j);
                cu_cur_imgs_pyramid_copy_.at(i).at(j) =
                    std::static_pointer_cast<FrameGpu>(cur_frames_->at(i))
                        ->cu_img_pyramid_copy_.at(j);
            }
    ...
    for (auto frame : ref_frames->frames_) {
        ...
        sparse_img_align_host_utils::extractFeaturesSubset(
            *frame, options_.max_level, patch_size_with_border_, nbr_extracted,
            host_cache_);
    ...
    sparse_img_align_device_utils::precomputeBaseCaches(
        host_cache_.uv_cache, host_cache_.xyz_ref_cache,
        host_cache_.first_ftr_index, host_cache_.nbr_of_ftrs,
        cu_T_imu_cam_bundle_, cu_T_cam_imu_bundle_, cu_camera_bundle_,
        host_cache_.total_nbr_of_ftrs, gpu_cache_);

    // To reserve sufficient memory for the reduction step we need to compute
    // the number of blocks and threads
    // for the reduction step.
    sparse_img_align_device_utils::computeNumBlocksAndThreadsReduction(
        host_cache_.total_nbr_of_ftrs, patch_area_, gpu_props_,
        num_blocks_reduce_, num_threads_reduce_);

    gpu_cache_.reserveReductionCacheCapacity(
        static_cast<size_t>(num_blocks_reduce_));
...
double SparseImgAlignGpu::evaluateError(const SparseImgAlignState& state,
                                        HessianMatrix* H,
                                        GradientVector* g) {
    if (!have_cache_)  // is reset at every new level.
    {
        ...
        sparse_img_align_device_utils::precomputeJacobiansAndRefPatches(
            cu_ref_imgs_pyramid_copy_, level_, patch_size_,
            options_.estimate_illumination_gain,
            options_.estimate_illumination_offset, host_cache_.first_ftr_index,
            host_cache_.nbr_of_ftrs, gpu_cache_);
    ...
    for (size_t i = 0; i < cu_T_cam_imu_bundle_.size(); ++i) {
        const Transformation T_cur_ref = cur_frames_->at(i)->T_cam_imu() *
                                         state.T_icur_iref *
                                         ref_frames_->at(i)->T_imu_cam();
        cu_T_cur_ref_bundle_.at(i) = imp::cu::Matrix<FloatTypeGpu, 3, 4>(
            T_cur_ref.getTransformationMatrix()
                .block<3, 4>(0, 0)
                .cast<FloatTypeGpu>());
    }
    ...
    sparse_img_align_device_utils::computeResidualsOfFrame(
        cu_cur_imgs_pyramid_copy_, cu_T_cur_ref_bundle_, cu_camera_bundle_,
        host_cache_.first_ftr_index, host_cache_.nbr_of_ftrs, level_,
        patch_size_, state.alpha, state.beta, gpu_cache_);

    FloatTypeGpu chi2 =
        sparse_img_align_device_utils::computeHessianAndGradient(
            H, g, host_cache_.total_nbr_of_ftrs * patch_area_, patch_area_,
            gpu_cache_, num_blocks_reduce_, num_threads_reduce_);
...
void extractFeaturesSubset(
    ...
    for (size_t i = 0; i < ref_frame.num_features_; ++i) {
        ...
        const FloatTypeGpu u_tl =
            ref_frame.px_vec_(0, i) * scale - patch_center_wb;
        const FloatTypeGpu v_tl =
            ref_frame.px_vec_(1, i) * scale - patch_center_wb;
        const int u_tl_i = std::floor(u_tl);
        const int v_tl_i = std::floor(v_tl);
        if (!(u_tl_i < 0 || v_tl_i < 0 ||
              u_tl_i + patch_size_wb >= cols_minus_one ||
              v_tl_i + patch_size_wb >= rows_minus_one)) {
            host_cache.push_uv(
                static_cast<FloatTypeGpu>(ref_frame.px_vec_(0, i)),
                static_cast<FloatTypeGpu>(ref_frame.px_vec_(1, i)));

            // evaluate jacobian. cannot just take the 3d points coordinate
            // because of
            ...
            if (ref_frame.landmark_vec_[i]) {
                ...
                const Position pos =
                    seed_ref.keyframe->T_world_cam() *
                    (seed_ref.keyframe->f_vec_.col(seed_ref.seed_id) * 1.0 /
                     seed_ref.keyframe->invmu_sigma2_a_b_vec_(
                         0, seed_ref.seed_id));
                depth = (pos - ref_pos).norm();
            }

            const Vector3ftGpu xyz_ref(
                ref_frame.f_vec_.col(i).cast<FloatTypeGpu>() * depth);
            host_cache.push_xyz(xyz_ref(0), xyz_ref(1), xyz_ref(2));
            nr_fts_extracted++;
        }
    }
    SVO_DEBUG_STREAM(
        "Img Align: Maximum Number of Features = " << ref_frame.num_features_);
}
