...
SparseImgAlign::SparseImgAlign(SolverOptions optimization_options,
    ...
    for (size_t i = 0; i < ref_frames_->size(); ++i) {
        sparse_img_align_utils::precomputeBaseCaches(
            *ref_frames_->at(i), fts_vec_.at(i),
            options_.use_distortion_jacobian, feature_counter, uv_cache_,
...
double SparseImgAlign::evaluateError(const SparseImgAlignState& state,
                                     HessianMatrix* H,
                                     GradientVector* g) {
    if (!have_cache_)  // is reset at every new level.
    {
        ...
        for (size_t i = 0; i < ref_frames_->size(); ++i) {
            sparse_img_align_utils::precomputeJacobiansAndRefPatches(
                ref_frames_->at(i), uv_cache_, jacobian_proj_cache_, level_,
                patch_size_, fts_vec_.at(i).size(),
                options_.estimate_illumination_gain,
                options_.estimate_illumination_offset, feature_counter,
    ...
    for (size_t i = 0; i < ref_frames_->size(); ++i) {
        ...
        sparse_img_align_utils::computeResidualsOfFrame(
            cur_frames_->at(i), level_, patch_size_, fts_vec_.at(i).size(),
            T_cur_ref, state.alpha, state.beta, ref_patch_cache_,
            xyz_ref_cache_, feature_counter, match_pxs, residual_cache_,
            visibility_mask_);
    }

    float chi2 = sparse_img_align_utils::computeHessianAndGradient(
        jacobian_cache_, residual_cache_, visibility_mask_, weight_scale_,
        weight_function_, H, g);
    return chi2;
}
    ...
    if (false) {
        ...
        for (size_t i = 0; i < nr_features; ++i) {
            ...
            size_t pixel_counter =
                0;  // is used to compute the index of the cached residual
        ...
        std::vector<cv::Mat> residual_image_rgb_vec(
            {cv::Mat(residual_image.size(), CV_32FC1, cv::Scalar(1.0)),
             residual_image,
...
void extractFeaturesSubset(
    ...
    //  if(fts.size() < 100)
    //  {
    //    std::unique_lock<decltype(ref_frame->seeds_mut_)>
    //    lock(ref_frame->seeds_mut_);
    //    size_t n = 0;
...
void precomputeBaseCaches(const Frame& ref_frame,
    ...
    for (const size_t i : fts) {
        uv_cache.col(feature_counter) =
            ref_frame.px_vec_.col(i).cast<FloatType>();

        // evaluate jacobian. cannot just take the 3d points coordinate because
        // of
        ...
        if (!use_distortion_jac &&
            ref_frame.cam()->getType() ==
                Camera::Type::kPinhole) {  // only allow ignoring jacobian for
                                           // pinhole projection
            Frame::jacobian_xyz2uv_imu(T_cam_imu, xyz_in_imu, frame_jac);
            frame_jac *= focal_length;
        } else {
            Frame::jacobian_xyz2image_imu(*ref_frame.cam(), T_cam_imu,
                                          xyz_in_imu, frame_jac);
            frame_jac *= (-1.0);
        }

        size_t col_index = 2 * feature_counter;
        jacobian_proj_cache.col(col_index) = frame_jac.row(0).cast<FloatType>();
        jacobian_proj_cache.col(col_index + 1) =
            frame_jac.row(1).cast<FloatType>();
        ++feature_counter;
    }
}
...
void precomputeJacobiansAndRefPatches(
    ...
    const int border = 1;
    const int patch_size_wb = patch_size + 2 * border;  // patch size with
                                                        // border
    ...
    for (size_t i = 0; i < nr_features; ++i, ++feature_counter) {
        // compute top left coordinate of patch to be interpolated
        const FloatType u_tl =
            uv_cache(0, feature_counter) * scale - patch_center_wb;
        const FloatType v_tl =
            uv_cache(1, feature_counter) * scale - patch_center_wb;
        ...
        for (int y = 0; y < patch_size_wb; ++y) {
            ...
            uint8_t* r = static_cast<uint8_t*>(ref_img.data) +
                         (v_tl_i + y) * stride + u_tl_i;
            for (int x = 0; x < patch_size_wb; ++x, ++r, ++pixel_counter) {
                // precompute interpolated reference patch color
                interp_patch_array[pixel_counter] = wtl * r[0] + wtr * r[1] +
                                                    wbl * r[stride] +
                                                    wbr * r[stride + 1];
            }
        }
        ...
        for (int y = 0; y < patch_size; ++y) {
            for (int x = 0; x < patch_size; ++x, ++pixel_counter) {
                ...
                // we use the inverse compositional: thereby we can take the
                // gradient
                // always at the same position.
                const FloatType dx =
                    0.5f * (interp_patch_array[offset_center + 1] -
                            interp_patch_array[offset_center - 1]);
                const FloatType dy =
                    0.5f * (interp_patch_array[offset_center + patch_size_wb] -
                ...
                jacobian_cache.block<6, 1>(0, jacobian_col) =
                    (dx * jacobian_proj_cache.col(jacobian_proj_col) +
                     dy * jacobian_proj_cache.col(jacobian_proj_col + 1)) *
                    scale;
                jacobian_cache(6, jacobian_col) =
                    estimate_alpha ? -(interp_patch_array[offset_center]) : 0.0;
...
void computeResidualsOfFrame(const FramePtr& cur_frame,
    ...
    for (size_t i = 0; i < nr_features; ++i, ++feature_counter) {
        ...
        float total_res = 0.0;
        for (int y = 0; y < patch_size; ++y) {
            uint8_t* cur_img_ptr = static_cast<uint8_t*>(cur_img.data) +
                                   (v_tl_i + y) * stride + u_tl_i;

            for (int x = 0; x < patch_size;
                 ++x, ++pixel_counter, ++cur_img_ptr) {
                // compute residual, with bilinear interpolation
                FloatType intensity_cur =
                    wtl * cur_img_ptr[0] + wtr * cur_img_ptr[1] +
                    wbl * cur_img_ptr[stride] + wbr * cur_img_ptr[stride + 1];
                FloatType res = static_cast<FloatType>(
                                    intensity_cur * (1.0 + alpha) + beta) -
                                ref_patch_cache(pixel_counter, feature_counter);
                residual_cache(pixel_counter, feature_counter) = res;

                // for camera control:
                total_res +=
                    intensity_cur - static_cast<float>(ref_patch_cache(
    ...
    for (size_t i = 0; i < mask_size; ++i) {
        if (visibility_mask(i) == true) {
            ...
            for (size_t j = 0; j < patch_area; ++j) {
                ...
                // Compute Jacobian, weighted Hessian and weighted "steepest
                // descend images" (times error).
...
float computeResidualHessianGradient(
    ...
    for (size_t i = 0; i < nr_features; ++i, ++feature_counter) {
        ...
        for (int y = 0; y < patch_size; ++y) {
            uint8_t* cur_img_ptr =
                (uint8_t*)cur_img.data + (v_tl_i + y) * stride + u_tl_i;
            ...
            for (int x = 0; x < patch_size;
                ...
                // compute Jacobian, weighted Hessian and weighted "steepest
                // descend images" (times error)
                const Vector8ft J_ft =
                    jacobian_cache.col(patch_offset + pixel_counter);
                const Vector8d J_d = J_ft.cast<double>();
                H->noalias() += J_d * J_d.transpose() * weight;
                g->noalias() -= J_d * res * weight;
