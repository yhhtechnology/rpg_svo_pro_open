...
SparseImgAlignBase::SparseImgAlignBase(SolverOptions optimization_options,
                                       SparseImgAlignOptions options)
    : vk::solver::
          MiniLeastSquaresSolver<8, SparseImgAlignState, SparseImgAlignBase>(
              optimization_options),
      options_(options),
      weight_function_(options_.robustification
                           ? (new vk::solver::TukeyWeightFunction)
                           : nullptr),
      weight_scale_(options_.weight_scale) {}

SparseImgAlignBase::SolverOptions
SparseImgAlignBase::getDefaultSolverOptions() {
...
void SparseImgAlignBase::setWeightedPrior(const Transformation& T_cur_ref_prior,
...
void SparseImgAlignBase::update(const SparseImgAlignState& state_old,
                                const UpdateVector& dx,
                                SparseImgAlignState& state_new) {
    state_new.T_icur_iref =
        state_old.T_icur_iref * Transformation::exp(-dx.head<6>());
...
void SparseImgAlignBase::applyPrior(const SparseImgAlignState& state) {
    if (iter_ == 0) {
        ...
        Matrix3d I_trans =
            Matrix3d::Identity() * prior_lambda_trans_ * H_max_diag_trans;

        double H_max_diag_rot = 0;
        for (size_t j = 3; j < 6; ++j)
            H_max_diag_rot = std::max(H_max_diag_rot, std::fabs(H_(j, j)));
        Matrix3d I_rot =
            Matrix3d::Identity() * prior_lambda_rot_ * H_max_diag_rot;
    ...
    H_.noalias() += I_prior_;
    g_.head<6>() +=
        I_prior_.block<6, 6>(0, 0) *
        Transformation::log(prior_.T_icur_iref.inverse() * state.T_icur_iref);
