...
class SparseImgAlignBase
    : public vk::solver::
          MiniLeastSquaresSolver<8, SparseImgAlignState, SparseImgAlignBase> {
 public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    ...
    SparseImgAlignBase(SolverOptions optimization_options,
    ...
    void setWeightedPrior(const Transformation& T_cur_ref_prior,
    ...
    /// The derived class has to specify the necessary modifications (e.g.
    /// memory allocation)
    /// in case the patchsize is changed.
    virtual void setPatchSizeSideEffects() = 0;

    /// Set number of features used for the alignment. If a number is <= 0 all
    /// features are used
    /// Specifying a number gives you the control about the performance of the
    /// alignment.
    /// Having an alignment running over 600 or 100 features doesn't make that
    /// much a difference
    inline void setMaxNumFeaturesToAlign(int num) { max_num_features_ = num; }
    ...
    virtual double evaluateError(const SparseImgAlignState& state,
                                 HessianMatrix* H,
                                 GradientVector* g) = 0;
    ...
    static constexpr int kHessianTriagN =
        36;  // Nr elements of the upper triangular part of the Hessian

 protected:
    SparseImgAlignOptions options_;
    FrameBundle::Ptr
        ref_frames_;  //!< reference frame, has depth for gradient pixels.
    FrameBundle::Ptr cur_frames_;  //!< only the image is known!
    Transformation
        T_iref_world_;  //!< Pose of reference frame's IMU in world coordinates
    ...
    int max_num_features_ =
        -1;  //!< number of features used for alignment (if -1, all are used)
