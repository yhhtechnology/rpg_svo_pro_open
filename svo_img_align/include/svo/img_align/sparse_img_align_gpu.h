...
struct HostCacheHandler {
    ...
    inline size_t capacity() const { return uv_cache.capacity(); }

    inline void push_uv(const float& u, const float& v) {
        uv_cache.push_back(
            {static_cast<FloatTypeGpu>(u), static_cast<FloatTypeGpu>(v)});
    }
    ...
    inline void push_xyz(const float& x, const float& y, const float& z) {
        xyz_ref_cache.push_back({static_cast<FloatTypeGpu>(x),
                                 static_cast<FloatTypeGpu>(y),
...
class SparseImgAlignGpu : public SparseImgAlignBase {
 ...
 public:
    SparseImgAlignGpu(SolverOptions optimization_options,
                      SparseImgAlignOptions options);

    void setPatchSizeSideEffects() { gpu_cache_.setPatchArea(patch_area_); }

    size_t run(const FrameBundle::Ptr& ref_frames,
               const FrameBundle::Ptr& cur_frames);

    /**
     * @brief getMedianDisparity returns the median disparty for all features
     * between the current frame and the
     * reference frame. The value is updated when calling
     * sparse_img_align_device_utils::computeDisparity()
     * (see .cpp file)
     */
 ...
 private:
    ...
    // data storage
    std::vector<std::vector<imp::cu::ImageGpu8uC1::Ptr>>
