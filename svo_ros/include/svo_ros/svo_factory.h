...
/// Get IMU Handler.
std::shared_ptr<ImuHandler> getImuHandler(const ros::NodeHandle& pnh);

#ifdef SVO_LOOP_CLOSING
/// Create loop closing module
std::shared_ptr<LoopClosing> getLoopClosingModule(
    const ros::NodeHandle& pnh, const CameraBundlePtr& cam = nullptr);
#endif

#ifdef SVO_GLOBAL_MAP
std::shared_ptr<GlobalMap> getGlobalMap(const ros::NodeHandle& pnh,
