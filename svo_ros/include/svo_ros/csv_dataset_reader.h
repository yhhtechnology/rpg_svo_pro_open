...
struct ImuMeasurement : public MeasurementBase {
    ...
    ImuMeasurement(int64_t timestamp_nanoseconds,
...
struct CameraMeasurement : public MeasurementBase {
    ...
    CameraMeasurement(int64_t timestamp_nanoseconds,
                      size_t camera_index,
...
struct FeatureTrackMeasurement : public MeasurementBase {
    ...
    FeatureTrackMeasurement(int64_t timestamp_nanoseconds,
                            size_t camera_index,
                            int track_id,
                            const Eigen::Vector2d& keypoint_measurement,
                            double keypoint_std_dev)
        : MeasurementBase(timestamp_nanoseconds,
                          MeasurementType::kFeatureTrack),
          camera_index(camera_index),
          track_id(track_id),
...
class CsvDatasetReader {
 public:
    typedef std::pair<int64_t, std::shared_ptr<dataset::MeasurementBase>>
        StampMeasurementPair;
    typedef std::multimap<int64_t, std::shared_ptr<dataset::MeasurementBase>>
        DataBuffer;

    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    ASLAM_POINTER_TYPEDEFS(CsvDatasetReader);

    CsvDatasetReader(const std::string& csv_directory,
 ...
 private:
    void loadImuData(const std::string data_dir, const int64_t playback_delay);

    void loadCameraData(const std::string& data_dir,
                        const size_t camera_index,
                        int64_t playback_delay);

    void loadFeatureTracksData(const std::string& data_dir,
                               const size_t camera_index,
                               int64_t playback_delay);
