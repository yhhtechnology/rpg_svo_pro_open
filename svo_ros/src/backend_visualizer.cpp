...
BackendVisualizer::BackendVisualizer(const std::string& trace_dir,
...
void BackendVisualizer::visualizeFrames(const gtsam::Values& values) {
    ...
    for (auto it = frames.begin(); it != frames.end(); ++it) {
        vk::output_helper::publishFrameMarker(
            pub_markers_, it->value.rotation().matrix(),
            it->value.translation().vector(), "keyframes", ros::Time::now(),
            (i++) * 10, 0, 0.05);
    }
}
...
void BackendVisualizer::visualizeSmartFactors(
    ...
    for (const auto& sf : smart_factors) {
        ...
        if (!sf.second.first->isDegenerate()) {
            ...
            ++n_degenerate;
            if (sf.second.first->isPointBehindCamera()) ++n_cheirality;
        }
    }
    pub_markers_.publish(m);
    ...
    VLOG(30) << "Viz: Number of points behind camera smart-factors:"
             << n_cheirality;
}
...
void BackendVisualizer::visualizePoseCovariance(
    ...
    vk::output_helper::publishGtsamPoseCovariance(
        pub_markers_, T_W_B.getPosition(), T_W_B.getRotationMatrix(),
        covariance, "covariance", trace_id_, 0, 2.0,
        Eigen::Vector4d(0, 0, 0.5, 0.5));
}

void BackendVisualizer::tracePose(
...
void BackendVisualizer::traceStates(const gtsam::Values& values) {
    ...
    for (auto b : biases) {
        ...
        const Eigen::Quaterniond q(T_wi.rotation().matrix());
        ofs_states_ << trace_id_ << " " << id << " " << bias.accelerometer().x()
                    << " " << bias.accelerometer().y() << " "
                    << bias.accelerometer().z() << " " << bias.gyroscope().x()
                    << " " << bias.gyroscope().y() << " "
                    << bias.gyroscope().z() << " " << velocity.x() << " "
                    << velocity.y() << " " << velocity.z() << " "
                    << T_wi.translation().x() << " " << T_wi.translation().y()
                    << " " << T_wi.translation().z() << " " << q.x() << " "
                    << q.y() << " " << q.z() << " " << q.w() << " "
                    << std::endl;
    }
    ++trace_id_;
    ...
    const swe::SweGtsamBookeeper& book)
{
  ...
  {
    // Write header.
    ofs_states_ << "# Iteration count, timestamp [ns], bax, bay, baz, bgx, bgy,
bgz, vx, "
...
void BackendVisualizer::traceCovariance(const Eigen::Matrix<double, 6, 6>& C,
    ...
    ofs_covariance_ << timestamp << ", " << C(0, 0) << "," << C(0, 1) << ","
                    << C(0, 2) << "," << C(0, 3) << "," << C(0, 4) << ","
                    << C(0, 5) << "," << C(1, 0) << "," << C(1, 1) << ","
                    << C(1, 2) << "," << C(1, 3) << "," << C(1, 4) << ","
                    << C(1, 5) << "," << C(2, 0) << "," << C(2, 1) << ","
