...
BaseOptions loadBaseOptions(const ros::NodeHandle& pnh, bool forward_default) {
    ...
    o.trace_dir = vk::param<std::string>(
        pnh, "trace_dir", ros::package::getPath("svo") + "/trace");
    o.quality_min_fts = vk::param<int>(pnh, "quality_min_fts", 50);
    o.quality_max_fts_drop = vk::param<int>(pnh, "quality_max_drop_fts", 40);
    o.relocalization_max_trials =
        vk::param<int>(pnh, "relocalization_max_trials", 50);
    o.poseoptim_prior_lambda =
        vk::param<double>(pnh, "poseoptim_prior_lambda", 0.0);
    o.poseoptim_using_unit_sphere =
        vk::param<bool>(pnh, "poseoptim_using_unit_sphere", false);
    o.img_align_prior_lambda_rot =
        vk::param<double>(pnh, "img_align_prior_lambda_rot", 0.0);
    o.img_align_prior_lambda_trans =
        vk::param<double>(pnh, "img_align_prior_lambda_trans", 0.0);
    o.structure_optimization_max_pts =
        vk::param<int>(pnh, "structure_optimization_max_pts", 20);
    o.init_map_scale = vk::param<double>(pnh, "map_scale", 1.0);
    std::string default_kf_criterion =
        forward_default ? "FORWARD" : "DOWNLOOKING";
    if (vk::param<std::string>(pnh, "kfselect_criterion",
                               default_kf_criterion) == "FORWARD")
    ...
    o.kfselect_numkfs_upper_thresh =
        vk::param<int>(pnh, "kfselect_numkfs_upper_thresh", 120);
    o.kfselect_numkfs_lower_thresh =
        vk::param<double>(pnh, "kfselect_numkfs_lower_thresh", 70);
    o.kfselect_min_dist_metric =
        vk::param<double>(pnh, "kfselect_min_dist_metric", 0.01);
    o.kfselect_min_angle = vk::param<double>(pnh, "kfselect_min_angle", 20);
    o.kfselect_min_disparity =
        vk::param<double>(pnh, "kfselect_min_disparity", 40);
    o.kfselect_min_num_frames_between_kfs =
        vk::param<int>(pnh, "kfselect_min_num_frames_between_kfs", 2);
    o.kfselect_backend_max_time_sec =
        vk::param<double>(pnh, "kfselect_backend_max_time_sec", 3.0);
    ...
    o.img_align_robustification =
        vk::param<bool>(pnh, "img_align_robustification", false);
    ...
    o.use_async_reprojectors =
        vk::param<bool>(pnh, "use_async_reprojectors", false);
...
DetectorOptions loadDetectorOptions(const ros::NodeHandle& pnh) {
    ...
    o.threshold_secondary =
        vk::param<int>(pnh, "detector_threshold_secondary", 200);
    o.threshold_shitomasi =
        vk::param<int>(pnh, "detector_threshold_shitomasi", 100);
...
DepthFilterOptions loadDepthFilterOptions(const ros::NodeHandle& pnh) {
    ...
    o.scan_epi_unit_sphere =
        vk::param<bool>(pnh, "scan_epi_unit_sphere", false);
    o.affine_est_offset =
        vk::param<bool>(pnh, "depth_filter_affine_est_offset", true);
    o.affine_est_gain =
        vk::param<bool>(pnh, "depth_filter_affine_est_gain", false);
...
InitializationOptions loadInitializationOptions(const ros::NodeHandle& pnh) {
    ...
    o.init_min_features_factor =
        vk::param<double>(pnh, "init_min_features_factor", 2.0);
    o.reproj_error_thresh = vk::param<double>(pnh, "reproj_err_thresh", 2.0);
    o.init_disparity_pivot_ratio =
        vk::param<double>(pnh, "init_disparity_pivot_ratio", 0.5);
    std::string init_method =
        vk::param<std::string>(pnh, "init_method", "FivePoint");
    if (init_method == "Homography")
        ...
        SVO_ERROR_STREAM(
            "Initialization Method not supported: " << init_method);
    return o;
}
...
LoopClosureOptions loadLoopClosureOptions(const ros::NodeHandle& pnh) {
    ...
    o.enable_image_logging =
        vk::param<bool>(pnh, "enable_image_logging", false);
    o.image_log_base_path =
        vk::param<std::string>(pnh, "image_log_base_path", "/home/");
    o.proximity_dist_ratio =
        vk::param<double>(pnh, "proximity_dist_ratio", 0.01);
    o.proximity_offset = vk::param<double>(pnh, "proximity_offset", 0.1);
    o.force_correction_dist_thresh_meter =
        vk::param<double>(pnh, "force_correction_dist_thresh_meter", 0.1);
...
GlobalMapPtr getGlobalMap(const ros::NodeHandle& pnh,
    ...
    o.init_min_lm_obs_frame =
        static_cast<size_t>(vk::param<int>(pnh, "gm_init_min_lm_obs_frame", 5));
    o.debug_fix_all_poses =
        vk::param<bool>(pnh, "gm_debug_fix_all_poses", false);
    o.print_stdcout = vk::param<bool>(pnh, "gm_print_stdcout", true);
    o.use_smart_factor = vk::param<bool>(pnh, "gm_use_smart_factor", false);
    o.use_imu = vk::param<bool>(pnh, "gm_use_imu", true);
    o.verbose = vk::param<bool>(pnh, "gm_verbose", false);
    o.use_thread = vk::param<bool>(pnh, "gm_use_thread", false);
    o.only_add_every_n = vk::param<int>(pnh, "gm_only_add_every_n", -1);
    o.optimize_every_n = vk::param<int>(pnh, "gm_optimize_every_n", 5);
    o.ignore_redundant_landmarks =
        vk::param<bool>(pnh, "gm_ignore_redundant_landmarks", false);
    o.ignore_grid_size = vk::param<int>(pnh, "gm_ignore_grid_size", 30);
    ...
    o.refine_extrinsics = vk::param<bool>(pnh, "refine_extrinsics", false);

    o.add_reprojected_fixed_landmark =
        vk::param<bool>(pnh, "gm_add_reprojected_fixed_landmark", false);
    o.max_reprojected_fixed_landmark_each_frame =
        static_cast<size_t>(vk::param<int>(
            pnh, "gm_max_reprojected_fixed_landmark_each_frame", 20));
    o.fix_lm_reobs_threshold =
        vk::param<int>(pnh, "gm_fix_lm_reobs_threshold", 5);

    o.min_num_obs =
        static_cast<size_t>(vk::param<int>(pnh, "gm_min_num_obs", 2));
    o.pt_cond_thresh = vk::param<double>(pnh, "gm_pt_cond_thresh", -1);
    o.pt_sigma_thresh = vk::param<double>(pnh, "gm_pt_sigma_thresh", 0.1);
    ...
    gm_o.init_roll_pitch_sigma_rad =
        vk::param<double>(pnh, "ba_init_roll_pitch_sigma_degree", 45) / 180.0 *
        M_PI;
    gm_o.point_prior_sigma_meter =
        vk::param<double>(pnh, "ba_point_sigma_meter", 0.001);

    GTSAMOptimizerOptions gtsam_opt_options;
    gtsam_opt_options.verbose = vk::param<bool>(pnh, "ba_verbose", false);
    ...
    gtsam_opt_options.trace_dir = vk::param<std::string>(
        pnh, "trace_dir", ros::package::getPath("svo") + "/trace");
    gtsam_opt_options.optim_method = vk::param<std::string>(
        pnh, "isam_optimization_method", std::string("GaussNewton"));
    ...
    GlobalMapPtr gm_ptr =
        std::make_shared<GlobalMap>(o, gm_o, gtsam_opt_options, cams);
...
FrameHandlerMono::Ptr makeMono(const ros::NodeHandle& pnh,
                               const CameraBundlePtr& cam) {
    // Create camera
    CameraBundle::Ptr ncam = (cam) ? cam : loadCameraFromYaml(pnh);
    if (ncam->numCameras() > 1) {
        LOG(WARNING)
            << "Load more cameras than needed, will erase from the end.";
...
FrameHandlerStereo::Ptr makeStereo(const ros::NodeHandle& pnh,
                                   const CameraBundlePtr& cam) {
    // Load cameras
    CameraBundle::Ptr ncam = (cam) ? cam : loadCameraFromYaml(pnh);
...
FrameHandlerArray::Ptr makeArray(const ros::NodeHandle& pnh,
                                 const CameraBundlePtr& cam) {
    // Load cameras
    CameraBundle::Ptr ncam = (cam) ? cam : loadCameraFromYaml(pnh);
