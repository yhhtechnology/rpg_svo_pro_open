...
void checkHeaderAndOpenStream(const std::string& filename,
...
CsvDatasetReader::CsvDatasetReader(const std::string& csv_directory,
    ...
    for (size_t i : camera_indices) {
        ...
        loadCameraData(dir, i,
                       100 * common::conversions::kMilliSecondsToNanoSeconds);
    }
...
void CsvDatasetReader::loadImuData(const std::string data_dir,
                                   const int64_t playback_delay) {
    const std::string kHeader =
        "#timestamp [ns],w_RS_S_x [rad s^-1],w_RS_S_y [rad s^-1],w_RS_S_z [rad "
    ...
    while (std::getline(fs, line)) {
        ...
        dataset::ImuMeasurement::Ptr imu_measurement(
            new dataset::ImuMeasurement(std::stoll(elements[0]),
                                        acc_gyr_measurement));
...
void CsvDatasetReader::loadCameraData(const std::string& data_dir,
    ...
    while (std::getline(fs, line)) {
        ...
        dataset::CameraMeasurement::Ptr camera_measurement(
            new dataset::CameraMeasurement(std::stoll(elements[0]),
                                           camera_index,
                                           data_dir + "/data/" + elements[1]));
...
void CsvDatasetReader::loadFeatureTracksData(const std::string& /*data_dir*/,
                                             const size_t /*camera_index*/,
                                             int64_t /*playback_delay*/) {
