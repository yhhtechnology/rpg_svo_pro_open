...
MotionDetectorOptions loadMotionDetectorOptions(const ros::NodeHandle& pnh) {
    ...
    o.ratio_moving_pixels_threshold = vk::param<double>(
        pnh, "zero_motion_ratio_moving_pixels_threshold", 0.1);
    ...
    DEBUG_CHECK(o.max_features_to_check > 0)
        << "max_features_to_check must be > 0";
...
CeresBackendInterface::Ptr makeBackend(const ros::NodeHandle& pnh,
    ...
    /// maximum number of iterations.
    backend_options.max_iteration_time = static_cast<double>(
        vk::param<double>(pnh, "ceres_max_iteration_time", -1.0));
    /// Should marginalization term be discarded after loop? Experimental
    backend_options.remove_marginalization_term_after_correction_ =
        vk::param<bool>(pnh, "ceres_remove_marginalization_term_after_loop",
                        false);
    backend_options.recalculate_imu_terms_after_loop =
        vk::param<bool>(pnh, "ceres_recalculate_imu_terms_after_loop", false);
    backend_options.remove_fixation_min_num_fixed_landmarks_ =
        static_cast<size_t>(vk::param<int>(
            pnh, "ceres_remove_fixation_min_num_fixed_points", 10u));
    ...
    ba_interface_options.extrinsics_pos_sigma_meter =
        vk::param<double>(pnh, "backend_extrinsics_rot_sigma_deg", 5.0) /
        180.0 * M_PI;

    CeresBackendInterface::Ptr ba_interface =
        std::make_shared<CeresBackendInterface>(
            ba_interface_options, backend_options,
            loadMotionDetectorOptions(pnh), camera_bundle);
