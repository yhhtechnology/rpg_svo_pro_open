...
BackendInterface::Ptr makeBackend(const ros::NodeHandle& pnh) {
    ...
    backend_options.verbose = vk::param<bool>(pnh, "ba_verbose", false);
    ...
    backend_options.trace_dir = vk::param<std::string>(
    ...
    ba_interface_options.min_num_obs = vk::param<int>(pnh, "ba_min_num_obs", 2);
    ba_interface_options.n_frames_in_init_ba =
        vk::param<int>(pnh, "ba_n_frames_in_init_ba", 8);

    BackendInterface::Ptr ba_interface = std::make_shared<BackendInterface>(
