...
void publishStringsAtPositions(ros::Publisher& pub,
...
Visualizer::Visualizer(const std::string& trace_dir,
    ...
    : pnh_(nh_private),
      ...
      viz_caption_str_(
          vk::param<bool>(pnh_, "publish_image_caption_str", false)),
      pc_(new PointCloud),
    ...
    pub_pose_graph_map_ =
        pnh_.advertise<PointCloud>("pose_graph_pointcloud", 10);
#endif
    ...
    pub_global_map_keypoints_vis_ = pnh_.advertise<visualization_msgs::Marker>(
        "global_map_keypoints_vis", 10);
    pub_visible_fixed_landmarks_ = pnh_.advertise<visualization_msgs::Marker>(
        "visible_fixed_landmarks", 10);
    pub_global_map_matched_points_ =
        pnh_.advertise<visualization_msgs::Marker>("global_map_matched", 10);
    ...
    pub_global_map_point_ids_ = pnh_.advertise<visualization_msgs::MarkerArray>(
...
void Visualizer::publishSvoInfo(const svo::FrameHandlerBase* const svo,
    ...
    if (pub_info_.getNumSubscribers() == 0) return;
...
void Visualizer::publishCameraPoses(const FrameBundlePtr& frame_bundle,
    ...
    vk::output_helper::publishTfTransform(
        ...
        ros::Time().fromNSec(timestamp_nanoseconds), "cam_pos", kWorldFrame,
        br_);

    for (size_t i = 0; i < frame_bundle->size(); ++i) {
        if (pub_cam_poses_.at(i).getNumSubscribers() == 0) return;
...
void Visualizer::publishBundleFeatureTracks(const FrameBundlePtr frames_ref,
                                            ...
                                            int64_t timestamp) {
    if (trace_id_ % img_pub_nth_ != 0 || !frames_ref) return;
    ...
    for (size_t i = 0; i < frames_ref->size(); ++i) {
        ...
        publishFeatureTracks(frames_ref->at(i)->px_vec_,
                             frames_cur->at(i)->px_vec_, matches_ref_cur,
                             frames_cur->at(i)->img_pyr_, img_pub_level_,
                             timestamp, i);
    }
}
...
void Visualizer::publishFeatureTracks(
    ...
    if (pub_images_.at(frame_index).getNumSubscribers() == 0) return;
...
void Visualizer::publishImages(const std::vector<cv::Mat>& images,
                               const int64_t timestamp_nanoseconds) {
    if (trace_id_ % img_pub_nth_ != 0) return;
    VLOG(100) << "Publish images.";

    for (size_t i = 0; i < images.size(); ++i) {
        if (pub_images_.at(i).getNumSubscribers() == 0) continue;

        // Downsample image for publishing.
        ImgPyr img_pyr;
        if (images[i].type() == CV_8UC1) {
            frame_utils::createImgPyramid(images[i], img_pub_level_ + 1,
                                          img_pyr);
        } else if (images[i].type() == CV_8UC3) {
            cv::Mat gray_image;
            cv::cvtColor(images[i], gray_image, cv::COLOR_BGR2GRAY);
            frame_utils::createImgPyramid(gray_image, img_pub_level_ + 1,
                                          img_pyr);
        } else {
...
void Visualizer::visualizeQuadrocopter(const Transformation& T_frame_world,
    ...
    if (pub_frames_.getNumSubscribers() > 0) {
        vk::output_helper::publishQuadrocopterMarkers(
            pub_frames_, "cam_pos", "cams", ros::Time().fromNSec(timestamp), 1,
...
void Visualizer::publishSeedsAsPointcloud(const Frame& frame,
    ...
    if (pub_pc_.getNumSubscribers() == 0) return;
    ...
    for (size_t i = 0; i < frame.num_features_; ++i) {
        if ((only_converged_seeds &&
            ...
            p.y = xyz.y();
            p.z = xyz.z();
            p.intensity = frame.img().at<uint8_t>(frame.px_vec_(1, i),
                                                  frame.px_vec_(0, i));
            pc_->push_back(p);
        }
    }
...
void Visualizer::publishMapRegion(const std::vector<FramePtr>& frames) {
    ...
    if (pub_pc_.getNumSubscribers() > 0) {
        ...
        for (const FramePtr& frame : frames) {
            for (size_t i = 0; i < frame->num_features_; ++i) {
                if (frame->landmark_vec_[i] == nullptr) continue;

                Point& point = *frame->landmark_vec_[i];
                if (point.last_published_ts_ == ts) continue;
...
void Visualizer::publishKeyframeWithPoints(const FramePtr& frame,
    ...
    for (size_t i = 0; i < frame->num_features_; ++i) {
        if (frame->landmark_vec_[i] != nullptr) {
            ...
            if (point->last_published_ts_ == timestamp) continue;
...
void Visualizer::publishActiveKeyframes(
    const std::vector<FramePtr>& active_kfs) {
...
void Visualizer::visualizeCoordinateFrames(const Transformation& T_world_cam) {
    ...
    // camera frame
    vk::output_helper::publishFrameMarker(
        pub_markers_, T_world_cam.getRotationMatrix(),
        T_world_cam.getPosition(), "cam", ros::Time::now(), 0, 0, 0.2);
    ...
    if (pub_pose_graph_map_.getNumSubscribers() > 0 &&
        ...
        if (redo_pointcloud) {
            ...
            const KeyFramePtr kf =
                kf_list[kf_list.size() - ignored_past_frames];
            PointType pt;
            pt.intensity = 60;
            std::vector<cv::Point3f> pw_vec;
            kf->getLandmarksInWorld(&pw_vec);
...
void Visualizer::writeCaptionStr(cv::Mat img_rgb) {
    if (viz_caption_str_) {
        cv::putText(img_rgb, img_caption_, cv::Point(20, 20),
                    cv::FONT_HERSHEY_COMPLEX_SMALL, 1, cv::Scalar(0, 0, 250), 1,
                    cv::LINE_AA);
    }
}

#ifdef SVO_GLOBAL_MAP
void Visualizer::visualizeGlobalMap(const GlobalMap& map,
                                    const std::string ns,
                                    const Eigen::Vector3f& color,
                                    const double scale) {
    ...
    if (pub_global_map_kfs_opt_.getNumSubscribers() > 0) {
        ...
        publishPositionVecAsPC(pub_global_map_kfs_opt_,
                               {kf_pos, kf_pos_with_priors}, {60, 120});
    }
    if (pub_global_map_query_kfs_.getNumSubscribers() > 0) {
        ...
        publishPositionVecAsPC(pub_global_map_query_kfs_,
                               {kf_pos, last_queried_kf_pos}, {60, 120});
    }
    ...
    if (pub_global_map_point_ids_.getNumSubscribers() > 0) {
        ...
        publishStringsAtPositions(pub_global_map_point_ids_, id_strs,
                                  positions);
    }
...
void Visualizer::visualizeFixedLandmarks(const FramePtr& frame) {
    ...
    publishLineList(pub_visible_fixed_landmarks_, vis,
                    "visible_fixed_landmarks",
