...
DEFINE_bool(stamp_estimate,
            true,
            "save stamped estimate, otherwise the id is "
            "used");
DEFINE_bool(benchmark_auto_restart,
            true,
...
class BenchmarkNode : public SvoInterface {
 ...
 public:
    BenchmarkNode(const PipelineType& type,
    ...
    void traceIdTimePose(const Transformation& pose,
                         const size_t id,
                         const double time_sec);

    void traceStatus(const size_t id, const double time_sec);

    void traceSpeedBias(const svo::ViNodeState& state,
                        const size_t id,
                        const double time_sec);
    ...
    // Blender dataset: since we can ininitialize with GT, the error can
    // directly
    // be comptued
...
BenchmarkNode::BenchmarkNode(const PipelineType& type,
    ...
    if (svo_->getBundleAdjuster()) {
        safeCreateTrace(kSaveVelBias + save_suf + "." + kExt,
                        &trace_speed_bias_);
...
void BenchmarkNode::traceIdTimePose(const Transformation& pose,
                                    const size_t id,
                                    const double time_sec) {
...
void BenchmarkNode::runBenchmark(const std::string& dataset_dir) {
    ...
    while (img_fs.good() && !img_fs.eof() && ros::ok()) {
        ...
        if (svo_->stage() == Stage::kTracking || svo_->getBundleAdjuster()) {
            ...
            if (!trace_only_kf_ || nframe.isKeyframe()) {
                ...
                if (svo_->getBundleAdjuster()) {
                    const AbstractBundleAdjustmentPtr& ba_ptr =
                        svo_->getBundleAdjuster();
                    if (ba_ptr->getType() == BundleAdjustmentType::kCeres) {
                        while (ba_ptr->lastOptimizedBundleId() <
                               nframe.getBundleId()) {
                            continue;
                        }
                    } else {
                        LOG(FATAL) << "Benchmarking not implemented yet for "
                                      "this backend "
                                      "type.";
                    }
                ...
                if (svo_->isBackendScaleInitialised() &&
                    ...
                    (!svo_->hasGlobalMap() ||
                     svo_->doesGlobalMapHaveInitialBA())) {
    ...
    if (svo_->lc_) {
        ...
        numquery_path = svo_->options_.trace_dir + "/" + "num_queries" +
                        suffix + "." + kExt;
        if (svo_->lc_->pgo_) {
...
void BenchmarkNode::traceDepthError(const FramePtr& frame,
    ...
    for (size_t i = 0; i < frame->numFeatures(); ++i) {
        if (frame->landmark_vec_[i]) {
            ...
            const double depth_true = depthmap.at<float>(
                (int)frame->px_vec_(1, i), (int)frame->px_vec_(0, i));
...
void BenchmarkNode::runBlenderBenchmark(const std::string& dataset_dir,
    ...
    trace_trans_error_.open(svo_->options_.trace_dir +
    ...
    if (trace_trans_error_.fail() || trace_rot_error_.fail() ||
        ...
        throw std::runtime_error(
            "Could not create tracefile. Does folder exist?");
    ...
    if (dataset.getNextFrame(n_pyr_levels, ref_frame, &depthmap)) {
        ...
        AbstractDetector::Ptr detector = feature_detection_utils::makeDetector(
            fast_options, ref_frame->cam());
    ...
    frame_count_ = 1;
    FramePtr cur_frame;
    while (dataset.getNextFrame(n_pyr_levels, cur_frame, nullptr) &&
           ros::ok()) {
        ...
        if (svo_->stage() != Stage::kTracking) {
            SVO_ERROR_STREAM(
                "SVO failed before entire dataset could be processed.");
            break;
        }

        if (frame_count_ == 50)
            svo_->reprojectors_.at(0)->options_.remove_unconstrained_points =
                true;

        // Compute pose error and trace to file
        if (svo_->getLastFrames() && svo_->isBackendScaleInitialised()) {
            Transformation T_f_gt(svo_->getLastFrames()->at(0)->T_f_w_ *
                                  T_w_gt);
...
void BenchmarkNode::runArrayBenchmark(const std::string& dataset_dir) {
    ...
    while (img_fs.good() && !img_fs.eof() && ros::ok()) {
        ...
        if (stamp < first_frame_id) continue;
        std::string();
        ...
        if (img1.empty() || img2.empty() || img3.empty() || img4.empty()) {
            SVO_ERROR_STREAM("Reading image " << dataset_dir << "/data/"
                                              << imgname1 << " failed.");
            return;
        }
