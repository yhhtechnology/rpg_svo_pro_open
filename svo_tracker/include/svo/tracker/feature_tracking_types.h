...
struct FeatureTrackerOptions {
    /// We do the Lucas Kanade tracking in a pyramidal way. max_level specifies
    /// the
    /// coarsest pyramidal level to optimize. For an image resolution of
    /// (640x480)
    /// we set this variable to 4 if you have an image with double the
    /// resolution,
...
class FeatureRef {
 public:
    ...
    FeatureRef(const FrameBundlePtr& frame_bundle,
               size_t frame_index,
               size_t feature_index);

    inline const FrameBundlePtr getFrameBundle() const { return frame_bundle_; }

    inline size_t getFrameIndex() const { return frame_index_; }

    inline size_t getFeatureIndex() const { return feature_index_; }
...
class FeatureTrack {
 public:
    ...
    inline int getTrackId() const { return track_id_; }

    inline const FeatureRefList& getFeatureTrack() const {
        return feature_track_;
    }

    inline size_t size() const { return feature_track_.size(); }

    inline bool empty() const { return feature_track_.empty(); }
    ...
    inline void pushBack(const FrameBundlePtr& frame_bundle,
                         const size_t frame_index,
                         const size_t feature_index) {
        feature_track_.emplace_back(
            FeatureRef(frame_bundle, frame_index, feature_index));
    }
...
typedef std::vector<FeatureTrack, Eigen::aligned_allocator<FeatureTrack> >
    FeatureTracks;
