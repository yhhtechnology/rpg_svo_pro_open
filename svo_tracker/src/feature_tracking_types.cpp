...
// -----------------------------------------------------------------------------
FeatureRef::FeatureRef(const FrameBundlePtr& frame_bundle,
                       size_t frame_index,
...
const Eigen::Block<Keypoints, 2, 1> FeatureRef::getPx() const {
    return frame_bundle_->at(frame_index_)
        ->px_vec_.block<2, 1>(0, feature_index_);
}

const Eigen::Block<Bearings, 3, 1> FeatureRef::getBearing() const {
    return frame_bundle_->at(frame_index_)
        ->f_vec_.block<3, 1>(0, feature_index_);
}
...
// -----------------------------------------------------------------------------
FeatureTrack::FeatureTrack(int track_id) : track_id_(track_id) {
