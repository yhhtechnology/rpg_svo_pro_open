...
FeatureTracker::FeatureTracker(const FeatureTrackerOptions& options,
    ...
    for (size_t i = 0; i < cams->getNumCameras(); ++i) {
        detectors_.push_back(feature_detection_utils::makeDetector(
...
size_t FeatureTracker::trackFrameBundle(const FrameBundlePtr& nframe_kp1) {
    ...
    // TODO(cfo): Datastructure could be simplified. We need only the first
    // frame in track.
    for (size_t frame_index = 0; frame_index < bundle_size_; ++frame_index) {
        ...
        for (size_t track_index = 0; track_index < tracks.size();
            ...
            const FeatureRef& ref_observation =
                (options_.klt_template_is_first_observation) ? track.front()
                                                             : track.back();

            const ImgPyr& ref_pyr = ref_observation.getFrame()->img_pyr_;
            const ImgPyr& cur_pyr = cur_frame->img_pyr_;

            // TODO(cfo): make work for feature coordinates with subpixel
            // reference patch!
            // Currently not a problem because feature detector returns integer
            // pos.
            Eigen::Vector2i ref_px_level_0 =
                ref_observation.getPx().cast<int>();
            Keypoint cur_px_level_0 = track.back().getPx();
            bool success = feature_alignment::alignPyr2D(
                ref_pyr, cur_pyr, options_.klt_max_level,
                options_.klt_min_level, options_.klt_patch_sizes,
            ...
            if (success) {
                ...
                new_scores(new_keypoints_counter) =
                    ref_observation.getFrame()
                        ->score_vec_[ref_observation.getFeatureIndex()];
        ...
        //    svo::common::container_helpers::eraseIndicesFromVector(remove_indices,
        //    &tracks);
        auto new_tracks =
            svo::common::container_helpers::eraseIndicesFromVector_DEPRECATED(
                tracks, remove_indices);
        tracks = new_tracks;
        ...
        VLOG(4) << "Tracker: Frame-" << frame_index
                << " - tracked = " << new_keypoints_counter;
    }
...
size_t FeatureTracker::initializeNewTracks(const FrameBundlePtr& nframe) {
    ...
    for (size_t frame_index = 0; frame_index < bundle_size_; ++frame_index) {
        ...
        detectors_.at(frame_index)
            ->detect(frame->img_pyr_, frame->getMask(), max_n_features, new_px,
        ...
        for (size_t feature_index = n_old; feature_index < frame->numFeatures();
             ++feature_index) {
