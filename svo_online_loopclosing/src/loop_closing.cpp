...
void LoopClosing::runPROnLatestKeyframe(const size_t n_ignored_latest,
    ...
    VLOG(40) << ">>>> Frame ID " << current_frame_ID
             << " added to PR. Timestamp " << cur_kf->timestamp_sec_abs_;
    ...
    timer_total.start();
    timer_each.start();
    extractBoWFeaturesFromImage(cur_kf->keyframe_image_,
                                &cur_kf->bow_keypoints_,
    ...
    if (suspend_lc_after_correction_) {
        ...
        if (suspended_frames_counter_ <
            static_cast<int>(options_.alpha * 20 + 5)) {
    ...
    for (size_t query_kf_idx = 10;
        ...
        VLOG(40)
            << "- Will check against frame (that passes the distance check) "
            << past_NframeID;
        num_queries_.back()++;
        double score = 0.0;
        ...
        featureMatchingFast(cur_kf->bow_features_, lc_kf_i->bow_features_,
                            ...
                            lc_kf_i->svo_features_mat_, cur_kf->bow_node_ids_,
                            lc_kf_i->bow_node_ids_, cur_kf->svo_node_ids_,
                            lc_kf_i->svo_node_ids_, options_.orb_dist_thresh,
                            &match_indices);
        ...
        geometricVerification(
            cur_kf->mixed_keypoints_, lc_kf_i->mixed_keypoints_,
            cur_kf->svo_bearingvectors_, lc_kf_i->svo_bearingvectors_,
            match_indices, K_, D_, cur_kf->num_bow_features_,
            lc_kf_i->num_bow_features_, options_.use_opengv, &inliers,
            &keypoints_matched1, &keypoints_matched2, &keypoints_matched1_udist,
                ...
                if (match_indices(0, l) > cur_kf->num_bow_features_ &&
                    ...
                    lc_to_cur_inlier_corresp_3d.back() =
                        std::pair<size_t, size_t>(
                            size_t(match_indices(1, l) -
                                   lc_kf_i->num_bow_features_),
                            size_t(match_indices(0, l) -
        ...
        undistortAndNormalise(
            cur_kf->mixed_keypoints_, lc_kf_i->mixed_keypoints_, K_, D_,
        ...
        if (scale_retrieval_approach_ == LCScaleRetMethod::kCommonLandmarks) {
            ...
            scale = getScaleCL(cur_kf->svo_keypointsvector_,
                               lc_kf_i->svo_keypointsvector_, cur_pw_vec,
                               lc_pw_vec, svo_trackIDs_current,
                               svo_trackIDs_past, cur_cam_pose_mat, K_, T_rel);
        } else if (scale_retrieval_approach_ ==
                   LCScaleRetMethod::kMixedKeyPoints) {
            // mixed keypoints approach
            VLOG(40) << "Mixed Keypoints Approach";
            cv::Mat cur_cam_pose_mat;
            cur_kf->getTwcCvMat(&cur_cam_pose_mat);
            scale = getScaleMK(keypoints_matched1_udist,
                               keypoints_matched2_udist, cur_pw_vec, lc_pw_vec,
                               match_indices, inliers, cur_cam_pose_mat, K_,
                               T_rel, cur_kf->num_bow_features_);
        } else {
        ...
        if (options_.enable_image_logging) {
            ...
            std::stringstream prev_img_path, window_path, window_path2,
                window_path3, out_img_cf_path, out_img_lc_path;
            ...
            //--------------------------------------------------------------------------------------------//
            cv::drawMatches(cur_kf->keyframe_image_, keypoints1, prev_img,
                            keypoints2, matches12, out_img);
            ...
            cv::drawMatches(cur_kf->keyframe_image_, keypoints1_all_svo,
                            prev_img, keypoints2_all_svo, matches12_all_svo,
                            out_img_all_svo);
        ...
        bool suc = calculateTransformationInWorldFrame(
            lc_pw_vec, cur_pw_vec, lc_to_cur_inlier_corresp_3d,
            current_frame_ID, past_NframeID, &w_T_new_old, &inlier_3d_indices);
        ...
        constructLoopViz(*cur_kf, *lc_kf_i,
                         &(loop_correction_viz_info_.back()));

        // save loop information for other modules (visualization, backend)
        {
            std::lock_guard<std::mutex> lock(lc_info_lock_);
            if (recovery_after_loss_ ||
                w_T_new_old.getPosition().norm() >
                    options_.force_correction_dist_thresh_meter) {
                lc_correction_info_.emplace_back(
                    lc_kf_i->NframeID_, cur_kf->NframeID_, w_T_new_old);
...
void LoopClosing::addFrameToPR(const svo::FrameBundlePtr& last_frames_) {
    ...
    if (svo_keyframe_count_ == 1) {
        ...
        if (completed_flags_.back() == true) {
            ...
            add_this_frame = commonLandMarkCheck(last_added_frame_trackIDs_,
                                                 cur_kf->svo_trackIDsvector_,
                                                 options_.alpha);
            ...
            VLOG(40)
                << "########## WARNING: Last thread still running ###########";
        }
    }
}

void LoopClosing::svoFrameToKeyframe(const FramePtr& frame,
                                     KeyFrame* kf) const {
    CHECK_NOTNULL(kf);
    kf->clearSVOFeatureInfo();
    kf->frame_id_ = frame->id();
    this->extractAndConvert(
        frame, &kf->timestamp_sec_abs_, &kf->T_w_c_, &kf->svo_keypointsvector_,
...
void LoopClosing::extractAndConvert(
    ...
    double* current_frame_timestamp_sec,
    Transformation* Twc,
    ...
    if (res != -1) {
        ...
        if (global_map_type_ == GlobalMapType::kBuiltInPoseGraph) {
            ...
            if (kf_idx > 0) {
                ...
                /* If this frame has a loop closure then add a loop constraint
                 * and
                 * optimize the pose graph in a separate
                 * thread.*/
                auto search =
                    cur_kf_to_lc_kf_bundle_id_map_.find(found_kf->NframeID_);
                if (search != cur_kf_to_lc_kf_bundle_id_map_.end()) {
                    ...
                    Transformation t_be_lc =
                        kf_list_[static_cast<size_t>(lc_kf_idx)]
                            ->T_w_c_.inverse() *
                        corrected_T_w_c;
                    if (!pgo_->has_updated_result_) {
                        std::thread pgo_thread(
                            &Pgo::addLoopConstraintToPgoProblem, pgo_, t_be_lc,
                            lc_frame_id, current_frame_id);
                        pgo_thread.detach();
...
void LoopClosing::updateSVOPointsDescriptors(
    const size_t kf_index, const bool replace_mixed_features) {
    ...
    /// Mix the keypoints from bow and svo for second approach to scale
    /// retrieval
...
void LoopClosing::updateMapPointsUsingDepth(
    svo::Frame& frame,
    std::vector<cv::Point3f>& svo_landmarksvector,
    const Transformation& pose,
    const BearingVecs& svo_bearingvector,
    ...
    for (size_t i = 0; i < svo_landmarksvector.size(); i++) {
        if (svo_featuretypevector.at(i) == FeatureType::kMapPoint) {
            // Get the new point in world coordinates based on bearing vector
            // and
            // depth
            double x, y, z;
            double multiplier =
                svo_depthvector.at(i) / svo_bearingvector.at(i)(2);
            ...
            svo_landmarksvector.at(i) = cv::Point3f((float)(point_world(0, 0)),
                                                    (float)(point_world(1, 0)),
                                                    (float)(point_world(2, 0)));
            frame.landmark_vec_[svo_originalindicesvec.at(i)]->pos_ =
                point_world;
        }
    }
}
...
bool LoopClosing::calculateTransformationInWorldFrame(
    ...
    const int& lc_frame_id,
    Transformation* w_T_new_old,
    std::vector<int>* inlier_indices) {
    // (consider all current 3D points, there might be old ones as well)
    if (point_correspondences.size() <
        static_cast<size_t>(options_.min_num_3d)) {
    ...
    for (const IdCorrespondence& correspondence : point_correspondences) {
        ...
        map_alignment_se3_->addCorrespondencePair(landmark_cf_pos,
                                                  landmark_lc_pos);
    }
