...
namespace svo {
void readFeatureAndLandmarkData(const std::string& pathToFolder,
                                const std::string& frameID1,
                                const std::string& frameID2,
                                std::vector<cv::Point2f>* keypoints1,
                                std::vector<cv::Point2f>* keypoints2,
                                std::vector<cv::Point3f>* landmarks1,
                                std::vector<cv::Point3f>* landmarks2,
    ...
    if (trackIDFile1) {
        int tvalue1;
        while (trackIDFile1 >> tvalue1) trackIDs1->push_back(tvalue1);
    }
    trackIDFile1.close();
    ...
    if (trackIDFile2) {
        int tvalue2;
        while (trackIDFile2 >> tvalue2) trackIDs2->push_back(tvalue2);
    }
    trackIDFile2.close();
...
cv::Mat readCampose(const string& pathToFolder,
                    const string& frameID,
                    int rows,
                    int cols) {
    double m;
    cv::Mat campose =
        cv::Mat(4, 4, CV_64F, double(0));  // Matrix to store values
