...
void extractFeaturesFromFolder(const std::string& path_to_folder,
    ...
    if ((dir = opendir(path_to_folder.c_str())) != NULL) {
        ...
        int count =
            scandir(path_to_folder.c_str(), &ent, *imgFilter, alphasort);
...
OrbVocabulary createVoc(const std::vector<std::vector<cv::Mat> >& features,
                        ...
                        const std::string& voc_name,
                        const int k,
                        const int L,
                        const WeightingType& weight,
...
void createBOW(const std::vector<cv::Mat>& feature,
...
double compareBOWs(const BowVector& v1,
                   const BowVector& v2,
                   const OrbVocabulary& voc) {
...
void getNodeID(const std::vector<cv::Mat>& feature,
               const OrbVocabulary& voc,
               const int levelup,
    ...
    for (unsigned int i = 0; i < feature.size(); i++) {
        node_ids->push_back(
            voc.getParentNode(voc.transform(feature[i]), levelup));
    }
}
...
void extractFeaturesFromSVOKeypoints(
    ...
    BearingVecs* svo_bearingvectors,
    std::vector<double>* svo_depthvector,
    ...
    for (unsigned int i = 0; i < keypoints.size(); i++) {
        ...
        int pos = std::find(old_svo_keypoints.begin() + i,
                            old_svo_keypoints.end(), svo_keypoints->back()) -
