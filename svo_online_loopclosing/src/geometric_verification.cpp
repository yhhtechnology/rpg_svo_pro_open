...
void featureMatchingFast(const std::vector<cv::Mat>& feature_vec1,
    ...
    for (unsigned int i = 0; i < feature_vec1_size; i++) {
        ...
        while ((iter = std::find(iter, node_id2.end(), node_id1[i])) !=
            ...
            double hamming_dist = cv::norm(feature_vec1[i], feature_vec2[index],
                                           cv::NORM_HAMMING);
    ...
    for (unsigned int i = 0; i < feature_vec4_size; i++) {
        ...
        while ((iter = std::find(iter, node_id3.end(), node_id4[i])) !=
            ...
            double hamming_dist_svo = cv::norm(
                feature_vec4[i], feature_vec3[index], cv::NORM_HAMMING);
            if (hamming_dist_svo < hamming_dist_min_svo &&
                hamming_dist_svo < dist_th) {
...
void geometricVerification(const vector<cv::Point2f>& keypoints1,
    ...
    vector<cv::Point2f>* keypoints_matched1_norm_udist =
        new vector<cv::Point2f>(
    ...
    cv::undistortPoints(*keypoints_matched1, *keypoints_matched1_norm_udist, K,
                        D, cv::noArray(), cv::noArray());
    cv::undistortPoints(*keypoints_matched2, *keypoints_matched2_norm_udist, K,
                        D, cv::noArray(), cv::noArray());
    ...
    if (use_open_gv) {
        ...
        for (int i = 0; i < match_indices.cols(); i++) {
            ...
            if (match_indices(0, i) < bow_featurevec1_size &&
                ...
                Eigen::Vector3d bv1 = Eigen::Vector3d(
                    keypoints_matched1_norm_udist->at(i).x,
                    keypoints_matched1_norm_udist->at(i).y, 1.0);
                bv1 = bv1 / bv1.norm();
                mixed_vec1.push_back(bv1);
                Eigen::Vector3d bv2 = Eigen::Vector3d(
                    keypoints_matched2_norm_udist->at(i).x,
        ...
        for (int i = 0; i < match_indices.cols(); i++) {
            if (match_indices(0, i) >= bow_featurevec1_size &&
                match_indices(1, i) >= bow_featurevec2_size) {
                mixed_vec1.push_back(svo_bearingvector1[match_indices(0, i) -
                                                        bow_featurevec1_size]);
                mixed_vec2.push_back(svo_bearingvector2[match_indices(1, i) -
                                                        bow_featurevec2_size]);
            }
        }
        ...
        typedef opengv::sac_problems::relative_pose::
            CentralRelativePoseSacProblem CentralRelative;
        ...
        *inliers = 0;
        ...
        //     Also check if the returned essential matrix is feasible eg: if it
        //     is
...
float getScaleCL(const std::vector<cv::Point2f>& keypoints1,
    ...
    for (unsigned int i = 0; i < landmarks1.size(); i++) {
        ...
        if (minindex > -1) {
            ...
            svopnt3DH =
                cam_pose_current.inv() * svo3DpntHW;  // in camera 1 frame
            cv::Point3f svopnt3D;
            svopnt3D.x =
                svopnt3DH.at<double>(0, 0) / svopnt3DH.at<double>(3, 0);
            svopnt3D.y =
                svopnt3DH.at<double>(1, 0) / svopnt3DH.at<double>(3, 0);
            svopnt3D.z =
                svopnt3DH.at<double>(2, 0) / svopnt3DH.at<double>(3, 0);
...
float getScaleMK(const std::vector<cv::Point2f>& keypoints_matched1,
    ...
    proj_mat << R, t;
    cv::Mat P2 =
        (cv::Mat_<double>(3, 4) << proj_mat(0, 0), proj_mat(0, 1),
    ...
    for (int i = 0; i < match_indices.cols(); i++) {
        ...
        if (inliers.at<bool>(i, 0) == 1 &&
            match_indices(0, i) > num_bow_features) {
