...
struct LoopClosureOptions {
    ...
    /* Path to the folder where images will be locked. Make sure folder is
     * already
     * created */
...
class LoopClosing {
 public:
    ...
    /* Update everything in the pose database with the result of latest pose
     * graph
     * optimization */
    ...
    inline bool proximityCheck(std::shared_ptr<KeyFrame> keyframe1,
        ...
        return (keyframe1->T_w_c_.getPosition() -
                keyframe2->T_w_c_.getPosition())
                   .norm() <= prox_dist_thresh_;
    }

    /*
     * This function updates the map points in the database to make sure they
     * are
    ...
    /* Flag to relax thresholds on loop closing when we loose feature tracking
     * for
     * long */
    ...
    inline void constructLoopViz(const KeyFrame& cur_kf,
                                 const KeyFrame& lc_kf,
                                 LoopVizInfo* viz) {
        viz->block<1, 3>(0, 0) =
