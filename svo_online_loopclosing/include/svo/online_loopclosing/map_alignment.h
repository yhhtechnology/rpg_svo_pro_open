...
class MapAlignmentSE3 {
 public:
     ...
     */
    void addCorrespondencePair(const Position& old_pos,
     ...
     * @param T_old_new Transformation such that T_old_new*p_old = p_new
     */
    bool getTransformRansac(const int min_num_3d,
     ...
     * @param T_old_new Transformation such that T_old_new*p_old = p_new
     */
    Transformation getTransformationCombined(Transformation& T_w_lc,
                                             Transformation& T_w_cf);

    /**
     * @brief Jointly optimize the epipolar and reprojection error to get
     * relative
     * pose
     */
    bool solveJointOptimisation(
    ...
    static void getClosest4DOFTransformInPlace(Transformation& T);
