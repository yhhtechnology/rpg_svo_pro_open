...
class KeyFrame {
 public:
    ...
    inline void transformMap(const Transformation& T) {
        ...
        for (size_t i = 0; i < svo_landmarksvector_cam_.size(); i++) {
            Eigen::Vector3d pos = Eigen::Vector3d(
                svo_landmarksvector_cam_[i].x, svo_landmarksvector_cam_[i].y,
                svo_landmarksvector_cam_[i].z);
            pos = T.transform(pos);
            svo_landmarksvector_cam_[i] =
    ...
    inline void getTwcCvMat(cv::Mat* Twc_cvmat) {
        ...
        (*Twc_cvmat) =
            (cv::Mat_<double>(4, 4) << Twc_mat(0, 0), Twc_mat(0, 1),
    ...
    inline void getLandmarksInWorld(std::vector<cv::Point3f>* pw_vec) {
        ...
        for (const cv::Point3f& pc : svo_landmarksvector_cam_) {
            Eigen::Vector3d pw =
                T_w_c_.transform(Eigen::Vector3d(pc.x, pc.y, pc.z));
    ...
    std::vector<size_t>
        svo_original_indexvec_;  // original index in frame class
