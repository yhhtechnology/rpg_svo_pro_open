...
class ReprojectionErrorSimple {
 public:
    ...
    ReprojectionErrorSimple(const Eigen::Matrix2d& sqrt_information,
    ...
    /*
     * Calculate the reprojection error in terms of the relative pose, using
     * known 3d points and 2d feature locations.
     */
    ...
    bool operator()(const T* const t_rel, const T* const q_rel, T* e) const {
        ...
        p_dash_eig << p_dash_hom_eig(0, 0) / p_dash_hom_eig(2, 0),
            p_dash_hom_eig(1, 0) / p_dash_hom_eig(2, 0);

        Eigen::Matrix<T, 2, 1> e_weighed =
            sqrt_information_.cast<T>() * (p_.cast<T>() - p_dash_eig);
    ...
    static ceres::CostFunction* Create(const Eigen::Matrix2d& sqrt_information,
                                       const Eigen::Matrix<double, 2, 1>& p,
                                       const Eigen::Matrix<double, 3, 1>& P) {
        return new ceres::AutoDiffCostFunction<ReprojectionErrorSimple, 2, 3,
                                               4>(
