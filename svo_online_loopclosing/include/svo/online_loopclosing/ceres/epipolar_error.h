...
class EpipolarError {
 public:
    EpipolarError(const double& sqrt_information,
                  const Eigen::Matrix<double, 2, 1> p_1,
                  const Eigen::Matrix<double, 2, 1> p_2)
        : p_1_(p_1), p_2_(p_2) {
    ...
    /*
     * Calculate the epipolar error in terms of the 2d feature coordinates and
     * the relative pose.
     */
    ...
    bool operator()(const T* const t_rel, const T* const q_rel, T* e) const {
        ...
        tx << T(0), -t_eig(2, 0), t_eig(1, 0), t_eig(2, 0), T(0), -t_eig(0, 0),
    ...
    static ceres::CostFunction* Create(const double& sqrt_information,
                                       const Eigen::Matrix<double, 2, 1>& p_1,
                                       const Eigen::Matrix<double, 2, 1>& p_2) {
