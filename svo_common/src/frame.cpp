...
Frame::Frame(const CameraPtr& cam,
             const cv::Mat& img,
...
void Frame::resizeFeatureStorage(size_t num) {
    if (static_cast<size_t>(px_vec_.cols()) < num) {
        ...
        SVO_ERROR_STREAM("Downsizing storage not implemented. cols = "
                         << px_vec_.cols() << " , desired = " << num
...
FeatureWrapper Frame::getFeatureWrapper(size_t index) {
    ...
    return FeatureWrapper(
        ...
        grad_vec_.col(index), score_vec_(index), level_vec_(index),
        landmark_vec_[index], seed_ref_vec_[index], track_id_vec_(index));
}
...
void Frame::setKeyPoints() {
    ...
    for (size_t i = 0; i < num_features_; ++i) {
        if (landmark_vec_[i] == nullptr ||
            type_vec_[i] == FeatureType::kOutlier)
            continue;
        ...
        if (u >= cu && v >= cv) {
            if (key_pts_[1].first == -1)
                key_pts_[1] = std::make_pair(i, landmark_vec_[i]->pos_);
                     ...
                     (px_vec_(0, key_pts_[1].first) - cu) *
                         (px_vec_(1, key_pts_[1].first) - cv))
                key_pts_[1] = std::make_pair(i, landmark_vec_[i]->pos_);
        }
        if (u >= cu && v < cv) {
            if (key_pts_[2].first == -1)
                key_pts_[2] = std::make_pair(i, landmark_vec_[i]->pos_);
                     ...
                     (px_vec_(0, key_pts_[2].first) - cu) *
                         (px_vec_(1, key_pts_[2].first) - cv))
                key_pts_[2] = std::make_pair(i, landmark_vec_[i]->pos_);
        }
        if (u < cv && v < cv) {
            if (key_pts_[3].first == -1)
                key_pts_[3] = std::make_pair(i, landmark_vec_[i]->pos_);
                     ...
                     (px_vec_(0, key_pts_[3].first) - cu) *
                         (px_vec_(1, key_pts_[3].first) - cv))
                key_pts_[3] = std::make_pair(i, landmark_vec_[i]->pos_);
        }
        if (u < cv && v >= cv) {
            if (key_pts_[4].first == -1)
                key_pts_[4] = std::make_pair(i, landmark_vec_[i]->pos_);
                     ...
                     (px_vec_(0, key_pts_[4].first) - cu) *
                         (px_vec_(1, key_pts_[4].first) - cv))
                key_pts_[4] = std::make_pair(i, landmark_vec_[i]->pos_);
        }
    }
}

bool Frame::isVisible(const Eigen::Vector3d& xyz_w, Eigen::Vector2d* px) const {
...
const cv::Mat& Frame::getMask() const { return cam_->getMask(); }

double Frame::getErrorMultiplier() const { return cam_->errorMultiplier(); }

double Frame::getAngleError(double img_err) const {
    return cam_->getAngleError(img_err);
}

void Frame::jacobian_xyz2image_imu(const svo::Camera& cam,
...
size_t FrameBundle::numFeatures() const {
    size_t n = 0;
    for (const FramePtr& f : frames_) n += f->numFeatures();
    return n;
}

size_t FrameBundle::numTrackedFeatures() const {
    size_t n = 0;
    for (const FramePtr& f : frames_) n += f->numTrackedFeatures();
    return n;
}

size_t FrameBundle::numTrackedLandmarks() const {
    size_t n = 0;
    for (const FramePtr& f : frames_) n += f->numTrackedLandmarks();
    return n;
}

size_t FrameBundle::numLandmarks() const {
    size_t n = 0;
    for (const FramePtr& f : frames_) n += f->numLandmarks();
    return n;
}

size_t FrameBundle::numLandmarksInBA() const {
    size_t n = 0;
    for (const FramePtr& f : frames_) n += f->numLandmarksInBA();
    return n;
}

size_t FrameBundle::numTrackedLandmarksInBA() const {
    size_t n = 0;
    for (const FramePtr& f : frames_) n += f->numTrackedLandmarksInBA();
    return n;
}

size_t FrameBundle::numFixedLandmarks() const {
    size_t n = 0;
    for (const FramePtr& f : frames_) n += f->numFixedLandmarks();
    return n;
}
...
bool getSceneDepth(const FramePtr& frame,
    ...
    for (size_t i = 0; i < frame->num_features_; ++i) {
        if (frame->landmark_vec_[i]) {
            depth =
                (frame->T_cam_world() * frame->landmark_vec_[i]->pos_).norm();
