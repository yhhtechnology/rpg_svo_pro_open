...
SyntheticDataset::SyntheticDataset(const std::string& dataset_dir,
                                   size_t cam_index,
...
bool SyntheticDataset::getNextFrame(size_t n_pyramid_levels,
                                    FramePtr& frame,
                                    cv::Mat* depthmap) {
    ...
    if (gt_fs_.is_open()) {
        ...
        if (gt_fs_ >> gt_id >> tx >> ty >> tz >> qx >> qy >> qz >> qw) {
            ...
            if (std::abs(q.norm() - 1.0) > 0.000001)
                LOG(WARNING)
                    << "test_utils::SyntheticDataset: Quaternion norm = "
                    << q.norm();
            q.normalize();
    ...
    if (depth_fs_.is_open()) {
        ...
        if (depth_fs_ >> depth_id >> depth_name) {
            ...
            if (depthmap) {
                std::string depth_filename(dataset_dir_ + "/data/" +
                                           depth_name);
                vk::blender_utils::loadBlenderDepthmap(
                    depth_filename.c_str(), cam_->imageWidth(),
                    cam_->imageHeight(), *depthmap);
            }
        }
    }
...
bool SyntheticDataset::skipNImages(size_t n) {
    for (size_t i = 0u; i < n; ++i) {
        FramePtr frame;
        if (!getNextFrame(1, frame, nullptr)) return false;
    }
    return true;
}
...
void SyntheticDataset::init() {
    ...
    if (!ncam_) LOG(FATAL) << "Failed to load camera.";
    ...
    if (!gt_fs_.is_open()) LOG(WARNING) << "No groundtruth trajectory found.";
    ...
    if (!depth_fs_.is_open()) LOG(WARNING) << "No depthmaps trajectory found.";
