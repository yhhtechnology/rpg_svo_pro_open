...
KeypointIdentifier::KeypointIdentifier(const FramePtr& _frame,
                                       const size_t _feature_index)
    : frame(_frame), frame_id(_frame->id_), keypoint_index_(_feature_index) {
    ;
}
...
Point::Point(const int id, const Eigen::Vector3d& pos) : id_(id), pos_(pos) {
...
void Point::addObservation(const FramePtr& frame, const size_t feature_index) {
    ...
    // TODO(cfo): maybe we should use a std::unordered_map to store the
    // observations.
...
void Point::initNormal() {
    ...
    if (const FramePtr& frame = obs_.front().frame.lock()) {
        ...
        normal_information_ = Eigen::Matrix2d::
            Identity();  // DiagonalMatrix<double,3,3>(pow(20/(pos_-ftr->frame->pos()).norm(),2),
                         // 1.0, 1.0);
        normal_set_ = true;
    } else
        SVO_ERROR_STREAM(
            "could not unlock weak_ptr<frame> in normal initialization");
}

bool Point::getCloseViewObs(const Eigen::Vector3d& framepos,
                            FramePtr& ref_frame,
    ...
    for (const KeypointIdentifier& obs : obs_) {
        if (FramePtr frame = obs.frame.lock()) {
            ...
            SVO_DEBUG_STREAM(
                "could not unlock weak_ptr<Frame> in Point::getCloseViewObs"
                << ", Point-ID = " << id_ << ", Point-nObs = " << obs_.size()
                << ", Frame-ID = " << obs.frame_id << ", Feature-ID = "
                << obs.keypoint_index_ << ", Point-Type = " << type_);
            return false;
        }
    }
    if (min_cos_angle <
        0.4)  // assume that observations larger than 60° are useless
    {
        SVO_DEBUG_STREAM(
            "getCloseViewObs(): obs is from too far away: " << min_cos_angle);
        return false;
    }
    return true;
}
...
double Point::getTriangulationParallax() const {
    ...
    if (!ref_frame) {
        SVO_ERROR_STREAM(
            "getTriangualtionParallax(): Could not lock ref_frame");
        return 0.0;
    }
...
bool Point::triangulateLinear() {
    ...
    for (const KeypointIdentifier& obs : obs_) {
        if (const FramePtr frame = obs.frame.lock()) {
            ...
            f_world.col(index) =
                T_world_cam * frame->f_vec_.col(obs.keypoint_index_);
    ...
    if (index != n_obs) {
        SVO_ERROR_STREAM(
            "triangulateLinear failed, could not lock all frames.");
        return false;
    }

    // from aslam: triangulation.cc
    const Eigen::MatrixXd BiD =
        f_world * f_world.colwise().squaredNorm().asDiagonal().inverse();
    const Eigen::Matrix3d AxtAx =
        n_obs * Eigen::Matrix3d::Identity() - BiD * f_world.transpose();
    const Eigen::Vector3d Axtbx =
        p_world.rowwise().sum() -
        BiD * f_world.cwiseProduct(p_world).colwise().sum().transpose();

    Eigen::ColPivHouseholderQR<Eigen::Matrix3d> qr =
        AxtAx.colPivHouseholderQr();
...
void Point::updateHessianGradientUnitPlane(const Eigen::Ref<BearingVector>& f,
...
void Point::updateHessianGradientUnitSphere(const Eigen::Ref<BearingVector>& f,
                                            const Eigen::Vector3d& p_in_f,
                                            const Eigen::Matrix3d& R_f_w,
                                            Eigen::Matrix3d& A,
                                            Eigen::Vector3d& b,
...
void Point::optimize(const size_t n_iter, bool using_bearing_vector) {
    ...
    for (size_t i = 0; i < n_iter; i++) {
            ...
            if (const FramePtr& frame = obs.frame.lock()) {
                if (using_bearing_vector) {
                    updateHessianGradientUnitSphere(
                        frame->f_vec_.col(obs.keypoint_index_),
                        frame->T_f_w_ * pos_,
                        frame->T_f_w_.getRotation().getRotationMatrix(), A, b,
                        new_chi2);
                } else {
                ...
                SVO_ERROR_STREAM(
                    "could not unlock weak_ptr<Frame> in Point::optimize");
        }
        ...
        if ((i > 0 && new_chi2 > chi2) || (bool)std::isnan((double)dp[0])) {
            ...
            cout << "it " << i << "\t FAILURE \t new_chi2 = " << new_chi2
                 << endl;
#endif
