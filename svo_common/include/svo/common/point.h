...
struct KeypointIdentifier {
    ...
    inline bool operator==(const KeypointIdentifier& other) const {
        ...
        return frame.lock().get() == other.frame.lock().get() &&
               ...
               keypoint_index_ == other.keypoint_index_;
    }
...
class Point {
 public:
    ...
    /// Initialize point normal. The inital estimate will point towards the
    /// frame.
    void initNormal();
    ...
    /// Get parallax angle of triangulation. Useful to check if point is
    /// constrained.
    ...
    /// update Hessian and Gradient of point based on one observation, using
    /// unit plane.
    void updateHessianGradientUnitPlane(const Eigen::Ref<BearingVector>& f,
    ...
    /// update Hessian and Gradient of point based on one observation, using
    /// unit sphere.
    void updateHessianGradientUnitSphere(const Eigen::Ref<BearingVector>& f,
                                         const Eigen::Vector3d& p_in_f,
                                         const Eigen::Matrix3d& R_f_w,
                                         Eigen::Matrix3d& A,
                                         Eigen::Vector3d& b,
    ...
    inline static void jacobian_xyz2f(const Eigen::Vector3d& p_in_f,
                                      const Eigen::Matrix3d& R_f_w,
