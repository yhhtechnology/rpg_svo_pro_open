...
class Frame {
 public:
    ...
    inline size_t numFeatures() const { return num_features_; }
    ...
    inline size_t numFixedLandmarks() const {
        size_t count = 0;
        for (size_t i = 0; i < num_features_; ++i) {
            if (isValidLandmark(i) && isFixedLandmark(type_vec_[i])) ++count;
        }
        return count;
    }
    ...
    inline size_t numLandmarksInBA() const {
        return static_cast<size_t>(std::count_if(
            landmark_vec_.begin(), landmark_vec_.end(),
            [](const PointPtr& p) { return p != nullptr && p->in_ba_graph_; }));
    }
    ...
    /// Masks is same size as the image. Convention: 0 == masked, nonzero ==
    /// valid.
    ...
    inline void setNFrameIndex(size_t nframe_index) {
        nframe_index_ = nframe_index;
    }
    ...
    inline Transformation T_world_imu() const {
    ...
    inline double getTimestampSec() const {
    ...
    inline Eigen::Vector3d imuPos() const {
        return T_world_imu().getPosition();
    }
    ...
    inline static void jacobian_xyz2uv(const Eigen::Vector3d& xyz_in_f,
    ...
    inline static void jacobian_xyz2uv_imu(const Transformation& T_cam_imu,
        ...
        J_proj << 1, 0, -p_in_cam[0] / p_in_cam[2], 0, 1,
            -p_in_cam[1] / p_in_cam[2];

        J = -1.0 / p_in_cam[2] * J_proj *
            T_cam_imu.getRotation().getRotationMatrix() * G_x;
    }
    ...
    inline static void jacobian_xyz2f_imu(const Transformation& T_cam_imu,
                                          const Eigen::Vector3d& p_in_imu,
        ...
        J_normalize << y2 + z2, -xy, -zx, -xy, x2 + z2, -yz, -zx, -yz, x2 + y2;
        J_normalize *= 1 / std::pow(x2 + y2 + z2, 1.5);

        J = J_normalize * T_cam_imu.getRotationMatrix() * G_x;
    }

    /// Jacobian to image plane(also taking distortion into consideration) w.r.t
    /// IMU pose.
    static void jacobian_xyz2image_imu(const Camera& cam,
                                       const Transformation& T_cam_imu,
                                       const Eigen::Vector3d& p_in_imu,
                                       Eigen::Matrix<double, 2, 6>& J);
...
class FrameBundle {
 public:
    ...
    inline const FramePtr& at(size_t i) const { return frames_.at(i); }

    inline size_t size() const { return frames_.size(); }

    inline bool empty() const { return frames_.empty(); }

    inline BundleId getBundleId() const { return bundle_id_; }
    ...
    /// IMU measurements since the last nframe (including the last IMU
    /// measurement
...
void computeNormalizedBearingVectors(const Keypoints& px_vec,
                                     const Camera& cam,
                                     Bearings* f_vec);
