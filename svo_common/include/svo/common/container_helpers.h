...
void eraseIndicesFromVector(const std::vector<size_t>& sorted_indices_to_remove,
    ...
    auto copy_until_and_skip = [&](const size_t end_index) {
        ...
        // TODO(tcies) Potential optimization: Use move iterators. Can't use
        // memcpy
...
inline std::vector<ElementType, Allocator> eraseIndicesFromVector_DEPRECATED(
    ...
    std::sort(mutable_indices_to_delete.begin(),
              mutable_indices_to_delete.end());
    ...
    typename std::vector<ElementType, Allocator>::const_iterator
        it_block_begin = data.begin();

    for (typename std::vector<size_t>::const_iterator it =
             mutable_indices_to_delete.begin();
         it != mutable_indices_to_delete.end(); ++it) {
        typename std::vector<ElementType, Allocator>::const_iterator
            it_block_end = data.begin() + *it;
        if (it_block_begin != it_block_end) {
            std::copy(it_block_begin, it_block_end,
                      std::back_inserter(reduced_vector));
        }
    ...
    if (it_block_begin != data.end()) {
        std::copy(it_block_begin, data.end(),
                  std::back_inserter(reduced_vector));
    }
