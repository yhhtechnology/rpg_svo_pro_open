...
struct CeresBackendOptions {
    ...
    /// EXPERIMENTAL: so far it seems that we need to do this, which is
    /// equivalent
...
struct CeresBackendInterfaceOptions {
    ...
    /// (!) After how many non moving frames should we introduce zero motion
    /// prior
...
class CeresBackendInterface : public AbstractBundleAdjustment {
 public:
     ...
     *        All landmark observations of the frame bundle are added to
     * backend.
    ...
    void makePublisher(const ros::NodeHandle& nh_private,
        ...
        publisher_.reset(
            new CeresBackendPublisher(nh_private, backend_.getMap()));
 ...
 protected:
     ...
     */
    bool addStatesAndInertialMeasurementsToBackend(
    ...
    // bundle id for which the images and observations are added (after
    // frontend)
