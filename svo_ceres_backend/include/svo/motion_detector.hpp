...
struct MotionDetectorOptions {
    ...
    /// (!) ratio of allowed moving correspondences
    /// (moving_corresp/total_corresp)
    double ratio_moving_pixels_threshold = 0.1;
    int min_number_correspondences = 5;  ///< min number of correspondendces
    int max_features_to_check =
        100;              ///< max number of correspondences to check
...
class MotionDetector {
 public:
    ...
    inline bool lastCheckValid() const { return last_check_valid_; }
