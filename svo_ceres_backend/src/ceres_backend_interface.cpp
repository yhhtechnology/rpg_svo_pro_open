...
DEFINE_double(extrinsics_sigma_rel_translation,
              0.0,
              "Relative translation sigma (temporal) of camera extrinsics");
DEFINE_double(extrinsics_sigma_rel_orientation,
              0.0,
              "Relative translation sigma (temporal) of camera extrinsics");
...
CeresBackendInterface::CeresBackendInterface(
    ...
    if (optimizer_options_.max_iteration_time > 0.0) {
        backend_.setOptimizationTimeLimit(optimizer_options_.max_iteration_time,
                                          1);
    }
...
void CeresBackendInterface::loadMapFromBundleAdjustment(
    ...
    if (imu_handler_ && imu_handler_->options_.temporal_stationary_check) {
        IMUTemporalStatus imu_status = imu_handler_->checkTemporalStatus(
            new_frames->at(0)->getTimestampSec());
    ...
    {
        // Last frames might not be keyframes (are not yet updated => update
        // pose)
            ...
            if (last_frames) {
                ...
                for (FramePtr& frame : *last_frames) {
                    outlier_rejection_->removeOutliers(
                        *frame, n_deleted_edges, n_deleted_corners,
                        deleted_points, !lock_to_fixed_landmarks_);
                }
        ...
        bool success = backend_.getSpeedAndBias(last_frames->getBundleId(),
                                                speed_and_bias);
        DEBUG_CHECK(success)
...
void CeresBackendInterface::bundleAdjustment(
    ...
    if (motion_detector_) {
        ...
        if (!motion_detector_->isImageMoving(sigma)) {
            ...
            if (no_motion_counter_ >
                options_.backend_zero_motion_check_n_frames) {
        ...
        if (frame->isKeyframe()) {
            ...
            for (size_t kp_idx = 0; kp_idx < frame->numFeatures(); ++kp_idx) {
                if (frame->landmark_vec_[kp_idx] &&
                    backend_.isPointInEstimator(
                        frame->landmark_vec_[kp_idx]->id())) {
    ...
    if (options_.skip_optimization_when_tracking_bad) {
        if (frame_bundle->numLandmarksInBA() <
            options_.min_added_measurements) {
            LOG(WARNING)
                << "Too few visual measurements, skip optimization once.";
...
void CeresBackendInterface::addLandmarksAndObservationsToBackend(
    ...
    for (size_t kp_idx = 0; kp_idx < frame->numFeatures(); ++kp_idx) {
        ...
        // check if landmark was already in to backend, if yes just add
        // observation.
    ...
    VLOG(6) << "Backend has: " << backend_.numFixedLandmarks()
            << " fixed landmarks out of " << backend_.numLandmarks()
            << std::endl;
...
void CeresBackendInterface::updateBundleStateWithBackend(
    ...
    if (get_speed_bias) {
        ...
        DEBUG_CHECK(success) << "Could not get speed/bias for frame bundle "
                             << frames->getBundleId() << " from backend";
        frames->setIMUState(
            T_WS.getRotation().rotate(speed_bias.block<3, 1>(0, 0)),
...
void CeresBackendInterface::startThread() {
    CHECK(thread_ == nullptr)
        << "Tried to start thread that is already running!";
    stop_thread_ = false;
    thread_.reset(
...
void CeresBackendInterface::setPerformanceMonitor(
    ...
    while (!stop_thread_) {
        {
            ...
            wait_condition_.wait(lock, [&] {
                return ((last_added_nframe_images_ !=
            ...
            vk::Timer timer;
            {
                ...
                if (is_w_T_valid_) {
                    ...
                    backend_.transformMap(
                        ...
                        optimizer_options_
                            .remove_marginalization_term_after_correction_,
            ...
            if (optimizer_options_.marginalize) {
                // add 1 here because we perform marginalization before
                // optimization, so
            ...
            // ----------------------------------------------------------
            if (g_permon_backend_) {
                g_permon_backend_->log("n_fixed_lm",
                                       backend_.numFixedLandmarks());
            }
            timer.start();
            {
            ...
            if (g_permon_backend_) {
                g_permon_backend_->log(
                    "tot_time", timers_[last_optimized_nframe_.load()].stop());
            ...
            DEBUG_CHECK(success)
                << "Could not get latest Transformation from ceres "
                   "optimizer";
            SpeedAndBias speed_and_bias;
            success = backend_.getSpeedAndBias(last_optimized_nframe_,
                                               speed_and_bias);
            DEBUG_CHECK(success)
                << "Could not get latest speed/bias from ceres "
                   "optimizer";
...
void CeresBackendInterface::updateActiveKeyframes() {
    ...
    while (!active_keyframes_.empty()) {
        if (oldest_keyframe_in_backend ==
            active_keyframes_.front()->bundleId()) {
            return;
        }
