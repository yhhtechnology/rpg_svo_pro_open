...
bool MotionDetector::isImageMoving(double &sigma) const {
    ...
    int num_correspondences =
        findFeatureCorrespondences(features_last, features_new);
    ...
    const int n_moving_pixels_threshold = static_cast<int>(
        opt_.ratio_moving_pixels_threshold * num_correspondences);
...
int MotionDetector::findFeatureCorrespondences(Keypoints &features_last,
    ...
    for (const FramePtr &frame : *new_frames_) {
        ...
        for (int i = 0; i < num_features; ++i) {
            ...
            if (findFeatureCorrespondence(cam_index, frame->track_id_vec_[i],
                                          &px)) {
