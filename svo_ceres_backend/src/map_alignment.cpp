...
bool MapAlignmentSE3::getTransformation(Transformation& T_old_new) {
    ...
    for (int i = 0; i < num_points_; ++i) {
        /* Don't close loop if associated points are too far apart. This can
         * happen if SVO didn't assign good points in the beginning for example.
...
bool MapAlignmentSE3::getTransformRansac(Transformation& T_old_new,
                                         const int& min_num_3d,
                                         int& n_ransac_inliers,
                                         bool& relax_thresh) {
    ...
    std::shared_ptr<opengv::sac_problems::point_cloud::PointCloudSacProblem>
        relposeproblem_ptr(
            new opengv::sac_problems::point_cloud::PointCloudSacProblem(
                adapter));
...
bool MapAlignmentSE3::solveJointOptimisation(
    ...
    const Eigen::MatrixXd& rel_pose,
    const Transformation& T_w_lc,
    const size_t& num_bow_features_lc,
    const size_t& num_bow_features_cf,
    const int& min_num_3d) {
    ...
    for (const std::pair<size_t, size_t>& correspondence :
        ...
        p << (double)keypoints_cf[correspondence.second + num_bow_features_cf]
                 .x,
