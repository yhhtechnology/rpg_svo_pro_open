...
TEST(okvisTestSuite, RelativePoseError) {
    ...
    for (size_t i = 0; i < n_poses; ++i) {
        ...
        T1.setRandom(svo::test_utils::sampleUniformRealDistribution<double>(
                         deterministic),
                     svo::test_utils::sampleUniformRealDistribution<double>(
                         deterministic, 0.0, M_PI));
        T2.setRandom(svo::test_utils::sampleUniformRealDistribution<double>(
                         deterministic),
                     svo::test_utils::sampleUniformRealDistribution<double>(
                         deterministic, 0.0, M_PI));
        ...
        std::shared_ptr<svo::ceres_backend::PoseParameterBlock>
            pose_parameter_block1 =
                std::make_shared<svo::ceres_backend::PoseParameterBlock>(T1,
                                                                         ++id);
        ...
        std::shared_ptr<svo::ceres_backend::PoseParameterBlock>
            pose_parameter_block2 =
                std::make_shared<svo::ceres_backend::PoseParameterBlock>(T2,
                                                                         ++id);
        ...
        std::shared_ptr<svo::ceres_backend::RelativePoseError>
            relative_pose_error =
                std::make_shared<svo::ceres_backend::RelativePoseError>(1.0,
                                                                        1.0);
        // add it
        ceres::ResidualBlockId res_id =
            map.addResidualBlock(relative_pose_error, nullptr,
                                 pose_parameter_block1, pose_parameter_block2);
        // check Jacobian
    ...
    // check convergence. this must converge to zero, since it is not an
    // overdetermined system.
    EXPECT_TRUE(map.summary.final_cost < 1.0e-10)
        << "No convergence. this must converge to zero, since it is not an "
           "overdetermined system.";
}
