...
TEST(poseLocalParametrizationTests, PlusTest) {
    ...
    svo::ceres_backend::PoseLocalParameterization::plus(x.data(), dx.data(),
                                                        x_dist.data());
    svo::Transformation T(Eigen::Vector3d(x[0], x[1], x[2]),
...
TEST(poseLocalParametrizationTests, MinusTest) {
    ...
    svo::ceres_backend::PoseLocalParameterization::minus(
        x.data(), x_dist.data(), dx_from_minus.data());
...
TEST(poseLocalParametrizationTests, PlusJacobianTest) {
    ...
    for (size_t i = 0; i < 6; ++i) {
        ...
        jacobian_numeric.col(i) =
            (result_plus - result_minus) * 1.0 / (2.0 * delta);
    }
    EXPECT_TRUE(EIGEN_MATRIX_NEAR(jacobian_analytic, jacobian_numeric, 1e-7));
}
...
TEST(poseLocalParametrizationTests, LiftJacobianTest) {
    ...
    svo::ceres_backend::PoseLocalParameterization::plusJacobian(
        x.data(), plus_jacobian.data());
    svo::ceres_backend::PoseLocalParameterization::liftJacobian(
        x.data(), lift_jacobian.data());
    Eigen::Matrix<double, 6, 6> identity;
    identity.setIdentity();
    EXPECT_TRUE(
        EIGEN_MATRIX_EQUAL_DOUBLE(lift_jacobian * plus_jacobian, identity));
}
