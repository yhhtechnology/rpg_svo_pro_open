...
bool align1D(const cv::Mat& cur_img,
             const Eigen::Ref<GradientVector>&
                 dir,  // direction in which the patch is allowed to move
    ...
    for (int y = 0; y < kPatchSize; ++y) {
        ...
        for (int x = 0; x < kPatchSize; ++x, ++it, ++it_dv) {
            Eigen::Vector3f J;
            const float dx =
                static_cast<float>(it[1]) - static_cast<float>(it[-1]);
            const float dy = static_cast<float>(it[ref_step]) -
                             static_cast<float>(it[-ref_step]);
            J[0] = 0.5f * (dir(0) * dx + dir(1) * dy);

            // If not using the affine compensation, set the jacobian be zero.
            // In this way, all the blocks related to affine parameters will be
            // zero.
    ...
    if (h_inv) *h_inv = 1.0 / H(0, 0) * kPatchSize * kPatchSize;
    ...
    for (int iter = 0; iter < n_iter; ++iter) {
        int u_r = std::floor(u);
        int v_r = std::floor(v);
        if (u_r < kHalfPatchSize || v_r < kHalfPatchSize ||
        ...
        for (int y = 0; y < kPatchSize; ++y) {
            uint8_t* it = (uint8_t*)cur_img.data +
                          (v_r + y - kHalfPatchSize) * cur_step + u_r -
                          kHalfPatchSize;
            for (int x = 0; x < kPatchSize; ++x, ++it, ++it_ref, ++it_ref_dv) {
                float cur_intensity = wTL * it[0] + wTR * it[1] +
                                      wBL * it[cur_step] +
                                      wBR * it[cur_step + 1];
...
#if SVO_DISPLAY_ALIGN_1D
        cv::Mat res_patch_normalized, cur_patch_normalized, ref_patch_img,
            ref_patch_normalized;
        patch_utils::normalizeAndUpsamplePatch(res_patch, 8,
                                               &res_patch_normalized);
        patch_utils::normalizeAndUpsamplePatch(cur_patch, 8,
                                               &cur_patch_normalized);
        patch_utils::patchToMat(ref_patch, kPatchSize, &ref_patch_img);
        patch_utils::normalizeAndUpsamplePatch(ref_patch_img, 8,
                                               &ref_patch_normalized);
        cv::Mat concatenated;
        patch_utils::concatenatePatches(
            {ref_patch_normalized, cur_patch_normalized, res_patch_normalized},
            &concatenated);
        cv::line(concatenated,
                 cv::Point2f(concatenated.rows / 2, concatenated.rows / 2),
                 cv::Point2f(concatenated.rows / 2 + 15 * dir(0),
                             concatenated.rows / 2 + 15 * dir(1)),
                 cv::Scalar(0, 0, 255), 3);
...
bool align2D_SSE2(const cv::Mat& cur_img,
    ...
    for (int iter = 0; iter < n_iter; ++iter) {
        ...
        int wTL = static_cast<int>((1.0f - subpix_x) * (1.0f - subpix_y) *
                                   (1 << W_BITS));
        int wTR =
            static_cast<int>(subpix_x * (1.0f - subpix_y) * (1 << W_BITS));
        int wBL =
            static_cast<int>((1.0f - subpix_x) * subpix_y * (1 << W_BITS));
        int wBR = (1 << W_BITS) - wTL - wTR - wBL;

        __m128i qw0 = _mm_set1_epi32(
            wTL +
            (wTR
             << 16));  // Sets the 4 signed 32-bit integer values to [wTL, wTR].
        ...
        for (int y = 0; y < patch_size; ++y) {
            ...
            __m128i v00 =
                _mm_unpacklo_epi8(_mm_loadl_epi64((const __m128i*)(it)), z);
            __m128i v01 =
            ...
            // _mm_unpacklo_epi16: Interleaves the lower 4 signed or unsigned
            // 16-bit integers in a with the lower 4 signed or unsigned 16-bit
            // integers in b.
            // _mm_madd_epi16: Multiplies the 8 signed 16-bit integers from a by
            // the 8 signed 16-bit integers from b.
            __m128i t0 = _mm_add_epi32(
                _mm_madd_epi16(_mm_unpacklo_epi16(v00, v01), qw0),
                _mm_madd_epi16(_mm_unpacklo_epi16(v10, v11), qw1));
            __m128i t1 = _mm_add_epi32(
                _mm_madd_epi16(_mm_unpackhi_epi16(v00, v01), qw0),
                _mm_madd_epi16(_mm_unpackhi_epi16(v10, v11), qw1));

            // _mm_srai_epi32: Shifts the 4 signed 32-bit integers in A right by
            // count bits while shifting in the sign bit.
            t0 = _mm_srai_epi32(
                _mm_add_epi32(t0, qdelta),
                W_BITS);  // adding qdelta is for rounding closest int
            t1 = _mm_srai_epi32(_mm_add_epi32(t1, qdelta), W_BITS);

            // compute 8xres:
            // _mm_packs_epi32: Packs the 8 signed 32-bit integers from a and b
            // into signed 16-bit integers and saturates.
            ...
            // _mm_mulhi_epi16: Multiplies the 8 signed 16-bit integers from a
            // by the 8 signed 16-bit integers from b.
            v10 = _mm_mullo_epi16(v00, diff);  // Packs the lower 16 bits of the
                                               // 8 signed 32-bit results.
                                               // [15:0]
            v11 = _mm_mulhi_epi16(v00, diff);  // Packs the upper 16 bits of the
                                               // 8 signed 32-bit results.
                                               // [31:16]

            // _mm_unpacklo_epi16: Interleaves the lower 4 signed or unsigned
            // 16-bit integers with the lower 4 signed or unsigned 16-bit
            // integers in b.
        ...
        // compute -A^-1*b
        update_u = ((A12 * b2 - A22 * b1) * Dinv) * 2;  // * 2 to compensate
                                                        // because above, we did
                                                        // not compute the
...
bool align2D_NEON(const cv::Mat& cur_img,
    ...
    for (int iter = 0; iter < n_iter; ++iter) {
        ...
        const uint16_t wTR = static_cast<uint16_t>(
            subpix_x * (1.0f - subpix_y) * (1 << SHIFT_BITS));
        const uint16_t wBL = static_cast<uint16_t>(
            (1.0f - subpix_x) * subpix_y * (1 << SHIFT_BITS));
        const uint16_t wBR =
        ...
        for (int y = 0; y < patch_size; ++y) {
            ...
            // Eigen::Vector multiply accumulate long: vmla -> Vr[i] := Va[i] +
            // Vb[i] * Vc[i]
        ...
        update = Hinv * Jres * 2;  // * 2 to compensate because above, we did
                                   // not compute the derivative correctly
        u += update[0];
        v += update[1];
        mean_diff += update[2];
...
void alignPyr2DVec(const std::vector<cv::Mat>& img_pyr_ref,
    ...
    for (size_t i = 0; i < px_ref.size(); ++i) {
        ...
        bool res = alignPyr2D(img_pyr_ref, img_pyr_cur, max_level, min_level,
                              patch_sizes, n_iter, min_update_squared,
                              px_ref_level_0, px_cur_level_0);
        status[i] = res ? 1 : 0;
...
//------------------------------------------------------------------------------
bool alignPyr2D(const std::vector<cv::Mat>& img_pyr_ref,
                const std::vector<cv::Mat>& img_pyr_cur,
                const int max_level,
                const int min_level,
                const std::vector<int>& patch_sizes,
...
#define SVO_DESCALE(x, n) (((x) + (1 << ((n)-1))) >> (n))
    ...
    for (int level = max_level; level >= min_level; --level) {
        ...
        const Eigen::Vector2f px_ref_flt =
            px_ref_level_0.cast<float>() / scale -
            Eigen::Vector2f(halfpatch_size, halfpatch_size);
        ...
        if (px_ref[0] < 1 || px_ref[1] < 1 ||
            px_ref[0] >= width - patch_size - 1 ||
            px_ref[1] >= height - patch_size - 1) {
#if SVO_PYRAMIDAL_VERBOSE
            std::cout << "reference pixel is too close to the border"
                      << std::endl;
#endif
            continue;
        }
        ...
        for (int y = 0; y < patch_size; ++y) {
            ...
            for (int x = 0; x < patch_size;
                 ++x, ++it, ++it_patch, ++it_dx, ++it_dy) {
                *it_patch = *it;
                *it_dx = static_cast<int16_t>(it[1]) - it[-1];
                *it_dy = static_cast<int16_t>(it[step]) -
                         it[-step];  // divide by 2 missing
        ...
        for (int iter = 0; iter < n_iter; ++iter) {
            ...
            const int u_r = std::floor(u);
            const int v_r = std::floor(v);
            if (u_r < 0 || v_r < 0 || u_r >= width - patch_size ||
                v_r >= height - patch_size) {
#if SVO_PYRAMIDAL_VERBOSE
                std::cout << "current patch is too close to the border "
                          << "u,v: " << u_r << ", " << v_r << "\t w,h:" << width
            ...
            for (int y = 0; y < patch_size; ++y) {
                ...
                for (int x = 0; x < patch_size; x += 8, it += 8, it_ref += 8) {
                    ...
                    // compute difference between reference and interpolated
                    // patch,
                    // use reinterpet-cast to make signed [-255,255]
                    int16x8_t res = vsubq_s16(
                        vreinterpretq_s16_u16(v00),
                    ...
                    // Vector multiply accumulate long: vmla -> Vr[i] := Va[i] +
                    // Vb[i] * Vc[i]
                    // int32x4_t  vmlal_s16(int32x4_t a, int16x4_t b, int16x4_t
                    // c);    // VMLAL.S16 q0,d0,d0
                    int16x8_t grad =
                        vld1q_s16(ref_patch_dx + y * patch_size + x);
                    vb1 = vmlal_s16(vb1, vget_low_s16(grad), vget_low_s16(res));
                    vb1 =
                        vmlal_s16(vb1, vget_high_s16(grad), vget_high_s16(res));

                    grad = vld1q_s16(ref_patch_dy + y * patch_size + x);
                    vb2 = vmlal_s16(vb2, vget_low_s16(grad), vget_low_s16(res));
                    vb2 =
                        vmlal_s16(vb2, vget_high_s16(grad), vget_high_s16(res));
                }
            }
            ...
            for (int y = 0; y < patch_size; ++y) {
                ...
                for (int x = 0; x < patch_size;
                     ++x, ++it, ++it_ref, ++it_ref_dx, ++it_ref_dy) {
                    uint16_t cur =
                        SVO_DESCALE(wTL * it[0] + wTR * it[1] + wBL * it[step] +
                                        wBR * it[step + 1],
                                    SHIFT_BITS);
                    // float cur = wTL*it[0] + wTR*it[1] + wBL*it[step] +
                    // wBR*it[step+1];
            ...
            update = Hinv * Jres * 2.0;  // * 2 to compensate because above, we
                                         // did not compute the derivative
                                         // correctly
            u += update[0];
            v += update[1];
        ...
        px_cur_level_0 =
            Keypoint((u + halfpatch_size + px_ref_offset[0]) * scale,
                     (v + halfpatch_size + px_ref_offset[1]) * scale);

        if (!converged && !go_to_next_level)
            return false;  // no need to go to next level when we didn't
                           // converge at this one.
