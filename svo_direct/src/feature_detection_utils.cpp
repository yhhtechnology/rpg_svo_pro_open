...
//------------------------------------------------------------------------------
AbstractDetector::Ptr makeDetector(const DetectorOptions& options,
                                   const CameraPtr& cam) {
...
void fillFeatures(const Corners& corners,
    ...
    for (const Corner& c : corners) {
        if (c.score > threshold) {
            // we don't need to check for grid occupancy since we already do
            // that during
            // feature extraction.
            if (!mask.empty() &&
                mask.at<uint8_t>(static_cast<int>(c.y),
            ...
            gradient_vec.emplace_back(
                GradientVector(std::cos(c.angle), std::sin(c.angle)));
            grid.occupancy_[grid.getCellIndex(c.x, c.y)] = true;
            grid.feature_occupancy_[grid.getCellIndex(c.x, c.y)] =
                Keypoint(c.x, c.y);
        }
    }
...
//------------------------------------------------------------------------------
void fastDetector(const ImgPyr& img_pyr,
                  const int threshold,
                  const int border,
    ...
    for (size_t level = min_level; level <= max_level; ++level) {
        ...
        fast::fast_corner_detect_10((fast::fast_byte*)img_pyr[L].data,
                                    ...
                                    img_pyr[L].step, threshold, fast_corners);
#endif
        std::vector<int> scores, nm_corners;
        fast::fast_corner_score_10((fast::fast_byte*)img_pyr[level].data,
                                   img_pyr[level].step, fast_corners, threshold,
                                   scores);
        ...
        for (const int& i : nm_corners) {
            ...
            if (grid.occupancy_.at(k)) continue;
            const float score =
                scores.at(i);  // vk::shiTomasiScore(img_pyr[L], xy.x, xy.y);
            if (score > corners.at(k).score)
                corners.at(k) =
                    Corner(xy.x * scale, xy.y * scale, score, level, 0.0f);
        }
    }
}

//------------------------------------------------------------------------------
void shiTomasiDetector(const ImgPyr& img_pyr,
                       const int threshold,
                       const int border,
                       const size_t min_level,
                       const size_t max_level,
    ...
    for (size_t level = min_level; level <= max_level; ++level) {
        ...
        for (size_t i = 0; i < nrows; ++i) {
            for (size_t j = 0; j < ncols; ++j) {
                double score;
                if (getShiTomasiScore(img_pyr[level], Eigen::Vector2i(i, j),
                                      &score)) {
                    if (score > threshold) {
...
//------------------------------------------------------------------------------
void edgeletDetector_V1(const ImgPyr& img_pyr,
                        const int threshold,
                        const int border,
                        const int min_level,
                        const int max_level,
    ...
    for (int level = min_level + 1; level <= max_level;
         ++level)  // note, we start one level higher, so we already have some
                   // smoothing
    {
        const int scale = (1 << level);
        ...
        for (int y = border; y < max_row; ++y) {
            ...
            for (int x = border; x < max_col; ++x) {
                ...
                const double score =
                    std::sqrt(p_dx[x] * p_dx[x] + p_dy[x] * p_dy[x]);
                if (score > c.score) {
                    c.x = x * scale;
                    c.y = y * scale;
                    c.level = level - 1;
...
//------------------------------------------------------------------------------
void edgeletDetector_V2(const ImgPyr& img_pyr,
                        const int threshold,
                        const int border,
                        const int min_level,
                        const int max_level,
    ...
    for (int y = border; y < max_row; ++y) {
        ...
        int16_t* p_dy = dy.ptr<int16_t>(y);
        for (int x = border; x < max_col; ++x) {
            angle.at<uint8_t>(y, x) =
                ((std::atan2(p_dy[x], p_dx[x]) + M_PI) / (2.0f * M_PI)) * 10;
    ...
    for (int y = border; y < score.rows - border; ++y) {
        ...
        for (int x = border; x < score.cols - border; ++x, ++p) {
            ...
            if (*p > c.score) {
                ...
                c.level = level - 1;
                c.score = *p;
                c.angle = getAngleAtPixelUsingHistogram(
...
//------------------------------------------------------------------------------
bool getCornerAngle(const ImgPyr& img_pyr,
    ...
    for (int y = 0; y < kPatchSize; ++y) {
        uint8_t* it = img.data + (px(1) + y - kHalfPatchSize) * stride +
                      (px(0) - kHalfPatchSize);
...
//------------------------------------------------------------------------------
bool getShiTomasiScore(const cv::Mat& img,
    ...
    if (x_min < 1 || x_max >= img.cols - 1 || y_min < 1 ||
        y_max >= img.rows - 1)
        return false;
    ...
    *score = 0.5f *
             (dXX + dYY - std::sqrt((dXX - dYY) * (dXX - dYY) + 4 * dXY * dXY));
    return true;
}
...
//------------------------------------------------------------------------------
void computeDerivMaxMagnitude(const cv::Mat& img_8u, cv::Mat& mag_8u) {
...
//------------------------------------------------------------------------------
void computeDerivHuangMumford(const cv::Mat& img_8u,
                              cv::Mat& mag_32f,
                              float alpha,
                              float q) {
    ...
    for (int y = 0; y < sz.height; ++y) {
        ...
        for (int x = 0; x < sz.width; ++x) {
            ...
            float img_xp =
                (x == sz.width - 1) ? (float)rimg[x] : (float)rimg[x + 1];
            float img_yp = (y == sz.height - 1) ? (float)rimg[x]
                                                : (float)rimg[x + img_stride];
            // uchar -> float
            img_c /= 255.0f;
            img_xp /= 255.0f;
            img_yp /= 255.0f;
...
//------------------------------------------------------------------------------
void nonmax(const cv::Mat& img_32f, const float thresh, Corners* corners) {
    // 8-neighbor nonmax suppression
...
//------------------------------------------------------------------------------
void displayGrid(const OccupandyGrid2D& old_grid,
                 const Keypoints& keypoints,
                 const int img_width,
                 const int img_height) {
    // copy detected features in feature_occupancy grid
    OccupandyGrid2D new_grid(old_grid.cell_size, old_grid.n_cols,
                             old_grid.n_rows);
...
void nonlinearDiffusion(const cv::Mat& img_8u,
    ...
    cv::Mat kernel_deriv_xy = (cv::Mat_<float>(3, 3) << 0.25, 0.00, -0.25, 0.00,
    ...
    for (int iter = 0; iter < num_iter; ++iter) {
        ...
        cv::filter2D(Lt, Lx, CV_32FC1, kernel_deriv_x, anchor, 0,
                     cv::BORDER_REPLICATE);
        cv::filter2D(Lt, Ly, CV_32FC1, kernel_deriv_x.t(), anchor, 0,
                     cv::BORDER_REPLICATE);
        cv::filter2D(Lt, Lxx, CV_32FC1, kernel_deriv_xx, anchor, 0,
                     cv::BORDER_REPLICATE);
        cv::filter2D(Lt, Lyy, CV_32FC1, kernel_deriv_xx.t(), anchor, 0,
                     cv::BORDER_REPLICATE);
        cv::filter2D(Lt, Lxy, CV_32FC1, kernel_deriv_xy, anchor, 0,
                     cv::BORDER_REPLICATE);
        ...
        cv::Mat numerator =
            Ly_Squared.mul(Lxx) - 2 * Lx.mul(Ly.mul(Lxy)) + Lx_Squared.mul(Lyy);
...
//------------------------------------------------------------------------------
void detectCannyEdges(const cv::Mat& src_gray,
                      cv::Mat& dest,
                      int low_threshold,
                      int ratio,
    ...
    // Reduce noise
    // cv::blur(src_gray, dest, cv::Size(smoothing_kernel_size,
    // smoothing_kernel_size));
    // cv::GaussianBlur(src_gray, dest, cv::Size(smoothing_kernel_size,
...
//------------------------------------------------------------------------------
void detectSobelEdges(const cv::Mat& src_gray,
                      cv::Mat& dest,
                      int low_threshold,
                      int kernel_size) {
    int scale = 1;
    ...
    cv::Sobel(blurred_input, grad_x, ddepth, 1, 0, 3, scale, delta,
              cv::BORDER_DEFAULT);
    cv::convertScaleAbs(grad_x, abs_grad_x);
    cv::Sobel(blurred_input, grad_y, ddepth, 0, 1, 3, scale, delta,
              cv::BORDER_DEFAULT);
    ...
    cv::threshold(dest, dest, low_threshold, 255, cv::THRESH_BINARY);
}

//------------------------------------------------------------------------------
void drawFeatures(const Frame& frame,
                  const size_t level,
...
void nonmax_3x3(const std::vector<Eigen::Vector2i>& corners,
    ...
    for (int i = 0; i < sz; i++) {
        ...
        if (i > 0)
            ...
            if (corners[i - 1].y() == pos.y() - 1 &&
                corners[i - 1].x() == pos.x() && scores[i - 1] >= score)
                continue;
        ...
        if (pos.x() != 0 && row_start[pos.x() - 1] != -1) {
            ...
            // current point,
            // if it exists.
            for (; corners[point_above].x() < pos.x() &&
                   corners[point_above].y() < pos.y() - 1;
                 point_above++) {
            }

            for (int i = point_above;
                 corners[i].x() < pos.x() && corners[i].y() <= pos.y() + 1;
                 i++) {
                int y = corners[i].y();
                if ((y == pos.y() - 1 || y == pos.y() || y == pos.y() + 1) &&
                    (scores[i] >= score))
                    goto cont;
            }
        }
        // Check below (if there is anything below)
        if (pos.x() != last_row && row_start[pos.x() + 1] != -1 &&
            point_below < sz)  // Nothing below
        {
            ...
            // exists.
            for (;
                 point_below < sz && corners[point_below].x() == pos.x() + 1 &&
                 corners[point_below].y() < pos.y() - 1;
                 point_below++) {
            }

            for (int i = point_below; i < sz && corners[i].x() == pos.x() + 1 &&
                                      corners[i].y() <= pos.y() + 1;
                 i++) {
                int y = corners[i].y();
                if ((y == pos.y() - 1 || y == pos.y() || y == pos.y() + 1) &&
...
void angleHistogram(const cv::Mat& img,
    ...
    for (int dy = -halfpatch_size; dy <= halfpatch_size; ++dy) {
        for (int dx = -halfpatch_size; dx <= halfpatch_size; ++dx) {
            double mag, angle;
            if (gradientAndMagnitudeAtPixel(img, x + dx, y + dy, &mag,
                                            &angle)) {
