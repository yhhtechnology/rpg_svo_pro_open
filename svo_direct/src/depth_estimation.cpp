...
DepthEstimator::DepthEstimator(const SolverOptions& solver_options)
    : vk::solver::MiniLeastSquaresSolver<1, double, DepthEstimator>(
          solver_options) {
    ;
}
...
void DepthEstimator::run(const FramePtr& cur_frame,
...
double DepthEstimator::evaluateError(const DepthEstimatorState& inverse_depth,
                                     HessianMatrix* H,
                                     GradientVector* g) {
    const BearingVector f_cur = T_cur_ref_.getRotation().rotate(f_ref_) +
                                T_cur_ref_.getPosition() * inverse_depth;
    Keypoint px_cur;
    Eigen::Matrix<double, 2, 3> projection_jacobian;
    cur_frame_->cam_->project3(f_cur, &px_cur, &projection_jacobian);
    if (!cur_frame_->cam_->isKeypointVisibleWithMargin(
            px_cur, (kPatchHalfsize + 3) * level_)) {
    ...
    warp::getWarpMatrixAffine(ref_frame_->cam(), cur_frame_->cam(), px_ref_,
                              f_ref_, 1.0 / std::max(0.000001, inverse_depth),
                              T_cur_ref_, level_, &A_cur_ref);
    if (!warp::warpAffine(A_cur_ref, ref_frame_->img_pyr_[level_], px_ref_,
                          level_, level_, kPatchHalfsize + 1,
                          ref_patch_with_border_)) {
    ...
    patch_utils::createPatchFromPatchWithBorder(ref_patch_with_border_,
                                                kPatchSize, ref_patch_);

    uint8_t cur_patch_with_border[(kPatchSize + 2) * (kPatchSize + 2)]
        __attribute__((aligned(16)));
    Keypoint px_cur_vec = px_cur / (1 << level_);
    warp::createPatchNoWarpInterpolated(cur_frame_->img_pyr_[level_],
                                        px_cur_vec, kPatchHalfsize + 1,
                                        cur_patch_with_border);

    if (VLOG_IS_ON(200)) {
        cv::Mat img_cur_rgb(cur_frame_->img_pyr_[level_].size(), CV_8UC3);
        cv::cvtColor(cur_frame_->img_pyr_[level_], img_cur_rgb,
                     cv::COLOR_GRAY2RGB);
        cv::Mat img_ref_rgb(ref_frame_->img_pyr_[level_].size(), CV_8UC3);
        cv::cvtColor(ref_frame_->img_pyr_[level_], img_ref_rgb,
                     cv::COLOR_GRAY2RGB);
        const Eigen::Vector2d px_ref_vec = px_ref_ / (1 << level_);
        cv::rectangle(img_cur_rgb, cv::Rect(px_cur_vec(0), px_cur_vec(1),
                                            kPatchSize + 3, kPatchSize + 3),
                      cv::Scalar(0, 255, 1));
        cv::rectangle(img_ref_rgb, cv::Rect(px_ref_vec(0), px_ref_vec(1),
                                            kPatchSize + 3, kPatchSize + 3),
                      cv::Scalar(0, 255, 1));
        cv::imshow("img_cur_rgb", img_cur_rgb);
        cv::imshow("img_ref_rgb", img_ref_rgb);
        cv::Mat img_cur(kPatchSize + 2, kPatchSize + 2, CV_8UC1,
                        cur_patch_with_border);
        cv::Mat img_ref(kPatchSize + 2, kPatchSize + 2, CV_8UC1,
                        ref_patch_with_border_);
    ...
    for (int y = 0; y < kPatchSize; ++y) {
        uint8_t* cur_px =
            (uint8_t*)cur_patch_with_border + (y + 1) * patch_step + 1;
        uint8_t* ref_px =
            (uint8_t*)ref_patch_with_border_ + (y + 1) * patch_step + 1;
        for (int x = 0; x < kPatchSize; ++x, ++cur_px, ++ref_px) {
            Eigen::Vector2d grad(
                0.5 * (cur_px[1] - cur_px[-1]),
                0.5 * (cur_px[patch_step] - cur_px[-patch_step]));
            double residual = *cur_px - *ref_px;
            double J =
                grad.dot(projection_jacobian *
                         T_cur_ref_.getRotation().rotate(
                             f_ref_ * (-1.0 / std::pow(inverse_depth, 2))));
...
void DepthEstimator::update(const DepthEstimatorState& state_old,
                            const UpdateVector& dx,
                            DepthEstimatorState& state_new) {
    state_new = state_old + dx(0);
}

bool DepthEstimator::solve(const HessianMatrix& H,
