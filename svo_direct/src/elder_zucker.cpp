...
void detectEdges(const std::vector<cv::Mat>& img_pyr,
                 const double sigma,
                 cv::Mat& edge_map,
                 cv::Mat& level_map) {
    //  printf("detect edges\n");
    ...
    for (int L = 0; L < n_levels; ++L) {
        ...
        const int delta = 0;
        cv::Mat dx, dy;
        cv::Scharr(img_pyr_smoothed[L], dx, CV_16S, 1, 0, 1, delta,
                   cv::BORDER_DEFAULT);
        cv::Scharr(img_pyr_smoothed[L], dy, CV_16S, 0, 1, 1, delta,
                   cv::BORDER_DEFAULT);
        ...
        for (int y = 0; y < n_rows; ++y) {
            ...
            for (int x = 0; x < n_cols; ++x) {
                ...
                float mag =
                    std::cos(p_a[x]) * p_dx[x] + std::sin(p_a[x]) * p_dy[x];
                if (std::abs(mag) < c1) p_a[x] = 0;
            }
        }
    }
    ...
    for (int y = 0; y < img_pyr_smoothed[0].rows; ++y) {
        ...
        for (int x = 0; x < img_pyr_smoothed[0].cols; ++x) {
            for (int L = 0; L < n_levels; ++L) {
                const float a =
                    angle_pyr[L].at<float>(y / (1 << L), x / (1 << L));
    ...
    for (int L = 0; L < n_levels; ++L) {
        ...
        for (int y = 0; y < n_rows; ++y) {
            ...
            for (int x = 0; x < n_cols; ++x) {
                ...
                p_l[x] = (ca * ca * p_dxdx[x]) + (sa * sa * p_dydy[x]) -
                         (2 * ca * sa * p_dxdy[x]);
                if (fabs(p_l[x]) < c2) p_l[x] = 0;
            }
        }
    }
    ...
    for (int y = 0; y < img_pyr_smoothed[0].rows; ++y) {
        ...
        for (int x = 0; x < img_pyr_smoothed[0].cols; ++x) {
            for (int L = 0; L < n_levels; ++L) {
                const float e =
                    lap_of_gau_pyr[L].at<float>(y / (1 << L), x / (1 << L));
                if (e != 0.0) {
                    p_e[x] = e;
                    p_lev[x] = L;
                    break;
...
void getCovarEntries(const cv::Mat& src,
                     cv::Mat& dxdx,
                     cv::Mat& dydy,
                     cv::Mat& dxdy) {
#ifdef __SSSE3__
    ...
    __m128i mask_hi =
        _mm_set_epi8(0x00, 0xFF, 0x00, 0xFF, 0x00, 0xFF, 0x00, 0xFF, 0x00, 0xFF,
                     0x00, 0xFF, 0x00, 0xFF, 0x00, 0xFF);
    __m128i mask_lo =
        _mm_set_epi8(0xFF, 0x00, 0xFF, 0x00, 0xFF, 0x00, 0xFF, 0x00, 0xFF, 0x00,
                     0xFF, 0x00, 0xFF, 0x00, 0xFF, 0x00);

    for (unsigned int i = 0; i < maxI; ++i) {
        bool end = false;
        for (unsigned int j = 0; j < maxJ;) {
            //__m128i result = _mm_set_epi16 (
            ...
            for (unsigned int x = 0; x < X; ++x) {
                ...
                for (unsigned int y = 0; y < Y; ++y) {
                    ...
                    __m128i mult_dx = _mm_set_epi16(m_dx, m_dx, m_dx, m_dx,
                                                    m_dx, m_dx, m_dx, m_dx);
                    __m128i mult_dy = _mm_set_epi16(m_dy, m_dy, m_dy, m_dy,
                                                    m_dy, m_dy, m_dy, m_dy);
                    uchar* p = (src.data + (stride * (i + y)) + x + j);
                    __m128i i0 = _mm_loadu_si128((__m128i*)p);
                    __m128i i0_hi = _mm_and_si128(i0, mask_hi);
                    __m128i i0_lo =
                        _mm_srli_si128(_mm_and_si128(i0, mask_lo), 1);
            ...
            // calculate covariance entries - remove precision (ends up being 4
            // bit), then remove 4 more bits
            __m128i i_hi_dx_dx =
                _mm_srai_epi16(_mm_mulhi_epi16(result_hi_dx, result_hi_dx), 4);
            __m128i i_hi_dy_dy =
                _mm_srai_epi16(_mm_mulhi_epi16(result_hi_dy, result_hi_dy), 4);
            __m128i i_hi_dx_dy =
                _mm_srai_epi16(_mm_mulhi_epi16(result_hi_dy, result_hi_dx), 4);
            __m128i i_lo_dx_dx =
                _mm_srai_epi16(_mm_mulhi_epi16(result_lo_dx, result_lo_dx), 4);
            __m128i i_lo_dy_dy =
                _mm_srai_epi16(_mm_mulhi_epi16(result_lo_dy, result_lo_dy), 4);
            __m128i i_lo_dx_dy =
                _mm_srai_epi16(_mm_mulhi_epi16(result_lo_dy, result_lo_dx), 4);

            // store
            uchar* p_lo_dxdx =
                (dxdx.data + (2 * stride * (i + cy))) + 2 * cx + 2 * j;
            uchar* p_hi_dxdx =
                (dxdx.data + (2 * stride * (i + cy))) + 2 * cx + 2 * j + 16;
            ...
            uchar* p_lo_dydy =
                (dydy.data + (2 * stride * (i + cy))) + 2 * cx + 2 * j;
            uchar* p_hi_dydy =
                (dydy.data + (2 * stride * (i + cy))) + 2 * cx + 2 * j + 16;
            ...
            uchar* p_lo_dxdy =
                (dxdy.data + (2 * stride * (i + cy))) + 2 * cx + 2 * j;
            uchar* p_hi_dxdy =
                (dxdy.data + (2 * stride * (i + cy))) + 2 * cx + 2 * j + 16;
...
void filterGauss3by316S(const cv::Mat& src, cv::Mat& dst) {
#ifdef __SSSE3__
    // sanity check
    ...
    for (unsigned int i = 0; i < maxI; ++i) {
        bool end = false;
        for (unsigned int j = 0; j < maxJ;) {
            // enter convolution with kernel. do the multiplication with 2/4 at
            // the same time
            ...
            __m128i i22 =
                _mm_loadu_si128((__m128i*)&src.at<short>(i + 2, j + 2));
            __m128i result = i11;
