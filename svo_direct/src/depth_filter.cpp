...
DepthFilter::DepthFilter(const DepthFilterOptions& options,
    ...
    feature_detector_ = feature_detection_utils::makeDetector(
        detector_options, cams->getCameraShared(0));
...
DepthFilter::DepthFilter(const DepthFilterOptions& options)
    ...
    if (options_.use_threaded_depthfilter) startThread();
}
...
void DepthFilter::addKeyframe(const FramePtr& frame,
    ...
    if (thread_ == nullptr) {
        ...
        depth_filter_utils::initializeSeeds(frame, feature_detector_,
        ...
        if (options_.extra_map_points) {
            for (size_t idx = 0; idx < frame->numFeatures(); idx++) {
                ...
                if (!isMapPoint(type) && type != FeatureType::kOutlier) {
                    sec_feature_detector_->closeness_check_grid_
                        .fillWithKeypoints(
            ...
            depth_filter_utils::initializeSeeds(
                ...
                options_.max_n_seeds_per_frame +
                    options_.max_map_seeds_per_frame,
                depth_min, depth_max, depth_mean);
        }
    } else {
...
void DepthFilter::updateSeedsLoop() {
    while (true) {
        ...
        {
            ulock_t lock(jobs_mut_);
            while (jobs_.empty() && !quit_thread_) jobs_condvar_.wait(lock);
        ...
        if (job.type == Job::SEED_INIT) {
            ...
            depth_filter_utils::initializeSeeds(
                ...
                options_.max_n_seeds_per_frame, job.min_depth, job.max_depth,
                job.mean_depth);
            if (options_.extra_map_points) {
                for (size_t idx = 0; idx < job.cur_frame->numFeatures();
                     idx++) {
                    const FeatureType& type = job.cur_frame->type_vec_[idx];
                    if (!isMapPoint(type) && type != FeatureType::kOutlier) {
                        sec_feature_detector_->closeness_check_grid_
                            .fillWithKeypoints(job.cur_frame->px_vec_.col(
                                static_cast<int>(idx)));
                    }
                }
                depth_filter_utils::initializeSeeds(
            ...
            depth_filter_utils::updateSeed(
                *job.cur_frame, *job.ref_frame, job.ref_frame_seed_index,
                *matcher_, options_.seed_convergence_sigma2_thresh, true,
                false);
        }
    }
}
...
size_t DepthFilter::updateSeeds(
    ...
    if (thread_ == nullptr) {
        for (const FramePtr& ref_frame : ref_frames_with_seeds) {
            for (size_t i = 0; i < ref_frame->num_features_; ++i) {
                ...
                if (isSeed(type)) {
                    ...
                    // we use a different threshold for map points to get better
                    // accuracy
                    if (type == FeatureType::kMapPointSeed ||
                        type == FeatureType::kMapPointSeedConverged) {
                        cur_thresh =
                            options_.mappoint_convergence_sigma2_thresh;
                    }
                    // We get higher precision (10x in the synthetic blender
                    // dataset)
                    // when we keep updating seeds even though they are
                    // converged until
                    // the frame handler selects a new keyframe.
        ...
        SVO_DEBUG_STREAM("DepthFilter: " << cur_frame->cam()->getLabel()
                                         << " updated " << n_success
                                         << " Seeds successfully.");
    } else {
        ulock_t lock(jobs_mut_);
...
void initializeSeeds(const FramePtr& frame,
    ...
    //! @todo (MWE) FIXME - When we (ab)use the init seeds to initialize the
    //! first
    //! seeds the check px_vec_.cols()<(n_new+n_old) will fail but in case we
    //! don't
    //! have any old features we don't have concurrency issues and we just do
    //! it;
    //! still we should resolve this at some point and have a clean init /
    //! update procedure
    //!
    //! maybe the detector should take over more of these things as the
    //! detectors
    ...
    if (no_features_in_frame) {
        ...
        feature_detector->detect(frame->img_pyr_, frame->getMask(),
                                 max_n_features, frame->px_vec_,
                                 frame->score_vec_, frame->level_vec_,
                                 frame->grad_vec_, frame->type_vec_);
        ...
        frame_utils::computeNormalizedBearingVectors(
            frame->px_vec_, *frame->cam(), &frame->f_vec_);
        ...
        feature_detector->detect(frame->img_pyr_, frame->getMask(),
                                 max_n_features, new_px, new_scores, new_levels,
                                 new_grads, new_types);
        frame_utils::computeNormalizedBearingVectors(new_px, *frame->cam(),
                                                     &new_f);
    }
    ...
    // initialize seeds
    frame->seed_mu_range_ =
        seed::getMeanRangeFromDepthMinMax(depth_min, depth_max);
    if (no_features_in_frame) {
        frame->invmu_sigma2_a_b_vec_.block(0, 0, 1, n_old)
            .setConstant(seed::getMeanFromDepth(depth_mean));
        frame->invmu_sigma2_a_b_vec_.block(1, 0, 1, n_old)
            .setConstant(seed::getInitSigma2FromMuRange(frame->seed_mu_range_));
        frame->invmu_sigma2_a_b_vec_.block(2, 0, 2, n_old).setConstant(10.0);
    } else {
        frame->invmu_sigma2_a_b_vec_.block(0, n_old, 1, n_new)
            .setConstant(seed::getMeanFromDepth(depth_mean));
        frame->invmu_sigma2_a_b_vec_.block(1, n_old, 1, n_new)
            .setConstant(seed::getInitSigma2FromMuRange(frame->seed_mu_range_));
        frame->invmu_sigma2_a_b_vec_.block(2, n_old, 2, n_new)
            .setConstant(10.0);
    }
...
bool updateSeed(const Frame& cur_frame,
                Frame& ref_frame,
    ...
    if (res != Matcher::MatchResult::kSuccess) {
        ...
        if (matcher.options_.verbose) {
            std::cout << "filter fail = " << Matcher::getResultString(res)
                      << std::endl;
        }
        return false;
    }
    ...
    // compute tau
    const FloatType depth_sigma =
        computeTau(T_cur_ref.inverse(), ref_ftr.f, depth, px_error_angle);
    ...
    if (use_vogiatzis_update) {
        if (!updateFilterVogiatzis(
                ...
                ref_frame.seed_mu_range_, state)) {
    ...
    // check if converged
    if (seed::isConverged(state, ref_frame.seed_mu_range_,
...
bool updateFilterVogiatzis(const FloatType z,  // Measurement
...
bool updateFilterGaussian(const FloatType z,  // Measurement
                          const FloatType tau2,
                          Eigen::Ref<SeedState>& mu_sigma2_a_b) {
    FloatType& mu = mu_sigma2_a_b(0);
    FloatType& sigma2 = mu_sigma2_a_b(1);
...
double computeEpiGradAngle(const Transformation& T_cur_ref,
    ...
    const BearingVector l(
        e_hom.cross(u_infty_hom));  // epipolar line in homogeneous coordinates

    const BearingVector f_ref_plus(
        f_ref + BearingVector(0.1 * grad_ref[0], 0.1 * grad_ref[1], f_ref[2]));
...
bool setSeedCovariance(const int halfpatch_size,
                       const double image_noise2,
                       SeedImplementation::Ptr seed) {
    FramePtr frame = seed->ftr_->frame.lock();
    if (!frame) {
        SVO_ERROR_STREAM(
            "Could not lock weak_ptr<Frame> in DepthFilter::setSeedCovariance");
        return false;
    }
    Eigen::Matrix2d H;
    H.setZero();
    ...
    for (int y = 0; y < patch_size; ++y) {
        uint8_t* p =
            img.data + (v - halfpatch_size + y) * step + (u - halfpatch_size);
        for (int x = 0; x < patch_size; ++x, ++p) {
            const Eigen::Vector2d J((p[1] - p[-1]), (p[step] - p[-step]));
            H += J * J.transpose();
        }
    }
    H /= 260100.0 * patch_size * patch_size;  // 260100 = 2^2 * 255^2 for the
                                              // missing 0.5 of the derivative
                                              // and the 255 of uint8_t image
...
double getSeedDisparityUncertainty(const SeedImplementation::Ptr& seed,
                                   const Transformation& T_cur_ref) {
    // compute epipolar line in current image
    // we use the essential matrix since we will only need the angle of the line
    // which should not be affected by the K-matrix. (distortion yes, but
    // hopefully
    // neglectible).
    Eigen::Matrix3d E_cur_ref(vk::skew(T_cur_ref.getPosition()) *
                              T_cur_ref.getRotationMatrix());
    Eigen::Vector3d l_cur(E_cur_ref * seed->ftr_->f);

    // TODO apply rotation to l_cur to get l_ref

    // find uncertainty in direction of epipolar line, marginalize bivariate
    // distribution
    double epi_angle_cur = atan(-l_cur[0] / l_cur[1]);
    double s = sin(-epi_angle_cur);
    double c = cos(-epi_angle_cur);
    double disp_sigma2 =
        seed->patch_cov_.determinant() /
        (seed->patch_cov_(1, 1) * c * c + 2 * seed->patch_cov_(0, 1) * s * c +
         seed->patch_cov_(0, 0) * s * s);
    return fmax(disp_sigma2, 1.0);  // we don't want less than 1.0px
                                    // uncertainty. because e.g. sampling errors
}
#endif
