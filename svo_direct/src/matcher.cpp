...
Matcher::MatchResult Matcher::findMatchDirect(const Frame& ref_frame,
    ...
    if (pxi[0] < boundary || pxi[1] < boundary ||
        pxi[0] >= static_cast<int>(ref_frame.cam()->imageWidth() /
                                   (1 << ref_ftr.level)) -
                      boundary ||
        pxi[1] >= static_cast<int>(ref_frame.cam()->imageHeight() /
                                   (1 << ref_ftr.level)) -
                      boundary)
    ...
    warp::getWarpMatrixAffine(ref_frame.cam_, cur_frame.cam_, ref_ftr.px,
                              ref_ftr.f, ref_depth,
                              cur_frame.T_cam_world() * ref_frame.T_world_cam(),
                              ref_ftr.level, &A_cur_ref_);
    search_level_ =
    ...
    */
    if (options_.use_affine_warp_) {
        ...
        if (!warp::warpPixelwise(cur_frame, ref_frame, ref_ftr, ref_ftr.level,
                                 search_level_, kHalfPatchSize + 1,
                                 patch_with_border_))
            return MatchResult::kFailWarp;
    }
    ...
    if (isEdgelet(ref_ftr.type)) {
        ...
        if (feature_alignment::align1D(
                ...
                patch_, options_.align_max_iter, options_.affine_est_offset_,
...
Matcher::MatchResult Matcher::findEpipolarMatchDirect(
    ...
    // image plane
    const BearingVector A = T_cur_ref.getRotation().rotate(ref_ftr.f) +
    ...
    if (isEdgelet(ref_ftr.type) && options_.epi_search_edgelet_filtering) {
        const Eigen::Vector2d grad_cur =
            (A_cur_ref_ * ref_ftr.grad).normalized();
...
bool Matcher::isPatchWithinImage(const Frame& frame,
                                 const Eigen::Vector2i& pxi,
                                 const int patch_level) {
    return !(
...
void Matcher::scanEpipolarLine(const Frame& frame,
...
void Matcher::scanEpipolarUnitSphere(const Frame& frame,
                                     const Eigen::Vector3d& A,
                                     const Eigen::Vector3d& B,
                                     const Eigen::Vector3d& C,
                                     const PatchScore& patch_score,
...
Matcher::MatchResult depthFromTriangulation(const Transformation& T_search_ref,
    ...
    const Eigen::Vector2d depth2 =
        -AtA.inverse() * A.transpose() * T_search_ref.getPosition();
