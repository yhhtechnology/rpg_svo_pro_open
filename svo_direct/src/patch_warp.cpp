...
void getWarpMatrixAffineHomography(const CameraPtr& cam_ref,
                                   const CameraPtr& cam_cur,
...
bool warpAffine(const AffineTransformation2& A_cur_ref,
...
bool warpPixelwise(const Frame& cur_frame,
    ...
    Eigen::Vector3d xyz_cur =
        cur_frame.T_cam_world() * (ref_frame.T_cam_world().inverse()) * xyz_ref;
    ...
    for (int y = -halfpatch_size; y < halfpatch_size; ++y) {
        for (int x = -halfpatch_size; x < halfpatch_size; ++x, ++patch_ptr) {
            ...
            cur_frame.cam()->backProject3(ele_search * (1 << level_cur),
                                          &ele_xyz_cur);
            ele_xyz_cur = ele_xyz_cur.normalized() * depth_cur;
            Eigen::Vector3d ele_xyz_ref = ref_frame.T_cam_world() *
                                          (cur_frame.T_cam_world().inverse()) *
                                          ele_xyz_cur;
            ...
            if (xi < 0 || yi < 0 || xi + 1 >= img_ref.cols ||
                yi + 1 >= img_ref.rows) {
                VLOG(200) << "ref image: col-" << img_ref.cols << ", row-"
                          << img_ref.rows;
                VLOG(200) << "xi: " << xi << ", "
                          << "yi: " << yi;
                return false;
...
void createPatchNoWarp(const cv::Mat& img,
                       const Eigen::Vector2i& px,
                       const int halfpatch_size,
                       uint8_t* patch) {
    CHECK_NOTNULL(patch);
    CHECK(px(0) >= halfpatch_size && px(1) >= halfpatch_size &&
...
void createPatchNoWarpInterpolated(const cv::Mat& img,
    ...
    CHECK(u_r >= halfpatch_size && v_r >= halfpatch_size &&
          u_r < img.cols - halfpatch_size && v_r < img.rows - halfpatch_size);
    ...
    for (int y = 0; y < patch_size; ++y) {
        uint8_t* img_ptr = (uint8_t*)img.data +
                           ...
                           (u_r - halfpatch_size);
        for (int x = 0; x < patch_size; ++x, ++patch_ptr, ++img_ptr) {
            *patch_ptr = wTL * img_ptr[0] + wTR * img_ptr[1] +
                         wBL * img_ptr[step] + wBR * img_ptr[step + 1];
            ;
        }
    }
}
