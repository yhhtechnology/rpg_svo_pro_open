...
//------------------------------------------------------------------------------
AbstractDetector::AbstractDetector(const DetectorOptions& options,
                                   const CameraPtr& cam)
    : options_(options),
      grid_(options_.cell_size,
            std::ceil(static_cast<double>(cam->imageWidth()) /
                      options_.cell_size),
            std::ceil(static_cast<double>(cam->imageHeight()) /
                      options_.cell_size)),
      closeness_check_grid_(options_.cell_size / options_.sec_grid_fineness,
                            std::ceil(options_.sec_grid_fineness *
                                      static_cast<double>(cam->imageWidth()) /
                                      options_.cell_size),
                            std::ceil(options_.sec_grid_fineness *
                                      static_cast<double>(cam->imageHeight()) /
                                      options_.cell_size)) {}
...
//------------------------------------------------------------------------------
void FastDetector::detect(const ImgPyr& img_pyr,
                          const cv::Mat& mask,
    ...
    fd_utils::fastDetector(img_pyr, options_.threshold_primary, options_.border,
                           options_.min_level, options_.max_level, corners,
                           grid_);
    fd_utils::fillFeatures(corners, FeatureType::kCorner, mask,
                           options_.threshold_primary, max_n_features, px_vec,
                           score_vec, level_vec, grad_vec, types_vec, grid_);

    resetGrid();
}

//------------------------------------------------------------------------------
void GradientDetector::detect(const ImgPyr& img_pyr,
                              const cv::Mat& mask,
                              const size_t max_n_features,
                              Keypoints& px_vec,
                              Scores& score_vec,
...
// const int stride=mag_ss.cols;
  ...
  if(false)
  {
    ...
    fd_utils::fillFeatures(corners, FeatureType::kEdgelet, mask,
                           options_.threshold_secondary, max_n_features, px_vec,
                           score_vec, level_vec, grad_vec, types_vec, grid_);
}

void GradientDetectorGrid::detect(const ImgPyr& img_pyr,
                                  const cv::Mat& mask,
                                  const size_t max_n_features,
                                  Keypoints& px_vec,
                                  Scores& score_vec,
    ...
    fd_utils::edgeletDetector_V2(img_pyr, options_.threshold_secondary,
...
//------------------------------------------------------------------------------
void FastGradDetector::detect(const ImgPyr& img_pyr,
                              const cv::Mat& mask,
                              const size_t max_n_features,
                              Keypoints& px_vec,
                              Scores& score_vec,
                              ...
                              FeatureTypes& types_vec) {
    {
        ...
        Corners corners(grid_.n_cols * grid_.n_rows,
                        Corner(0, 0, options_.threshold_primary, 0, 0.0f));
        fd_utils::fastDetector(img_pyr, options_.threshold_primary,
                               options_.border, options_.min_level,
                               options_.max_level, corners, grid_);
        fd_utils::fillFeatures(corners, FeatureType::kCorner, mask,
                               options_.threshold_primary, max_n_features,
    ...
    if (max_features > 0) {
        ...
        fd_utils::edgeletDetector_V2(img_pyr, options_.threshold_secondary,
                                     ...
                                     options_.max_level, corners, grid_);
        fd_utils::fillFeatures(corners, FeatureType::kEdgelet, mask,
                               options_.threshold_secondary, max_features,
                               px_vec, score_vec, level_vec, grad_vec,
                               types_vec, grid_);
    }
...
//------------------------------------------------------------------------------
void ShiTomasiGradDetector::detect(const ImgPyr& img_pyr,
                                   const cv::Mat& mask,
                                   const size_t max_n_features,
                                   Keypoints& px_vec,
                                   Scores& score_vec,
                                   ...
                                   FeatureTypes& types_vec) {
    {
        ...
        fd_utils::shiTomasiDetector(img_pyr, options_.threshold_shitomasi,
                                    options_.border, options_.min_level,
                                    options_.max_level, corners, grid_,
    ...
    if (max_features > 0) {
        ...
        fd_utils::fillFeatures(corners_fast, FeatureType::kCorner, mask,
                               options_.threshold_primary, max_n_features,
                               px_vec, score_vec, level_vec, grad_vec,
                               types_vec, grid_);
        ...
        fd_utils::edgeletDetector_V2(img_pyr, options_.threshold_secondary,
                                     options_.border, options_.min_level,
                                     options_.max_level, corners_grad, grid_);
        fd_utils::fillFeatures(corners_grad, FeatureType::kEdgelet, mask,
                               options_.threshold_secondary, max_features,
                               px_vec, score_vec, level_vec, grad_vec,
                               types_vec, grid_);
    }
...
void ShiTomasiDetector::detect(const ImgPyr& img_pyr,
                               const cv::Mat& mask,
                               const size_t max_n_features,
                               Keypoints& px_vec,
                               Scores& score_vec,
...
#else  // new
void AllPixelsDetector::detect(const ImgPyr& img_pyr,
                               const cv::Mat& mask,
                               const size_t /*max_n_features*/,
                               Keypoints& px_vec,
                               Scores& score_vec,
    ...
    for (int y = border; y < height - border; ++y) {
        for (int x = border; x < width - border; ++x) {
            px_vec.col(feature_index++) =
                svo::Keypoint(x * pyr_init_scale, y * pyr_init_scale);
        }
    }
}
#endif

void CannyDetector::detect(const ImgPyr& img_pyr,
                           const cv::Mat& mask,
                           const size_t max_n_features,
                           Keypoints& px_vec,
                           Scores& score_vec,
...
void SobelDetector::detect(const ImgPyr& img_pyr,
                           const cv::Mat& mask,
                           const size_t max_n_features,
                           Keypoints& px_vec,
                           Scores& score_vec,
...
//------------------------------------------------------------------------------
void GradientHuangMumfordDetector::detect(const ImgPyr& img_pyr,
                                          const cv::Mat& mask,
                                          const size_t max_n_features,
                                          Keypoints& px_vec,
                                          Scores& score_vec,
    ...
    cv::threshold(mag_level_32f, mag_level_thresholded,
                  options_.threshold_primary, 1.0, cv::THRESH_BINARY_INV);
  ...
  cv::waitKey(0);
    ...
    //  // Create feature for every corner that has high enough corner score
    //  fd_utils::fillFeatures(
    //        corners, FeatureType::kEdgelet, mask,
    //        options_.threshold_secondary,
