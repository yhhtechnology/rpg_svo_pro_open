...
class Matcher {
 public:
    ...
    MatchResult findMatchDirect(const Frame& ref_frame,
    ...
    MatchResult findEpipolarMatchDirect(const Frame& ref_frame,
                                        ...
                                        const double d_max_inv,
                                        double& depth);
    /// search epipolar line between A~C~B for the best match with respect to
    /// patch score
    /// the search is done on patch_level, returns image coordinates and best
    /// ZMSSD
...
void createPatchWithBorderNoWarp(const cv::Mat& img,
...
void createPatchFromPatchWithBorder(const uint8_t* const patch_with_border,
