...
struct DetectorOptions {
    ...
    /// Only for detectors supporting specific feature levels like the
    /// AllPixelDetector.
    int sampling_level = 0;
    int level = 0;

    /// fineness level of the secondary grid (used for extra shi tomasi features
    /// when loop closing is enabled)
    size_t sec_grid_fineness = 1;

    /// Corner Strength Thrshold for shitomasi features (used only when loop
    /// closing is enabled)
