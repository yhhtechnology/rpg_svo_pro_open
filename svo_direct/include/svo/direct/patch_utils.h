...
inline void patchToMat(const uint8_t* const patch_data,
...
inline void normalizeAndUpsamplePatch(const cv::Mat& patch,
    ...
    cv::Mat img_gray;
    cv::resize(patch_normalized, img_gray, cv::Size(0, 0), upsample_factor,
               upsample_factor, cv::INTER_NEAREST);
    *img_rgb = cv::Mat(img_gray.size(), CV_8UC3);
    cv::cvtColor(img_gray, *img_rgb, cv::COLOR_GRAY2RGB);
}

inline void concatenatePatches(std::vector<cv::Mat> patches,
                               cv::Mat* result_rgb) {
