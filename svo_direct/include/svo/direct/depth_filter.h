...
class DepthFilter {
 ...
 public:
    ...
    /// Add new keyframe to the queue, depth_max is only required for
    /// direct-depth
    ...
    /// Update seeds
    /// \param frames_with_seeds List of frames which contain seeds that should
    /// be
    /// updated with new_frame
    /// \param new_frame The new frame that is used as observation to update all
    /// seeds in frames_with_seeds
    size_t updateSeeds(const std::vector<FramePtr>& frames_with_seeds,
    ...
    DetectorPtr
        sec_feature_detector_;  // for extra points used for loop closing
...
bool updateFilterVogiatzis(const FloatType z,
                           const FloatType tau2,
                           const FloatType z_range,
                           Eigen::Ref<SeedState>& seed);

bool updateFilterGaussian(const FloatType z,
                          const FloatType tau2,
                          Eigen::Ref<SeedState>& seed);
