...
void detectNonlinearDiffusionEdges(const ImgPyr& img_pyr) {
    ...
    for (int iter = 0; iter < num_iter; ++iter) {
        ...
        cv::Canny(Lt_u8, edges, canny_low_threshold,
...
void test() {
    ...
    std::string dataset_dir =
        ros::package::getPath("rpg_datasets") + "/rpg_urban_pinhole";
    ...
    if (false) {
        ...
        for (size_t i = 0; i < frame->numFeatures(); ++i) {
            ...
            warp::createPatchNoWarp(
                frame->img_pyr_[level],
                (frame->px_vec_.col(i) / (1 << level)).cast<int>(),
                kHalfPatchSize, patch);
            cv::Mat patch_upsampled;
            cv::Mat patch_img(kPatchSize, kPatchSize, CV_8UC1, patch);
            patch_utils::normalizeAndUpsamplePatch(patch_img, kPatchSize,
                                                   &patch_upsampled);
            ...
            double angle_hist =
                feature_detection_utils::getAngleAtPixelUsingHistogram(
                    ...
                    (frame->px_vec_.col(i) / (1 << level)).cast<int>(),
                    kHalfPatchSize);
            cv::line(patch_upsampled, cv::Point2f(patch_upsampled.rows / 2,
                                                  patch_upsampled.rows / 2),
                     cv::Point2f(
                         patch_upsampled.rows / 2 + 15 * std::cos(angle_hist),
            ...
            cv::line(patch_upsampled, cv::Point2f(patch_upsampled.rows / 2,
                                                  patch_upsampled.rows / 2),
                     cv::Point2f(
                         patch_upsampled.rows / 2 + 15 * std::cos(angle_grad),
            ...
            frame->grad_vec_.col(i) =
                GradientVector(std::cos(angle_hist), std::sin(angle_hist));
        }
        cv::Mat img_rgb_2;
