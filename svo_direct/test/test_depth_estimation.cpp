...
/// Wrapper that brings the distortion function to the form needed by the
/// differentiator.
struct DepthJacobianFunctor : public aslam::common::NumDiffFunctor<2, 1> {
    DepthJacobianFunctor(const svo::Camera::ConstPtr& camera,
                         const Eigen::Vector3d& f_ref,
                         const svo::Transformation& T_cur_ref)
        : camera_(camera), f_ref_(f_ref), T_cur_ref_(T_cur_ref) {
        ;
    }
    ...
    virtual bool functional(
        const typename aslam::common::NumDiffFunctor<1, 1>::InputType&
            inv_depth,
        typename aslam::common::NumDiffFunctor<2, 1>::ValueType& px_cur,
        typename aslam::common::NumDiffFunctor<2, 1>::JacobianType* Jout)
        const {
        CHECK(camera_);
        svo::BearingVector f_cur =
            T_cur_ref_ * (f_ref_ * (1.0 / inv_depth(0.0)));
        ...
        if (Jout) {
            ...
            *Jout = projection_jacobian *
                    T_cur_ref_.getRotation().rotate(
                        f_ref_ * (-1.0 / std::pow(inv_depth(0, 0), 2)));
        }
...
TEST(TestDepthEstimation, JacobianWrtInverseDepth) {
    ...
    TEST_JACOBIAN_FINITE_DIFFERENCE(DepthJacobianFunctor, inv_depth, 1e-8, 1e-6,
                                    camera, f_ref, T_cur_ref);
}
...
TEST(TestDepthEstimation, testBearing) {
    ...
    svo::BearingVector f_cur_1 = T_cur_ref.getRotation().rotate(f_ref) +
                                 T_cur_ref.getPosition() * inv_depth;
...
TEST(TestDepthEstimation, DISABLED_testDataset) {
    std::string dataset_dir =
        ros::package::getPath("rpg_datasets") + "/rpg_vfr_pinhole";
    ...
    ref_frame->invmu_sigma2_a_b_vec_(0, 0) =
        svo::seed::getMeanFromDepth(depth_init);
    VLOG(200) << "ground-truth depth = " << 1.0 / ground_truth_depth;
    svo::DepthEstimator depth_estimator(
        svo::DepthEstimator::getDefaultSolverOptions());
