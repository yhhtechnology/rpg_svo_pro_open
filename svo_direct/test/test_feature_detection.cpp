...
cv::Mat CannyThreshold(cv::Mat& src_gray,
                       int lowThreshold = 20,
                       int ratio = 3,
                       int kernel_size = 3) {
...
int main(int argc, char** argv) {
    ...
    if (argc >= 4) options.threshold_primary = std::atof(argv[3]);
    ...
    while (dataset.getNextFrame(5u, frame, nullptr)) {
        ...
        svo::CameraPtr cam = aslam::createCamera<aslam::PinholeCamera>(
            intrinsics, img_src.cols, img_src.rows);
        ...
        for (size_t i = 0; i < frame->img_pyr_.size(); ++i) {
            ...
            svo::feature_detection_utils::computeDerivHuangMumford(
                blurred_input, mag_pyr_32f[i], atof(std::argv[1]),
                std::atof(std::argv[2]));
        }
        ...
        cv::threshold(mag_level_32f, mag_level_thresholded,
                      detectorOptions.threshold_primary, 1.0,
        ...
        cv::threshold(grad, grad, atof(std::argv[4]), 255, CV_THRESH_BINARY);
