...
void testAlignment1D_sameImg() {
    ...
    cur_frame = ref_frame;
    uint8_t ref_patch_with_border[(kPatchSize + 2) * (kPatchSize + 2)]
    ...
    for (int i = 0; i < ref_frame->px_vec_.cols(); ++i) {
        ...
        warp::createPatchNoWarpInterpolated(ref_frame->img_pyr_[0], kp_ref,
                                            kHalfPatchSize + 1,
                                            ref_patch_with_border);
        patch_utils::createPatchFromPatchWithBorder(ref_patch_with_border,
                                                    kPatchSize, ref_patch);
        ...
        bool res = feature_alignment::align1D(
            cur_frame->img_pyr_[0], ref_frame->grad_vec_.col(i),
...
void testAlignment1D_differentImg() {
    ...
    // For each feature, test 1D alignment.
    Matcher matcher;
    CHECK_EQ(static_cast<size_t>(ref_frame->px_vec_.cols()),
             ref_frame->numFeatures());
    for (int i = 0; i < ref_frame->px_vec_.cols(); ++i) {
        double ref_depth = ref_depthmap.at<float>(ref_frame->px_vec_(1, i),
                                                  ref_frame->px_vec_(0, i));
        const Eigen::Vector3d xyz_cur =
            T_cur_ref * (ref_frame->f_vec_.col(i) * ref_depth);
        Keypoint px_cur;
        ...
        matcher.findMatchDirect(*ref_frame, *cur_frame, ref_ftr, ref_depth,
                                px_cur_distorted);

        // Compute gradient at true location.
        double angle;
        feature_detection_utils::getCornerAngle(cur_frame->img_pyr_, px_cur, 0u,
                                                &angle);
