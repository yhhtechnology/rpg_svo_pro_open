...
class Dataset {
 public:
    ...
    svo::FramePtr getNextImage(size_t img_align_max_level, int user_id) {
        ...
        // make frame
        svo::FramePtr frame = std::make_shared<svo::Frame>(
            cam_, img.clone(), stamp, img_align_max_level + 1, user_id);
 ...
 private:
    ...
    void skipFrames(size_t first_frame_id) {
        ...
        LOG(FATAL)
            << "image list corrupted or first_frame_id exceeds rang listed.";
    }
...
inline Eigen::Vector3d generateRandomPoint(double max_depth, double min_depth) {
    ...
    // between -1 ~ +1
    raw_point[0] =
        (static_cast<double>(rand()) / static_cast<double>(RAND_MAX) - 0.5) * 2;
    raw_point[1] =
        (static_cast<double>(rand()) / static_cast<double>(RAND_MAX) - 0.5) * 2;
    raw_point[2] =
        (static_cast<double>(rand()) / static_cast<double>(RAND_MAX) - 0.5) * 2;
...
inline void calcHist(const std::vector<double>& values,
                     size_t bins,
...
inline cv::Mat drawHist(const std::vector<size_t>& hist,
    ...
    for (size_t i = 0; i < hist_bin_num; i++) {
        ...
        cv::rectangle(
            histogram, cv::Point2f(bin_bl + 0.1 * bin_width, height),
            cv::Point2f(bin_bl + 0.9 * bin_width, height - bin_height),
            cv::Scalar(0), cv::FILLED);
        cv::putText(histogram, ss.str(),
                    cv::Point2f(bin_bl + 0.1 * bin_width, height / 2.0),
                    cv::FONT_HERSHEY_PLAIN, 1.2, cv::Scalar(120), 2);
        cv::putText(histogram, std::to_string(hist[i]),
