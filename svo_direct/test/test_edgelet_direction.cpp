...
void test() {
    ...
    while (dataset.getNextFrame(5u, cur_frame, nullptr)) {
        ...
        for (size_t i = 0; i < ref_frame->numFeatures(); ++i) {
            ...
            Keypoint px;
            if (!cur_frame->cam()->project3(xyz_cur, &px)) continue;

            // Apply matching
            FeatureWrapper ref_ftr = ref_frame->getFeatureWrapper(i);
            if (matcher.findMatchDirect(*ref_frame, *cur_frame, ref_ftr,
                                        ref_depth,
                                        px) == Matcher::MatchResult::kSuccess) {
                ...
                cur_frame->grad_vec_.col(num_matches) =
                    matcher.A_cur_ref_ * ref_ftr.grad;
                ++num_matches;
            }
        }
        cur_frame->num_features_ = num_matches;
        std::cout << "Successfully projected " << num_matches << " features."
                  << std::endl;
