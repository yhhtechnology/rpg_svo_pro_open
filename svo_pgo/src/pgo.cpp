...
void Pgo::addSequentialConstraintToPgoProblem(const Transformation t_be,
    ...
    if (!problem_lock_) {
        ...
        VLOG(40) << "Sequential constraint added for frame "
                 << constraint.id_begin << " and " << constraint.id_end;
        ...
        VLOG(20)
            << "###### Warning: Problem is locked, not adding any sequential "
               "constraints #########";
    }
}
...
void Pgo::addConstraint(const Transformation& t_be,
                        const int& frame_id_b,
                        const int& frame_id_e,
...
void Pgo::solve() {
    ...
    problem_.SetParameterBlockConstant(
        pose_start_iter->second.q.coeffs().data());
    ceres::Solve(options_, &problem_, &summary_);
    VLOG(10) << summary_.FullReport();
}
