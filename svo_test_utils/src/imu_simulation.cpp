...
void simulateBias(const size_t n_measurements,
                  const double dt,
    ...
    for (size_t i = 1; i < n_measurements; ++i) {
        const Eigen::Vector3d N(normal_dist(gen), normal_dist(gen),
                                normal_dist(gen));
...
void simulateBiases(const size_t n_measurements,
                    const double dt,
    ...
    internal::simulateBias(n_measurements, dt,
                           imu_calib.gyro_bias_random_walk_sigma, bias_gyr_init,
                           bias_gyr);
    internal::simulateBias(n_measurements, dt,
                           imu_calib.acc_bias_random_walk_sigma, bias_acc_init,
                           bias_acc);
}
