...
IntersectionResult Plane::intersect(const Ray& ray_obj,
    ...
    // Degenerate cases: ray lies within plane or is parallel to plane without
    // touching it
    if (std::abs(dir[2]) < epsilon) {
        if (std::abs(origin[2]))
            return IntersectionResult(
                IntersectionResult::Status::kLineIntersection);
        else
            return IntersectionResult(
                IntersectionResult::Status::kNoIntersection);
    }
    ...
    *intersection = origin + d * dir;
    ...
    if (d >= 0 && std::abs((*intersection)[0]) <= width_ / 2.0 &&
        std::abs((*intersection)[1]) <= height_ / 2.0) {
        return IntersectionResult(
            IntersectionResult::Status::kUniqueIntersection);
    } else {
        return IntersectionResult(IntersectionResult::Status::kNoIntersection);
    }
}

std::vector<ObjectWithPose> Scene::createBox(double sizeX,
                                             double sizeY,
                                             double sizeZ) {
...
std::vector<ObjectWithPose> Scene::createCanyon(double width,
                                                double length,
                                                double height) {
...
IntersectionResult SimpleRayTracer::intersectPixel(
    const CameraWithPose& cam,
    ...
    for (size_t i = 0; i < scene_.num_objects(); ++i) {
        ...
        const Ray ray_obj(
            T_obj_cam.getPosition(),
            (T_obj_cam.getRotation().rotate(dir_cam)).normalized());
        IntersectionResult res =
            scene_.object(i)->intersect(ray_obj, &intersection_obj);
        if (res.status_ == IntersectionResult::Status::kUniqueIntersection) {
            Eigen::Vector3d current_intersection_cam =
                T_world_cam.inverse() * scene_.T_w_obj(i) * intersection_obj;
...
cv::Mat SimpleRayTracer::render(const CameraWithPose& cam,
    ...
    cv::Mat canvas = cv::Mat::zeros(cam.cam()->imageHeight(),
                                    cam.cam()->imageWidth(), CV_32FC3);
    for (int i = 0; i < canvas.rows; ++i) {
        for (int j = 0; j < canvas.cols; ++j) {
            const Eigen::Vector2d px((double)j, (double)i);

            if (cam.cam()->hasMask() && cam.cam()->isMasked(px)) continue;

            Eigen::Vector3d intersection;
            Object::Ptr intersected_object;
            IntersectionResult res = intersectPixel(
                cam, T_world_cam, px, &intersection, intersected_object);
            if (res.status_ ==
                IntersectionResult::Status::kUniqueIntersection) {
                if (depth_shading)
                    canvas.at<cv::Vec3f>(i, j) =
                        1.f / intersection.norm() *
                        intersected_object->property_.color_;
                else
                    canvas.at<cv::Vec3f>(i, j) =
                        intersected_object->property_.color_;
            }
...
CameraPtr CameraWithPose::createCamera<vk::cameras::PinholeGeometry>(
    ...
    const double f =
        (double)image_width /
        (2.0 * std::tan(0.5 * fov_deg * common::conversions::kDegToRad));
    intrinsics << f, f, (double)image_width / 2.0, (double)image_height / 2.0;
    return vk::cameras::factory::makePinholeCamera(intrinsics, image_width,
                                                   image_height);
}
