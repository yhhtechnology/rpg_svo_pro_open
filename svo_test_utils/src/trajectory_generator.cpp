    ...
    if (fs.is_open()) {
        while (fs >> stamp >> tx >> ty >> tz >> qx >> qy >> qz >> qw) {
            ...
            if (std::abs(q.norm() - 1.0) > 0.000001)
                LOG(WARNING)
                    << "trajectory::Trajectory: Quaternion norm = " << q.norm();
            q.normalize();
            poses.push_back(StampedPose(
                stamp, Transformation(q, Eigen::Vector3d(tx, ty, tz))));
        }
    } else {
...
Trajectory Trajectory::createCircularTrajectory(size_t length,
                                                double fps,
    ...
    for (size_t i = 0; i < length; ++i) {
        ...
        Eigen::Vector3d pos =
            center + radius * Eigen::Vector3d(cos(theta), sin(theta), 0.0);
...
Trajectory Trajectory::createStraightTrajectory(size_t length,
                                                double fps,
    ...
    for (size_t i = 0; i < length; ++i) {
        Eigen::Vector3d current_pos =
            start + (double)i / (double)length * (end - start);
        poses.push_back(StampedPose(
            stamp, Transformation(Eigen::Quaterniond(1.0, 0.0, 0.0, 0.0),
                                  current_pos)));
        stamp += 1.0 / fps;
    }
    return Trajectory(poses);
